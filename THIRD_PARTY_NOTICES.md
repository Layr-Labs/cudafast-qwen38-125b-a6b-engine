...
challenge additionally depends on third-party model artifacts and Rust
...
- The Apache License 2.0 appendix below is retained for the Rust crate
  dependencies listed in the next section.

## Rust crate dependencies

The Apple-Metal/Swift seed package was removed in the final de-Swift, and with
it every SwiftPM dependency (`swift-transformers` and the vendored MLX/Metal
forks `Layr-Labs/mlx-swift`, `Layr-Labs/mlx-swift-lm`). The scored engine is now
the Rust Engine Protocol v1 adapter (`harness/protocol-adapter`), which links
(does not vendor) the two crates below and their transitive graph, resolved
against the committed `harness/protocol-adapter/Cargo.lock`. Each license text is
available in the corresponding crate's repository.

| Crate | License |
|---|---|
| `serde`, `serde_json` (and their transitive dependencies) | MIT OR Apache-2.0 |

## ds4 engine

The `ds4/` directory is the `Layr-Labs/ds4` inference engine, our port of
`antirez/ds4`, VENDORED into this repository as plain files from one signed
commit (`ds4/VENDOR.json` records which). Both are MIT licensed. The port's `LICENSE`
file carries the copyright notices of the ds4.c authors and of the GGML
authors, whose GGUF quant layouts, tables and CPU quant/dot logic it retains
under the MIT license. The engine is built from that tree by
`tools/ds4/build.sh`. The port's own history lives in `Layr-Labs/ds4` and lands
here through `tools/ds4/vendor-sync.sh`; a participant's changes to the engine
live in their submission, which is the point of vendoring it.

| Artifact | Upstream | License |
|---|---|---|
| ds4 port, vendored @ `278b799b974cb580e0f96ed3dce68bfbe0d8675b` | <https://github.com/Layr-Labs/ds4> | MIT |
| ds4, the upstream the port forks | <https://github.com/antirez/ds4> | MIT |
| GGML/llama.cpp pieces retained inside ds4 | <https://github.com/ggml-org/llama.cpp> | MIT |
| Qwen3.8-Flash-Next UD-Q4_K_XL GGUF | <https://huggingface.co/unsloth/Qwen3.8-Flash-Next-GGUF> | Qwen Community 1.0 (base model) |

## Tooling

- `macmon` (macOS GPU telemetry used by the retired MLX local thermal gate) is
  <https://github.com/vladkens/macmon>, MIT; it is not part of this repository.
  The CUDA ranked box reads GPU temperature through `nvidia-smi` instead.
