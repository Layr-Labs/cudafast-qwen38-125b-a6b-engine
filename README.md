...
> **NOTE — the track is UNARMED until the goldens are re-authored on ds4.**
> Three statements describe the arm state.

1. `fixtures/qwen3_8_125b_a6b_track.json` sets `official_scoring_enabled` to
   `false`. That flag is the single authority on the arm state, and the
   benchmarker enforces it: it refuses to seal an official scoring artifact
   while the flag is `false` or absent. It flips back to `true` only after the
   goldens below are re-authored on the ds4 engine and the serial baseline is
   re-pinned.
...
3. The launch reference candidate is stock SERIAL (`num_speculative_tokens` 0),
   scored against the serial baseline pinned in each golden, so the expected
   launch composite is approximately 1.000 within bands (prefill +/-5 %, decode
   +2 % up) -- a null control of the scored pipeline. The native MTP head is the
   participants' improvement path (a later, separate submission).
