...
# Layr-Labs: the drafter-budget predicate lives at module level, taking host
# arrays and one int, so it can be extracted and unit tested on a host with no
# CUDA build of vLLM (tools/test_drafter_budget.py parses this file with ast and
# execs just this def). Pure numpy: no device tensor is touched.
def batch_cannot_use_drafts(
    max_seq_len: np.ndarray,
    prefill_len: np.ndarray,
    num_computed_tokens: np.ndarray,
    inflight_slack: int,
) -> bool:
    """True when every request in the batch has spent its output budget.

    A draft proposed on step T is verified on step T+1, so it is worth
    proposing only for a request that is still running after step T. A request
    stops at max_seq_len == prompt_len + max_tokens tokens, so the drafts are
    dead on arrival once step T is certain to take the request there.

    The test is a LOWER bound on the sequence length the request is certain to
    hold when step T lands, so it can only ever be late, never early:

      * `prefill_len` is the token count the request was admitted with -- exact.
      * `num_computed_tokens` is the CPU mirror of the scheduler's counter,
        taken before this step. Under async scheduling it runs AHEAD: it counts
        the draft tokens of the step(s) still in flight, which a rejection
        rolls back. `inflight_slack` is the most it can be ahead by, so
        `num_computed_tokens + 1 - inflight_slack` is a committed length.
      * the step commits at least the one token sampled at the request's last
        query position, whatever becomes of the drafts it is verifying -- hence
        the trailing + 1.

    A request in a NON-final prefill chunk samples nothing, so that + 1 is
    optimistic for it; it can only decide the test for max_tokens <= 1, where
    no draft is ever consumable anyway.

    max_seq_len == 0 marks a slot no real request occupies (dummy / warmup /
    cudagraph-capture batches); those must keep exercising the drafter.
    """
    if max_seq_len.size == 0:
        return False
    committed_len = np.maximum(prefill_len, num_computed_tokens + 1 - inflight_slack)
    return bool(np.all((committed_len + 1 >= max_seq_len) & (max_seq_len > 0)))
...
class GPUModelRunner(LoRAModelRunnerMixin):
    def __init__(self, vllm_config: VllmConfig, device: torch.device):
        ...
        # Layr-Labs: under async scheduling the CPU mirror of
        # num_computed_tokens counts the draft tokens of the step(s) still in
        # flight, which a rejection rolls back. This is the most it can run
        # ahead of a committed length by. Sync scheduling applies the
        # rejections before the next step is scheduled, so the mirror is exact.
        self._inflight_computed_token_slack = (
            self.num_speculative_steps * self.parallel_config.pipeline_parallel_size
    ...
    def sample_tokens(
        ...
        if self.speculator is not None and self._batch_cannot_use_drafts(input_batch):
            # Layr-Labs: every request in this batch has spent its output
            # budget -- the token sampled above takes it to max_tokens -- so it
            # finishes here and no draft it could receive is ever verified.
            # Skip the drafter entirely: on the first step of a max_tokens == 1
            # completion that is the drafter's full prefill pass over the
            # prompt (~200 ms on a 1024-token prompt), and on the last step of
            # every max_tokens == N completion it is a full round of draft
            # steps, both pure waste. Accepted tokens are unaffected -- under
            # greedy verification the committed tokens are the target's argmax
            # regardless of what (if anything) was drafted -- and the drafts
            # left in the per-request slots are never read: the scheduler does
            # not schedule a request it is certain has reached max_tokens, and
            # add_request() zero-fills the slot when it is handed to the next
            # request.
            pass
    ...
    def _batch_cannot_use_drafts(self, input_batch: InputBatch) -> bool:
        """True when no request in the batch can ever consume a draft token.

        A request whose output budget the token sampled in this step exhausts
        (total_len reaches max_seq_len == prompt_len + max_tokens) finishes
        here, so no draft proposed here is ever verified. max_tokens == 1 is
        the special case where that is true on the request's first step.
        Reads only the CPU-side request state (no GPU sync): every array below
        is host numpy, and postprocess_sampled writes this step's counts to the
        GPU copies only.
        """
        idx = input_batch.idx_mapping_np[: input_batch.num_reqs]
        return batch_cannot_use_drafts(
            self.req_states.max_seq_len[idx],
