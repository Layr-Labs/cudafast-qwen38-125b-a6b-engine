    ...
    def schedule(self, throttle_prefills: bool = False) -> SchedulerOutput:
        ...
        while req_index < len(self.running) and token_budget > 0:
            ...
            if (
                ...
                # Layr-Labs: with speculative decoding the in-flight step holds
                # 1 + num_drafts output slots, so it MAY already finish the
                # request (num_tokens = tokens confirmed so far; the placeholders
                # are the in-flight slots). The check above only catches the case
                # where it surely does, and after a rejection its rollback
                # accounting lands one short, so a request that finishes by
                # accepting its drafts still gets an extra step scheduled: a full
                # verify forward for a request that is already complete, which the
                # next request then waits behind (~60 ms at depth 1, measured
                # 2026-09-02 on the GB10). Do not pre-schedule past a step that can
                # finish the request; if it turns out not to, the request is
                # scheduled again as soon as that step's output lands (one
                # scheduling round-trip, once per request end). Token-exact: no
                # forward that produces committed tokens is skipped.
                req_index += 1
                continue
