...
# eight-tape cohort: `benchd iterate --mode official` seals the composite
# itself (tools/qwen38-125b-a6b-measure-and-score.sh). So this preflight verifies
# the staged pool goldens (and the live golden among them), the fixture arm
# state, the pinned benchd against its own manifest, and the serial serve
# identity -- it does NOT read a MLXFAST_QWEN38_BASELINE_WORKSPACE.
#
...
for var in BENCHD MLXFAST_SKIP_WEIGHTS_DOWNLOAD SKIP_MODEL_DOWNLOAD MLXFAST_LOCAL_COOL_GATE MLXFAST_LOCAL_ALLOW_GOLDEN_DRIFT MLXFAST_SKIP_ENGINE_BUILD MLXFAST_SKIP_WEIGHTS_SHA256 DS4_MODEL DS4_MTP_PATH DS4_MTP_DRAFT_TOKENS DS4_CTX_SIZE DS4_EOS_IDS; do
  eval "value=\${${var}:-}"
...
#   * requires any box-preset SERVE_UP_SPECULATIVE to AGREE with the derived
#     value, so a box that pre-set a speculative serve cannot silently measure a
#     different identity than the declaration is scored as.
#
...
serve_spec="${SERVE_UP_SPECULATIVE:-${decl_spec}}"
[[ "${serve_spec}" == "${decl_spec}" ]] \
  || fail "a box pre-set SERVE_UP_SPECULATIVE=${serve_spec} but the participant declaration derives ${decl_spec}; a staged serve must match the spec the declaration is scored as"
if [[ "${decl_spec}" == "1" ]]; then
  armed="$(jq -r '.official_scoring_enabled // false' "${CONTRACT}")"
  [[ "${armed}" == "true" ]] \
