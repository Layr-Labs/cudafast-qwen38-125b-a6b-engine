...
/* Speculative-cycle counters for a qwen4exp session, for the shim's
 * spec_counters path.  `drafts`, `hits` and `quenches` are the existing
 * `drafted`, `accepted` and `quenches` and keep exactly the meaning they had.
 *
 * `verify_replay_disagreements` is new and DIAGNOSTIC.  On a rejecting round
 * the cycle runs the batched verify's row-0 argmax and a one-row replay of the
 * same position; where they differ, the replay stands -- it is the replay's
 * distribution the next fed token is sampled from -- and this counts the
 * difference.  It is never a refusal.  A low rate is the tower's batch-shape
 * residual, which is expected and bounded by tests/test_qwen4exp_graph; a high
 * one means a rollback object is not restoring the state to the round's start.
 * benchd can seal a ceiling on the rate later; nothing here enforces one.
 *
 * Returns 0 and fills `out` for a qwen4exp session, non-zero otherwise. */
typedef struct {
    uint64_t rounds;
    uint64_t drafts;
    uint64_t hits;
