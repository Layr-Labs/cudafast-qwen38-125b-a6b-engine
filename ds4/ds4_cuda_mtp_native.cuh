...
__global__ static void mtp_native_projection2_refine_kernel(
    ...
    if (local_lane < 32u) {
        ...
        /* A warp probes 32 pivots together rather than waiting for a chain
         * of dependent loads in one lane. Ballot results are warp-uniform. */
        while (hi - lo > 32u) {
            const uint32_t span = hi - lo;
            const uint32_t pivot = lo + (span * local_lane) / 32u;
            const uint32_t below = __ballot_sync(0xffffffffu, other[pivot] < id);
            const uint32_t first_ge = __popc(below);
            const uint32_t next_hi = first_ge < 32u
                ? lo + (span * first_ge) / 32u : hi;
            if (first_ge) lo += (span * (first_ge - 1u)) / 32u + 1u;
            hi = next_hi;
        }
        const uint32_t pivot = lo + local_lane;
        const uint32_t below = __ballot_sync(0xffffffffu, pivot < hi && other[pivot] < id);
        lo += __popc(below);
        if (!local_lane)
