...
__global__ __launch_bounds__(256) static void mtp_native_unpack_sort_ids(
        uint32_t *ids, const uint64_t *keys, int id_bits) {
    constexpr int threads = 256, items = 8;
    static_assert(threads * items == MTP_NATIVE_CAP, "sort covers the complete shortlist");
...
#pragma unroll
    ...
    // The input ordering is immaterial for key-only sorting. The striped
    // output permits coalesced stores in ascending original-token-ID order.
...
extern "C" int ds4_gpu_mtp_native_screen(ds4_gpu_tensor *out,
     ...
     * mtp_native_unpack_sort_ids uses UINT32_MAX - (uint32_t)key, and a key's low
     * word is 0xffffffff - id by construction (q8_top1_pack_key, and the
     * mandatory UINT64_MAX - id rows share that low word), so each tile holds the
    ...
    if (!cuda_ok(cudaGetLastError(),"native unpack and original-ID sort")) return -1;
