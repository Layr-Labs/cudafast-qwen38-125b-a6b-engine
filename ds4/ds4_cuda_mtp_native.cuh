...
#include "ds4_cuda_mtp_screen_warp.cuh"
#include "ds4_cuda_mtp_screen_warp_tune.cuh"

static bool mtp_screen_warp_use(uint32_t width, int device) {
    return MTP_NATIVE_SCREEN_GROUPS == 24u && MTP_NATIVE_DIM == 2560u &&
        cuda_q8_use_dp4a() && getenv("DS4_MTP_NO_SCREEN_WARP") == nullptr &&
        (getenv("DS4_MTP_FORCE_SCREEN_WARP") != nullptr ||
         (mtp_screen_prefer_warp && width == mtp_screen_tuned_width && device == mtp_screen_tuned_device));
}
...
extern "C" int ds4_gpu_mtp_native_screen(ds4_gpu_tensor *out,
    ...
    if (fuse_keys) {
        if (mtp_screen_warp_use(width, current)) {
            mtp_native_screen_staged_warp_kernel<<<(width+3u)/4u,128,0,cuda_decode_stream()>>>(
