...
/* One complete paired-lane group wave; full refinement still uses80 groups.
...
__global__ static void mtp_native_projection_kernel(
    ...
    /* Both private launches process exactly one activation row. */
...
extern "C" int ds4_gpu_mtp_native_screen(ds4_gpu_tensor *out,
    ...
    if (!cuda_ok(cudaGetLastError(),"native one-wave screen")) return -1;
