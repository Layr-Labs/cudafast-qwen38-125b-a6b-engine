...
struct mtp_native_layout {
    uint64_t scores, key_in, key_out, ids_in, ids_out, flag, temporary;
};
...
static mtp_native_layout mtp_native_offsets(uint32_t width) {
    ...
    l.key_out = mtp_native_align(l.key_in + (uint64_t)width * 4u);
    l.ids_in = mtp_native_align(l.key_out + (uint64_t)width * 4u);
    l.ids_out = mtp_native_align(l.ids_in + (uint64_t)width * 4u);
    l.flag = mtp_native_align(l.ids_out + (uint64_t)width * 4u);
...
extern "C" int ds4_gpu_mtp_native_screen_init(uint32_t width,
    ...
    *bytes = 0; *capacity = 0;
    ...
    size_t a = 0;
    if (cub::DeviceRadixSort::SortPairsDescending(nullptr, a,
            (const uint32_t *)nullptr, (uint32_t *)nullptr,
            (const uint32_t *)nullptr, (uint32_t *)nullptr, width, 0, 32,
            cuda_decode_stream()) != cudaSuccess) return -1;
    *bytes = mtp_native_offsets(width).temporary + a;
    *capacity = MTP_NATIVE_CAP;
    return 1;
}
__global__ static void mtp_native_keys(uint32_t *keys, uint32_t *ids, uint32_t *invalid,
    ...
    /* Input IDs ascend in original-token order. CUB radix sort is stable,
     * so equal score keys retain the old ascending-ID tie policy. */
    ids[i] = id;
    if (!id || i >= prefix) keys[i] = UINT32_MAX;
    else keys[i] = q8_top1_float_ordered_key(value == 0.0f ? 0.0f : value);
}
__global__ static void mtp_native_unpack_ids(uint32_t *ids, const uint32_t *sorted_ids) {
    const uint32_t i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i < MTP_NATIVE_CAP) ids[i] = sorted_ids[i];
}
...
extern "C" int ds4_gpu_mtp_native_screen(ds4_gpu_tensor *out,
    ...
    uint32_t *key_in = (uint32_t *)(base + l.key_in);
    uint32_t *key_out = (uint32_t *)(base + l.key_out);
    uint32_t *ids_in = (uint32_t *)(base + l.ids_in);
    uint32_t *ids_out = (uint32_t *)(base + l.ids_out);
    ...
    mtp_native_keys<<<(width+255u)/256u,256,0,cuda_decode_stream()>>>(
        key_in,ids_in,flag,scores,width,prefix,tail,vocab);
    ...
    if (!cuda_ok(cub::DeviceRadixSort::SortPairsDescending(base+l.temporary,temporary,
            key_in,key_out,ids_in,ids_out,width,0,32,cuda_decode_stream()),"native score sort")) return -1;
    mtp_native_unpack_ids<<<(MTP_NATIVE_CAP+255u)/256u,256,0,cuda_decode_stream()>>>((uint32_t *)ids->ptr,ids_out);
