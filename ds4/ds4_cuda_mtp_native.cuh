...
static const unsigned char *mtp_screen_tape(const unsigned char *w,
    ...
    *out_groups = src_groups;
    ...
    /* A tape built during the uncaptured warm-up remains usable from a graph
     * capture.  Only allocation/build must be forbidden while capturing. */
    if (g_tape && g_src == w && g_vocab == n_vocab &&
        g_groups == MTP_SCREEN_TAPE_GROUPS) {
        *out_groups = g_groups;
        return g_tape;
    }
