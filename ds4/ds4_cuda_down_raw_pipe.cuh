/* Prefill routed-down raw-byte pipeline. Included after the original tile
 * and its helpers. This preserves the original decoded tiles, accumulation,
 * pair windows and output order; only Q5_1 input-byte delivery changes. */
#pragma once

/* Four warps cover a 32-channel by 32-pair tile: bit zero chooses
 * the 16-channel half and bit one the 16-pair half. The parent has four
 * channel halves and one token half. Halving the channel extent reduces
 * raw and decoded weight storage while every live dot keeps its K order. */
enum { QW_DOWN_RAW_BM = 32, QW_DOWN_RAW_NT = 2 };

__device__ __forceinline__ static void qw_down_raw_words6(
        const uint4 *panel, uint32_t row, uint32_t group, uint32_t *out) {
    const uint32_t *p = (const uint32_t *)(const void *)panel +
                        row * 24u + group * 6u;
...
#pragma unroll
    for (int i = 0; i < 3; i++) {
        asm volatile("ld.shared.v2.u32 {%0,%1}, [%2];\n"
                     : "=r"(out[2*i]), "=r"(out[2*i+1])
                     : "r"(addr + (uint32_t)i * 8u));
    }
#else
    for (int i = 0; i < 6; i++) out[i] = p[i];
#endif
}

template <int DownType = -1, bool Wide6 = false>
__global__ __launch_bounds__(QW_DOWN_MMA_THREADS) static void
qwen4exp_moe_down_raw_pipe_kernel(
        float *partial,
        const char *down,
        const int8_t *mq,
        const float *ms,
        ...
        uint32_t out_dim,
        uint32_t dq_stage) {
    __shared__ __align__(16) int8_t sA[QW_DOWN_RAW_BM * QW_MMA_LD];
    __shared__ __align__(16) int8_t sB[QW_MMA_BN * QW_MMA_LD];
    __shared__ float  sWA[QW_DOWN_RAW_BM * QW_MMA_G];
    __shared__ float  sWB[QW_DOWN_RAW_BM * QW_MMA_G];
    __shared__ float  sXS[QW_MMA_BN * QW_MMA_G], sXSUM[QW_MMA_BN * QW_MMA_G];
    __shared__ uint32_t sPair[QW_MMA_BN];
    /* Original Q5_1 bytes for one four-group K chunk, 32 * 96 bytes.
    ...
    __shared__ __align__(16) uint4 sRaw[QW_DOWN_RAW_BM * 6u];

    const uint32_t tid  = threadIdx.x;
    const uint32_t warp = tid >> 5;
    const uint32_t lane = tid & 31;
    const uint32_t row0 = blockIdx.x * QW_DOWN_RAW_BM;
    ...
    auto issue_raw = [&](uint32_t kc) {
        ...
        for (uint32_t i = tid; i < QW_DOWN_RAW_BM * 6u;
    ...
    for (int32_t nbase = 0; nbase < cnt; nbase += QW_MMA_BN) {
        ...
        float acc[QW_DOWN_RAW_NT * 4];
#pragma unroll
        for (int i = 0; i < QW_DOWN_RAW_NT * 4; i++) acc[i] = 0.0f;

        for (uint32_t kc = 0; kc < groups; kc += QW_MMA_G) {
            if (raw_pipe) qw_cpasync_wait0();
            __syncthreads();
            for (uint32_t idx = tid; idx < QW_DOWN_RAW_BM * QW_MMA_G;
...
#pragma unroll
            for (int gg = 0; gg < QW_MMA_G; gg++) {
                ...
                const uint32_t ar = (warp & 1u) * 16u + (lane >> 2);
...
#pragma unroll
                ...
                const uint32_t m0 = (warp & 1u) * 16u + (lane >> 2);
#pragma unroll
                for (int nt = 0; nt < QW_DOWN_RAW_NT; nt++) {
                    ...
                    if ((int)((warp >> 1u) * 16u) + nt * 8 >= take) break;
                    const uint32_t bn = (warp >> 1u) * 16u + nt * 8u + (lane >> 2);
#pragma unroll
                    for (int r = 0; r < 2; r++) {
                    ...
                    const uint32_t n0 = (warp >> 1u) * 16u + nt * 8u + (lane & 3u) * 2u;
#pragma unroll
                    for (int r = 0; r < 4; r++) {
...
#pragma unroll
            for (int r = 0; r < 4; r++) {
                const uint32_t nn = (warp >> 1u) * 16u + nt * 8u + (lane & 3u) * 2u + (r & 1);
