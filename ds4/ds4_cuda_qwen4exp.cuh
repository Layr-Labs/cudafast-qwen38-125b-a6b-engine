 ...
 * the consumer's programmatic launch gated to n_tokens <= 2).  Their consumers
 * fence BEFORE their own top trigger, so a consumer's dependents still only
 * launch after everything upstream of the consumer has retired, exactly as
 * when the consumer was launched plainly.
 *
