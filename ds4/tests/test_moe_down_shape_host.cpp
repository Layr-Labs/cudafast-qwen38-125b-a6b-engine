/* Execute actual generic/shape-specialized MoE down CUDA bodies with scheduled host fibers.
...
template<int Type,unsigned Rows> static void trial(unsigned pattern) {
    ...
    auto run=[&](unsigned mode) {
        for(unsigned b=0;b<rows/8;b++) for(unsigned t=0;t<(nt+1)/2;t++) {
            ...
            cta(256,[&] {
                if(mode==1) qwen4exp_moe_down_q_kernel<2,Type,true,true,true,Rows>(
...
int main() {
    ...
    auto dispatch=[&](unsigned g=20,unsigned o=2560,unsigned n=2,
        ...
        controls++;return qwen4exp_down_decode_rows(g,o,n,u,ty,rb,eb);
    };
    need(dispatch()==2,"Q5 two rows dispatch");
    need(dispatch(20,2560,1,10,8,680,1740800)==1,"Q8 one row dispatch");
    need(dispatch(20,2560,2,10,8,680,1740800)==2,"Q8 two rows dispatch");
    ...
    setenv("DS4_QWEN4EXP_NO_DOWN_DECODE_SHAPE","1",1);
    need(!dispatch(),"disable valve");unsetenv("DS4_QWEN4EXP_NO_DOWN_DECODE_SHAPE");
    for(unsigned pattern=0;pattern<7;pattern++) {
        trial<7,2>(pattern);trial<8,1>(pattern);trial<8,2>(pattern);
    }
    std::printf("MoE down shape: %u kernel cases, %u dispatch controls; staged/unstaged/specialized bits and guards PASS\n",cases,controls);
}
