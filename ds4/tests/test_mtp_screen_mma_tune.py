...
source=(root/'ds4_cuda_mtp_screen_mma_tune.cuh').read_text()
launch=re.search(r'mtp_native_screen_mma_kernel<<<([^,]+),([^,]+), 0, stream>>>',source)
assert launch
source=source.replace(launch[0],f'probe_geometry({launch[1]}, {launch[2]}); mtp_native_screen_mma_kernel')
...
#include <vector>
using std::isfinite;
static int mtp_screen_tuned_device=-1;static bool mtp_screen_prefer_mma=false;
...
static void probe_geometry(unsigned blocks,unsigned threads){assert(blocks==(4096u+15u)/16u && threads==32u);}
...
static void launch(bool mma,uint64_t *keys,uint32_t *flag,const unsigned char *w,const int8_t *x,const float *xs,unsigned width,unsigned vocab,unsigned prefix,unsigned tail){
 assert(width==4096&&vocab==4115&&prefix==3820&&tail==276);
 for(unsigned i=0;i<width;i++)keys[i]=((uint64_t)(i+(int)x[0]+w[2])<<32)|i;
 if(mma&&scenario=="bad_result")keys[17]^=1;if(mma&&scenario=="bad_flag")*flag=1;
 ...
 gpu_us+=mma?cost:10;order.push_back(mma);
}
static void mtp_native_screen_mma_kernel(uint64_t *k,uint32_t *f,const unsigned char *w,const int8_t *x,const float *s,uint32_t width,uint32_t v,uint32_t p,uint32_t t){launch(true,k,f,w,x,s,width,v,p,t);}
...
int main(int argc,char **argv){assert(argc==2);scenario=argv[1];auto sep=scenario.find(':');
 ...
 std::string report=mtp_screen_mma_tune();assert(allocs==frees&&hosts==host_frees&&events==destroyed);
 auto c=calls;assert(report==mtp_screen_mma_tune()&&calls==c);
 ...
 if(success){assert(mtp_screen_tuned_device==0&&mtp_screen_tuned_width==4096&&elapsed==56&&allocs==6&&hosts==3&&events==2);
  assert(mtp_screen_prefer_mma==(scenario=="fast"||scenario=="large_noisy"||scenario=="six_wins"));
 ...
 }else{assert(!mtp_screen_prefer_mma&&mtp_screen_tuned_device==-1&&mtp_screen_tuned_width==0);if(!fail_name.empty())assert(count[fail_name]>=fail_n);}
...
cases=['fast','marginal','slower','one_slow','noisy','large_noisy','lucky_outlier','big_bad_outlier','six_wins','five_wins','edge',
 'DS4_MTP_NO_SCREEN_MMA','DS4_MTP_NO_SCREEN_MMA_TUNE','DS4_MTP_NO_FUSED_SCREEN_KEYS','DS4_QWEN4EXP_NO_ROW_TILE','DS4_QWEN4EXP_PAIR_LANES_R2',
