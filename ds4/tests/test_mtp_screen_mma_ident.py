...
static const char *mtp_screen_mma_tune(){
    static std::string report="scrMma["+std::string(184,'T')+"]";calls++;return report.c_str();
}
...
int main(int argc,char **argv){assert(argc==2);mode=argv[1][0]-'0';
    ...
    if(mode==2){assert(result.empty()&&calls==0);}
        ...
        auto first=result.find("scrMma[");assert(first!=std::string::npos);
...
'''
with tempfile.TemporaryDirectory(prefix='screen-mma-ident-') as d:
