...
static unsigned scores,probs,plain,fused,producer,consumer;static int error;static bool disabled,split_disabled,rescale_disabled;static unsigned forced_group;
static const ds4_gpu_tensor *expected_q8,*expected_doubled;static uint64_t expected_qoff,expected_soff;
static const char *test_env(const char *n){return (!strcmp(n,"DS4_QWEN4EXP_NO_QSA_FOLD_QUANT")&&disabled)||(!strcmp(n,"DS4_QWEN4EXP_NO_QSA_RESCALE")&&rescale_disabled)?"1":nullptr;}
...
static unsigned qwen4exp_qsa_split_width(unsigned n,unsigned,unsigned,unsigned){return split_disabled?0:forced_group?forced_group:n==1?2:4;}
...
'''+r'''
template<unsigned G,unsigned V,class... A>static void qwen4exp_qsa_split_probs_rescale_kernel(A...){producer++;probs++;}
template<class... A>static void qwen4exp_qsa_split_fold_rescale_quant_kernel(A... args){consumer++;qwen4exp_qsa_split_fold_quant_kernel(args...);}
'''+fn('static int qwen4exp_hc_ranges_disjoint(')+'\n'+fn('static int qwen4exp_qsa_attention_split(').replace('getenv(', 'test_env(')+'\n'+fn('extern "C" int ds4_gpu_qwen4exp_qsa_attention_fold_q8_dpos_tensor(').replace('getenv(', 'test_env(')+r'''
int main(){
 unsigned cases=0;
 auto check=[&](int scenario,unsigned variant){
  ...
  scores=probs=plain=fused=producer=consumer=0;error=0;disabled=split_disabled=rescale_disabled=false;forced_group=0;
  ...
  if(scenario==24){rescale_disabled=true;}
  if(scenario==25){forced_group=variant;if(variant==12)want=0;}
  if(scenario==26){rows=variant;sel=cnt=nullptr;}
  if(scenario==27){scratch->ptr=t[variant].ptr;}
  ...
  if(want!=0){
   const unsigned chosen=forced_group?forced_group:rows==1?2:4;
   const bool share=!rescale_disabled&&(chosen==2||chosen==4)&&scenario!=27;
   assert(producer==unsigned(share)&&consumer==unsigned(share));
  }
  cases++;
 };
 ...
 check(12,1);check(13,1);check(14,16);check(14,32);check(24,0);for(unsigned g:{1u,2u,3u,4u,6u,12u})check(25,g);for(unsigned n:{1u,2u,3u})check(26,n);for(unsigned i:{0u,1u,2u,3u,4u,5u,6u,9u})check(27,i);
