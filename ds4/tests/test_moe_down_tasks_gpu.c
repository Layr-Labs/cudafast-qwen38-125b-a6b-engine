/* Complete MoE comparison for the bounded down MMA tasks, including graph
...
static const char *disable = "DS4_QWEN4EXP_NO_DOWN_PAIR_TASKS";
...
static void require(int ok, const char *what) {
    if (!ok) { fprintf(stderr, "MoE down tasks: %s\n", what); exit(1); }
}
...
#undef INVOKE
    ...
    printf("MoE down tasks gt=%u dt=%u experts=%u offset=%u PASS\n",gt,dt,experts,64+alignment); fflush(stdout);
}
...
int main(void) {
    ...
    printf("MoE down tasks: %u complete buffers, %u graph checks, %u actual replays PASS\n",
