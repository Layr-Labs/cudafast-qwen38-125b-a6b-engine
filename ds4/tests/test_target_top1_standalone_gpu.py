...
int main() {
 ...
 for(uint32_t width:{65536u,65537u,65543u,248320u,262145u}) for(uint32_t rows:{1u,2u}) {
  ...
  CK(cudaMalloc(&d,n*4)); CK(cudaMalloc(&old,16)); CK(cudaMalloc(&out,16)); CK(cudaMalloc(&partial,144));
  cudaGraph_t graph; cudaGraphExec_t exec;
  CK(cudaStreamBeginCapture(stream,cudaStreamCaptureModeGlobal));
  target_top1_partition_kernel<<<dim3(8,rows),1024,0,stream>>>(partial,d,width,rows);
  ...
  for(unsigned mode=0;mode<20;mode++) {
   ...
   if(mode==6){std::fill(x.begin(),x.end(),-1.f);for(unsigned r=0;r<rows;r++)for(unsigned i:{1023u,1024u,8191u,8192u})x[(size_t)r*width+i]=7.f;}
   ...
   CK(cudaMemsetAsync(old,0xa5,16,stream)); CK(cudaMemsetAsync(out,0xa5,16,stream)); CK(cudaMemsetAsync(partial,0xa5,144,stream));
   indexer_top1_kernel<<<rows,1024,0,stream>>>(old,d,width,rows);
   target_top1_partition_kernel<<<dim3(8,rows),1024,0,stream>>>(partial,d,width,rows);
   target_top1_finish_kernel<<<rows,32,0,stream>>>(out,partial);
   CK(cudaGetLastError()); CK(cudaStreamSynchronize(stream));
   uint32_t a[4],b[4]; unsigned char scratch[144];
   ...
   CK(cudaMemcpy(scratch,partial,144,cudaMemcpyDeviceToHost));
   for(unsigned i=rows*64;i<144;i++)NEED(scratch[i]==0xa5);
