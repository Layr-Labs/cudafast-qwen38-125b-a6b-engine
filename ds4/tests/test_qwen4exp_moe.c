...
static void run_router_tail_fuse_case(void) {
    ...
    for (uint32_t width = 1; width <= TAIL_TOKENS; width++) {
         ...
         * routed launch just wrote, so the shared expert reads those bytes
         * instead of running its own byte-identical duplicate; the region
         * pointer is the ONLY thing that differs.  Reference: the entry
         * quantising x itself, exactly as it did before the pointer existed.
         * The candidate takes the pointer the routed call hands out -- which
         * must be non-NULL and is seated at that call's layout offset, not
         * at the tier scratch's base, so this A/B is where an assumed-offset
         * regression would surface.  Every byte must agree. */
        unsetenv("DS4_QWEN4EXP_NO_ROUTER_FUSE");
        require_ok(ds4_gpu_qwen4exp_router_tail_moe_tensor(
                       out_t, mid_t, part_t, &gate_slab, &up_slab, &down_slab,
                       IN_DIM, MID_DIM, OUT_DIM, sel_t, w_t, logits_t,
