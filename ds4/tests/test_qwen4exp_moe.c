...
static void run_production_expert_cases(void) {
    ...
    for (uint32_t c = 0; c < n_seen; c++) {
        ...
        /* Each panel optimization must match its synchronous panel byte for
        ...
        if (down_type == TYPE_Q8_0 || down_type == TYPE_Q5_1) {
            for (unsigned panel = 0; panel < 2; panel++) {
                const char *env_name = panel == 0 ? "DS4_QWEN4EXP_NO_DOWN_ASYNC"
                                                  : "DS4_QWEN4EXP_NO_DOWN_WIDE";
                const char *pin = getenv(env_name);
                ...
                require_ok((saved ? setenv(env_name, saved, 1)
                                  : unsetenv(env_name)) == 0,
                           "async panel environment restore");
                free(saved); free(panel_ref); free(panel_got);
                printf("asynchronous %s panel type %u: exact at widths 1/2, three route patterns\n",
                       panel == 0 ? "down" : "down-wide", down_type);
