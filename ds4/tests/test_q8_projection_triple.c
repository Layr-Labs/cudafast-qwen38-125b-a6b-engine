/* Three Q8 projections: independent maps, complete buffers and changed graphs. */
#define _GNU_SOURCE
#include "ds4_gpu.h"
#include <stdint.h>
#include <stdio.h>
...
static void must(int ok,const char *s){if(!ok){fprintf(stderr,"QSA Q8 triple: %s\n",s);exit(1);}}
static void *alloc(size_t n){void *p=malloc(n);must(p!=NULL,"host allocation");return p;}
static uint64_t in_dim,od[3],groups,mb[3],offsets[3][SETS];
static size_t xb,qb,yb[3];
static unsigned char *maps[3],*xhost[SETS],*qref[SETS],*refs[SETS][3],*poison;
static ds4_gpu_tensor *xt[SETS],*qt[SETS],*yt[MODES][SETS][3];
...
static void reset(unsigned mode) {
    for(unsigned i=0;i<SETS;i++)for(unsigned b=0;b<3;b++)
        must(ds4_gpu_tensor_write(yt[mode][i][b],0,poison,yb[b]),"reset complete output");
}
static int triple(ds4_gpu_tensor *const outs[3],unsigned i,unsigned rows) {
    const void *wm[3]={maps[0],maps[1],maps[2]};
    uint64_t off[3];for(unsigned b=0;b<3;b++)off[b]=offsets[b][i];
    return ds4_gpu_matmul_q8_0_preq_triple_rows_exact_tensor(outs,wm,mb,off,
        in_dim,od,qt[i],0,(uint64_t)rows*groups*32u,rows);
}
static void operate(unsigned mode,unsigned rows) {
    for(unsigned i=0;i<SETS;i++) {
        if(mode)must(triple(yt[mode][i],i,rows),"three projections");
        else for(unsigned b=0;b<3;b++)must(ds4_gpu_matmul_q8_0_preq_rows_exact_tensor(
            yt[mode][i][b],maps[b],mb[b],offsets[b][i],in_dim,od[b],qt[i],
            0,(uint64_t)rows*groups*32u,rows),"original projection");
    }
}
...
static void check_shape(unsigned in,unsigned out0,unsigned out1,unsigned out2,unsigned offset) {
    in_dim=in;od[0]=out0;od[1]=out1;od[2]=out2;groups=in/32;
    xb=CAP*in*4u+64;qb=CAP*groups*36u+64;
    for(unsigned b=0;b<3;b++) {
        const uint64_t stride=(od[b]*groups*34u+63u)&~63ull;
        mb[b]=offset+SETS*stride+64;
        maps[b]=mmap(NULL,mb[b],PROT_READ|PROT_WRITE,MAP_PRIVATE|MAP_ANONYMOUS,-1,0);
        must(maps[b]!=MAP_FAILED,"weight map");
        for(unsigned i=0;i<SETS;i++) {
            offsets[b][i]=offset+i*stride;
    ...
    for(unsigned b=1;b<3;b++)must(ds4_gpu_set_aux_model_map_range(maps[b],mb[b],0,mb[b]),"aux mapping");
    size_t maxy=yb[0]>yb[1]?yb[0]:yb[1];if(yb[2]>maxy)maxy=yb[2];poison=alloc(maxy);memset(poison,0xa5,maxy);
    for(unsigned i=0;i<SETS;i++) {
    ...
    for(unsigned a=0;a<3;a++)for(unsigned b=a+1;b<3;b++) {
        ds4_gpu_tensor *outs[3];for(unsigned c=0;c<3;c++)outs[c]=yt[1][0][c];
        outs[b]=outs[a];must(!triple(outs,0,1),"identical outputs rejected");
        ds4_gpu_tensor *over=ds4_gpu_tensor_view(outs[a],4,yb[a]-4);
        must(over!=NULL,"overlap view");outs[b]=over;
        must(!triple(outs,0,1),"partial overlap rejected");ds4_gpu_tensor_free(over);
    }
    ds4_gpu_tensor *outs[3];for(unsigned c=0;c<3;c++)outs[c]=yt[1][0][c];
    outs[1]=qt[0];must(!triple(outs,0,1),"quant input overlap rejected");
    size_t sum=0;for(unsigned b=0;b<3;b++)sum+=od[b]*4u;
    ...
    printf("QSA_Q8_TRIPLE_CHECK in=%u out0=%u out1=%u offset=%u sets=4 eager_buffers=%u graph_buffers=%u changed_replays=%u PASS\n",in,out0,out1,offset,eager,captured,replayed);fflush(stdout);
    ...
    ds4_gpu_cleanup();for(unsigned b=0;b<3;b++)munmap(maps[b],mb[b]);free(poison);
}
int main(void) {
    setenv("DS4_CUDA_DECODE_GRAPHS","1",1);
    check_shape(96,515,33,65,64);
    check_shape(2560,12288,512,512,64);
    check_shape(2560,12288,512,512,66);
    puts("QSA_Q8_TRIPLE_ALL PASS");return 0;
}
