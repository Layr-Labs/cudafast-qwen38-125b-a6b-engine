...
gate = gate.replace('getenv("DS4_MTP_FORCE_BLOCK_ID_SORT")', '(forced ? "1" : nullptr)')
...
static int mtp_id_tuned_device=0;
static bool mtp_id_prefer_block18=true;
'''
...
code += 'static bool allow(Buffer *ids,const uint64_t *key_out,bool disabled,int id_bits=18,int current=0,bool forced=false){' + gate + 'return block_ids;}\n'
code += r'''
int main(){
    ...
    assert(!allow(&ids,keys,false,17));assert(!allow(&ids,keys,false,18,1));
    mtp_id_prefer_block18=false;assert(!allow(&ids,keys,false));
    assert(allow(&ids,keys,false,17,1,true));assert(!allow(&ids,keys,true,18,0,true));
    mtp_id_prefer_block18=true;
    ...
    std::cout<<"PASS "<<checked<<" extracted load/unpack/striped-store cases and 16 dispatch controls under UBSan; CUB collective is a scalar contract stub, not GPU execution\n";
}
'''
