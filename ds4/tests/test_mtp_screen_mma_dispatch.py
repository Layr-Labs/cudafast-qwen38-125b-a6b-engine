...
launch=re.search(r'mtp_native_screen_mma_kernel<<<([^,]+),([^,]+),0,cuda_decode_stream\(\)>>>',body)
assert launch
body=body.replace(launch.group(0),f'record_grid({launch[1]}, {launch[2]}); mtp_native_screen_mma_kernel')
body=re.sub(r'<<<.*?>>>','',body,flags=re.S)
gate=definition(native,'static bool mtp_screen_mma_use(')
...
static bool mtp_screen_prefer_mma=true;
...
static bool mma_available=true;
static bool cuda_q8_use_dp4a(){return true;}
static bool cuda_q8_mma_available(){return mma_available;}
...
template<class... A>static void mtp_native_screen_mma_kernel(A...){mark("mma");}
...
int main(){
 ...
 auto run=[&](int want,int path){
  ...
  if(want==2048){
   ...
   assert(has("mma")==bool(path==1));if(path==1)assert(observed_grid==(width+63)/64&&observed_block==128);assert(has("fused")==bool(path==0));assert(has("coarse")==bool(path==2));
 ...
 for(int prefer:{0,1})for(int capable:{0,1})for(int force:{0,1})for(int no:{0,1}){
  env.clear();mtp_screen_prefer_mma=prefer;mma_available=capable;
  if(force)env.insert("DS4_MTP_FORCE_SCREEN_MMA");if(no)env.insert("DS4_MTP_NO_SCREEN_MMA");
  run(2048,capable&&!no&&(force||prefer)?1:0);
 }
 env.clear();mtp_screen_prefer_mma=true;mma_available=true;
 ...
 for(const char *s:{"device","capture","clear","quant","mma","read","score_sort","unpack","id_sort","refine"}){failure=s;run(-1,-1);}failure.clear();
 env.insert("DS4_MTP_NO_SCREEN_MMA");failure="fused";run(-1,-1);failure.clear();env.clear();
