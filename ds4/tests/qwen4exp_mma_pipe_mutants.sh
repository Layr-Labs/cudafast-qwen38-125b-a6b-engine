...
# The consumers take their activations from the buffer after the one the
# producers published (the data only: the barriers still pair up, so this
# is a wrong answer, not a hang).
mutant stage_skew caught \
    's|        const unsigned char \*sA = sA_all + buf \* C::A_BYTES; /\* consumers: stage s'"'"'s activations \*/|        const unsigned char *sA = sA_all + ((buf + 1) % STAGES) * C::A_BYTES; /* consumers: stage s'"'"'s activations */|'
