/* Native GPU comparison of cooperative word-header gate/up with the existing
 * unstaged byte-decoder path (DS4_GATEUP_COOP=0). The down projection is the
 ...
 * Host correctness checks are separate: gateup_word_header_host.py.
 */
#include "ds4_gpu.h"
#include <stdint.h>
#include <stdio.h>
...
enum { CAP = 8, GUARD = 64 };
static const char *disable = "DS4_GATEUP_COOP";
...
static void require(int ok, const char *what) {
    if (!ok) { fprintf(stderr, "MoE gate/up word header: %s\n", what); exit(1); }
}
...
#define POISON(M) do { for(unsigned pj=0;pj<3;pj++) \
    ...
    for (unsigned wi = 0; wi < sizeof(widths)/sizeof(widths[0]); wi++) {
        ...
        for (unsigned trial = 0; trial < 10; trial++) {
            ...
            require(setenv(disable,"0",1) == 0, "reference dispatch"); POISON(0); INVOKE(0,width);
            for (unsigned j = 0; j < 3; j++) require(ds4_gpu_tensor_read(out[0][j],0,expected[j],bytes[j]), "reference output");
            for (unsigned m = 0; m < 2; m++) {
                require((m ? unsetenv(disable) : setenv(disable,"0",1)) == 0, "graph dispatch");
                POISON(m);
...
#undef INVOKE
    ...
    printf("MoE gate/up word header gt=%u dt=%u experts=%u offset=%u PASS\n",gt,dt,experts,64+alignment); fflush(stdout);
}
...
int main(void) {
    ...
    printf("MoE gate/up word header: %u complete buffers, %u graph checks, %u actual replays PASS\n",
