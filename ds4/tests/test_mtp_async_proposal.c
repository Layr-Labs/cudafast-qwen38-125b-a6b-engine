...
int main(int argc,char **argv) {
    ...
    h.want_margin=true;mode=0;reset();
    assert(ds4_qwen4exp_mtp_head_forward_last(&h,tokens,input,12,1,got,NULL,g_err,sizeof g_err)==0);
    assert(async_calls==0&&old_calls==1&&h.last_margin>=0.f);
    h.want_margin=false;
