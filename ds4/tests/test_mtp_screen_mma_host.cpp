...
static void one(unsigned prefix,unsigned tail,unsigned offset,unsigned pattern){
 ...
 if(width>1024)blocks={0,width/128,prefix/64,(width-1)/64};
 else for(unsigned b=0;b<(width+63)/64;b++)blocks.push_back(b);
 ...
 for(unsigned b:blocks)for(unsigned row=b*64;row<std::min(width,(b+1)*64);row++){
 ...
 for(unsigned b:blocks){
  blockIdx.x=b;cta(128,[&](){mtp_native_screen_mma_kernel(out.data(),&invalid,w,x.data(),xs.data(),width,vocab,prefix,tail);});
  for(unsigned row=b*64;row<std::min(width,(b+1)*64);row++){
...
int main(){unsigned cases=0;
 ...
 std::printf("MTP screen actual-body PASS: %u cases; exact finite keys, nonfinite flags, mandatory IDs, prefix/tail maps and input/output guards\n",cases);
}
