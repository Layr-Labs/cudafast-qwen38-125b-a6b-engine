/* Actual screen CUDA body with host fibers and fragment-consuming MMA
 * emulation. Independent scalar dots/key generation are the oracle. */
#include <ucontext.h>
#include <algorithm>
#include <array>
#include <cmath>
...
alignas(16) static unsigned char host_shared_storage[16u + 16u*816u + 16u];
static const unsigned char *weight_begin, *weight_end;
static size_t copied_bytes, vector_loads, halfword_loads;
template<class T> static T stage_global_read(const void *ptr) {
    const auto *p=(const unsigned char *)ptr;
    need(p>=weight_begin && p+sizeof(T)<=weight_end,"stage source outside tensor");
    need((uintptr_t)p%alignof(T)==0,"stage source alignment");
    const size_t within=(size_t)(p-weight_begin)%2720u;
    need(within+sizeof(T)<=816u,"stage source outside original coarse prefix");
    copied_bytes+=sizeof(T);
    T raw;std::memcpy(&raw,p,sizeof raw);return raw;
}
static uint4 mtp_stage_load4(const void *p){vector_loads++;return stage_global_read<uint4>(p);}
static uint16_t mtp_stage_load2(const void *p){halfword_loads++;return stage_global_read<uint16_t>(p);}
static const unsigned char *shared_block_begin,*shared_block_end;
static void host_fragment_bounds(const unsigned char *payload,unsigned byte) {
    need(byte<=28u && byte%4u==0u,"fragment word index");
    const auto *first=host_shared_storage+16u;
    need(payload>=first+2u && payload+32u<=first+16u*816u,"fragment must read shared slab");
    need(((size_t)(payload-first)%816u)%34u==2u,"fragment original Q8 block alignment");
    shared_block_begin=payload-2u;shared_block_end=payload+32u;
}
template<class T> static T checked_smem_read(const T *ptr) {
    const auto *p=(const unsigned char *)ptr;
    need(p>=shared_block_begin && p+sizeof(T)<=shared_block_end,"fragment read outside original 34-byte block");
    T v;std::memcpy(&v,ptr,sizeof v);return v;
}
static unsigned __cvta_generic_to_shared(const void *ptr) {
    const auto *p=(const unsigned char *)ptr;
    need(p>=host_shared_storage+16u && p<host_shared_storage+16u+16u*816u,
         "shared address conversion outside slab");
    return (unsigned)(p-host_shared_storage);
}
static uint32_t mtp_screen_shared4(unsigned address) {
    need(address%4u==0u,"shared word alignment");
    return checked_smem_read((const uint32_t *)(host_shared_storage+address));
}
static uint32_t mtp_screen_shared2(unsigned address) {
    need(address%2u==0u,"shared halfword alignment");
    return checked_smem_read((const uint16_t *)(host_shared_storage+address));
}
#define __device__
#define __global__
#define __forceinline__ inline
#include "mtp_screen_mma_bodies.inc"
static void cta(int nth,std::function<void()> f) {
    threads=nth;blockDim.x=nth;body=std::move(f);block_arrivals=0;
    std::memset(host_shared_storage,0xad,sizeof host_shared_storage);
    ...
    for(unsigned i=0;i<16u;i++) {
        need(host_shared_storage[i]==0xad,"shared prefix guard");
        need(host_shared_storage[16u+16u*816u+i]==0xad,"shared suffix guard");
    }
}
...
static void one(unsigned prefix,unsigned tail,unsigned offset,unsigned pattern){
 ...
 auto *w=raw+offset;
 weight_begin=w;weight_end=w+bytes;copied_bytes=vector_loads=halfword_loads=0;
 ...
 if(width>1024){
  const unsigned last=(width-1u)/16u;
  for(unsigned anchor:{0u,width/32u,prefix/16u,last})
   for(unsigned j=0;j<4u;j++)blocks.push_back(std::min(anchor,last-3u)+j);
 } else for(unsigned b=0;b<(width+15)/16;b++)blocks.push_back(b);
 ...
 for(unsigned b:blocks)for(unsigned row=b*16;row<std::min(width,(b+1)*16);row++){
 ...
 for(unsigned b:blocks){
  blockIdx.x=b;cta(32,[&](){mtp_native_screen_mma_kernel(out.data(),&invalid,w,x.data(),xs.data(),width,vocab,prefix,tail);});
  for(unsigned row=b*16;row<std::min(width,(b+1)*16);row++){
 ...
 need(copied_bytes==touched.size()*816u,"each original coarse byte copied exactly once");
 need(offset==0u ? vector_loads>0u && !halfword_loads : halfword_loads>0u && !vector_loads,"copy alignment path");
...
int main(){unsigned cases=0;
 ...
 std::printf("MTP staged screen actual-body PASS: %u cases; exact keys/flags, mapped prefix-only copy bounds, shared guards, exact byte count and both alignment paths\n",cases);
}
