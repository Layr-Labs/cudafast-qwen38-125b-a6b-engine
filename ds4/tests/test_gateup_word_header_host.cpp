...
static unsigned group_checks, kernel_checks;
static void check_group(const uint4 *panel,unsigned row,unsigned g) {
    uint32_t raw[8];
    alignas(16) int8_t old_bytes[32],new_bytes[32];
    float oa[2]={19,23},ob[2]={29,31},na[2]={19,23},nb[2]={29,31};
    const char *original=(const char *)(panel+row*QW_GU_COOP_ROW_U4);
    qw_gu_coop_raw_load(panel,row,g,raw);
    dev_qwen4exp_group_decode_w(12,original,g,raw,old_bytes,oa,ob);
    qw_gu_coop_decode(panel,row,g,raw,new_bytes,na,nb);
    need(!std::memcmp(old_bytes,new_bytes,32),"group quant words");
    need(!std::memcmp(oa,na,sizeof oa),"group scale bits/guard");
    need(!std::memcmp(ob,nb,sizeof ob),"group minimum bits/guard");
    ++group_checks;
}

static void header_checks() {
    std::vector<uint4> panel(4*QW_GU_COOP_ROW_U4);
    auto *bytes=(uint8_t *)panel.data();
    for(size_t i=0;i<panel.size()*16;i++)bytes[i]=(uint8_t)random_word();
    // Every half bit pattern in each halfword, including signed zero,
    // subnormals, infinities and NaNs. Both paths use the same conversion.
    for(unsigned h=0;h<65536;h++) {
        panel[0].x=h|((uint32_t)(uint16_t)(h^0xa891u)<<16);
        for(unsigned g=0;g<8;g++)check_group(panel.data(),0,g);
    }
    // Every possible byte at every scale/min position. Other header bytes
    // carry random background bits so high-plane fields are not all zero.
    for(unsigned row=0;row<4;row++) for(unsigned sb=0;sb<10;sb++) {
        const size_t offset=(row*QW_GU_COOP_ROW_U4+sb*9u)*16;
        for(unsigned byte=4;byte<16;byte++) for(unsigned v=0;v<256;v++) {
            bytes[offset+byte]=(uint8_t)v;
            for(unsigned g=0;g<8;g++)check_group(panel.data(),row,sb*8+g);
        }
    }
    for(unsigned i=0;i<20000;i++) {
        unsigned row=random_word()%4,sb=random_word()%10;
        auto &hdr=panel[row*QW_GU_COOP_ROW_U4+sb*9];
        hdr={random_word(),random_word(),random_word(),random_word()};
        check_group(panel.data(),row,sb*8+random_word()%8);
    }
}
...
#define ARGS out,(const char *)gate.data(),(const char *)up.data(),xq, \
    ...
    expert_bytes,row_bytes,12,12,groups,rows,stride,slots
                ...
                else if(mode==1)qwen4exp_moe_gateup_split_kernel<2,12,true,4,true>(ARGS);
...
int main() {
    ...
    std::printf("PASS: %u exact header/group checks; %u actual-kernel fiber cases\n",
                group_checks,kernel_checks);
}
