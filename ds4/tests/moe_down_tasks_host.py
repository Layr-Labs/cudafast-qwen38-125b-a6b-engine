#!/usr/bin/env python3
"""Execute whole-expert and bounded-task down-MMA bodies using host CUDA fibers.
...
with tempfile.TemporaryDirectory(prefix='moe-down-tasks-') as tmp:
    tmp = Path(tmp)
    (tmp / 'down_tasks_bodies.inc').write_text(code)
    ...
    subprocess.run([os.environ.get('CXX', 'c++'), *flags, '-I', str(tmp),
                    '-I', str(root),
                    str(root / 'tests/test_moe_down_tasks_host.cpp'),
