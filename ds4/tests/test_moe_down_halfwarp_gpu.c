/* Native GPU bit comparison for halfwarp routed-down dispatch.
...
enum { CAP = 8, GUARD = 64 };
static const char *disable = "DS4_QWEN4EXP_NO_DOWN_SUBWARP";
...
static void require(int ok, const char *what) {
    if (!ok) { fprintf(stderr, "MoE down halfwarp: %s\n", what); exit(1); }
}
...
#undef INVOKE
    ...
    printf("MoE down halfwarp gt=%u dt=%u experts=%u offset=%u PASS\n",gt,dt,experts,64+alignment); fflush(stdout);
}
...
int main(void) {
    ...
    printf("MoE down halfwarp: %u complete buffers, %u graph checks, %u actual replays PASS\n",
