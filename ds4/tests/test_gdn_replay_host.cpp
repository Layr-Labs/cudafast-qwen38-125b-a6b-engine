...
template<class T> static void __stcs(T *p,T v) { *p=v; }
static float host_rsqrtf(float v) { return 1.f/std::sqrt(v); }
#define rsqrtf host_rsqrtf
#define __device__
#define __global__
...
static void trial(unsigned nk,unsigned nv,unsigned layout,unsigned rounds,bool sample_rows) {
    ...
    std::vector<float> decay(nv),bias(nv),weight(cd*4);
    guarded conv_a(3*cd),conv_b(3*cd),conv_snap_a(3*cd),conv_snap_b(3*cd);
    guarded qkv_other(2*cd),gates(2*nv*2);
    for(unsigned i=0;i<3*cd;i++) conv_a.data()[i]=conv_b.data()[i]=random_float(.1f);
    for(unsigned i=0;i<cd*4;i++) weight[i]=random_float(.2f);
    ...
    for(unsigned r=0;r<rounds;r++) {
        ...
        std::copy_n(qkv.data(),2*cd,qkv_other.data());
        for(unsigned c=0;c<2*nk+nv;c++) {
            blockIdx={c,0,0};
            cta(128,[&]{qwen4exp_gdn_conv_kernel<false>(
                qkv.data(),conv_a.data(),weight.data(),conv_snap_a.data(),
                nk,nv,1,2,1,1e-6f,&pending);});
            cta(128,[&]{qwen4exp_gdn_conv_kernel<true>(
                qkv_other.data(),conv_b.data(),weight.data(),conv_snap_b.data(),
                nk,nv,1,2,1,1e-6f,&pending,(float2*)gates.data(),
                alpha.data(),beta.data(),decay.data(),bias.data());});
        }
        for(unsigned i=0;i<2*cd;i++) same(qkv.data()[i],qkv_other.data()[i],"convolution output bits");
        for(unsigned i=0;i<3*cd;i++) {
            same(conv_a.data()[i],conv_b.data()[i],"convolution history bits");
            same(conv_snap_a.data()[i],conv_snap_b.data()[i],"convolution snapshot bits");
        }
        for(unsigned i=0;i<2*nv;i++) {
            float expected_g=expf(decay[i%nv]*qwen4exp_gdn_softplus(alpha.data()[i]+bias[i%nv]));
            float expected_b=qwen4exp_gdn_sigmoid(beta.data()[i]);
            same(gates.data()[2*i],expected_g,"computed decay bits");
            same(gates.data()[2*i+1],expected_b,"computed beta bits");
        }
        for(auto g:{&conv_a,&conv_b,&conv_snap_a,&conv_snap_b,&qkv_other,&gates}) g->check();
        ...
        for(unsigned c:cells) {
            ...
            cta(128,[&]{qwen4exp_gdn_replay_kernel(
                ...
                layout,&prefix,0,(r%3)?(const float2*)gates.data():nullptr);});
        }
...
int main() {
    ...
    std::puts("PASS actual convolution, precomputed gates, recurrence, state, snapshots, bounded log, canaries");
}
