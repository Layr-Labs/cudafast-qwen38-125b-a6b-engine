...
static void trial(unsigned nk,unsigned nv,unsigned layout,unsigned rounds,bool sample_rows,
    ...
    const char *pattern=deferred?"RRRRRAARRRRRRAAAAAA":"RRRAARRRRRARARAAA";
    for(unsigned r=0;r<rounds;r++) {
        ...
        need(prefix<=(deferred?3u:2u),"bounded prefix");
        flushes += prefix>=2;
        ...
        if(deferred) {
            ...
            if(prefix<2)need(base->a==base_before,"non-flush checkpoint immutable");
    ...
    if(deferred && rounds>=20) need(descriptors==0xffffu,"all ring descriptors exercised");
