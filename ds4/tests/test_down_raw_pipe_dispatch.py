...
while depth:
 depth+=(s[i]=='{')-(s[i]=='}');i+=1
body=s[a:i]
launch=re.search(r'(qwen4exp_moe_down_raw_pipe_kernel<[^>]+>)<<<\s*(dim3\(out_dim / QW_DOWN_RAW_BM, gu_rows, 1\)),\s*(QW_DOWN_MMA_THREADS), 0, stream>>>',body)
assert launch, 'raw32 grid must derive from the actual raw tile geometry'
body=body.replace(launch[0],f'record_grid({launch[2]}, {launch[3]}); {launch[1]}')
launch64=re.search(r'(qwen4exp_moe_down_raw64_kernel<[^>]+>)<<<\s*(dim3\(out_dim / QW_DOWN_MMA_BM, gu_rows, 1\)),\s*(QW_DOWN_MMA_THREADS), 0, stream>>>',body)
assert launch64, 'raw64 grid must derive from the actual 64-channel tile'
body=body.replace(launch64[0],f'record_grid({launch64[2]}, {launch64[3]}); {launch64[1]}')
...
static std::string force="32";
static int qwen4exp_down_prefill_choice=0,qwen4exp_down_prefill_device=-1;
static int g_n_gpus=1,tier=0;struct GPU{int device_id;};static GPU g_gpu[1]={{0}};
static const char *probe_getenv(const char *s){
 if(!strcmp(s,"DS4_QWEN4EXP_FORCE_DOWN_RAW_PIPE"))return force.empty()?nullptr:force.c_str();
 return env.count(s)?"1":nullptr;
}
#define getenv probe_getenv
constexpr unsigned QW_MMA_BM=32,QW_MMA_G=4,QW_DOWN_MMA_BM=64,QW_DOWN_MMA_THREADS=128,QW_DOWN_RAW_BM=32;
...
static unsigned grid_x,grid_y,block_x;
static void record_grid(dim3 g,unsigned b){grid_x=g.x;grid_y=g.y;block_x=b;}
static int path,combines,actual_tile;static std::string fail_at,last;
...
template<int Type,bool Wide>static void qwen4exp_moe_down_raw_pipe_kernel(float *,const char *p,
 ...
 assert((((uintptr_t)p)|eb|rb)%16==0 && out%64==0);assert(grid_x==out/32 && grid_y==512 && block_x==128);path=5;actual_tile=32;last="tile";
}
template<int Type,bool Wide>static void qwen4exp_moe_down_raw64_kernel(float *,const char *p,
 const int8_t *,const float *,const int32_t *,const int32_t *,const int32_t *,const int32_t *,const int32_t *,
 uint64_t eb,uint64_t rb,unsigned type,unsigned groups,unsigned out,unsigned dq){
 assert(Type==7 && Wide && type==7 && dq==1 && (groups&3u)==0);
 assert((((uintptr_t)p)|eb|rb)%16==0 && out%64==0);assert(grid_x==out/64 && grid_y==512 && block_x==128);path=5;actual_tile=64;last="tile";
}
...
static int run(unsigned n_tokens,unsigned mid_dim,unsigned out_dim,Slab dn,uintptr_t address,
 ...
 unsigned threads=256,gu_rows=512;
'''+eligibility+body+r'''
 return 2;
}
int main(){
 ...
 auto check=[&](int want,int result,unsigned nt=1024,unsigned md=640,unsigned od=2560,
  ...
  path=combines=actual_tile=0;grid_x=grid_y=block_x=0;last.clear();
 ...
 force="64";check(5,1);assert(actual_tile==64);
 force.clear();check(1,1);
 qwen4exp_down_prefill_choice=32;check(1,1);
 qwen4exp_down_prefill_device=0;check(5,1);assert(actual_tile==32);
 qwen4exp_down_prefill_choice=64;check(5,1);assert(actual_tile==64);
 qwen4exp_down_prefill_choice=0;check(1,1);
 qwen4exp_down_prefill_choice=32;
 for(unsigned md:{128u,256u,1024u}){Slab d;d.row_bytes=md/32*24;d.expert_bytes=2560*d.row_bytes;check(1,1,64,md,2560,d);}
 check(1,1,64,640,1280);
 {Slab d;d.row_bytes+=16;check(1,1,64,640,2560,d);d=Slab{};d.expert_bytes+=16;check(1,1,64,640,2560,d);}
 tier=1;check(1,1);tier=0;
 g_n_gpus=2;check(1,1);g_n_gpus=1;
 g_gpu[0].device_id=1;check(1,1);g_gpu[0].device_id=0;
 for(const char *f:{"bad","0","-1","320"," 32"}){force=f;check(5,1);assert(actual_tile==32);}
 force="64";env.insert("DS4_QWEN4EXP_NO_DOWN_RAW_PIPE");check(1,1);env.clear();
