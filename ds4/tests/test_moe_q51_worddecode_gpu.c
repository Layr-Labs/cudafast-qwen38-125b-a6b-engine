/* Native GPU bit comparison for packed Q5_1 routed-down dispatch.
...
enum { CAP = 8, GUARD = 64 };
static const char *disable = "DS4_QWEN4EXP_NO_DOWN_WORD_DECODE";
...
static void require(int ok, const char *what) {
    if (!ok) { fprintf(stderr, "MoE Q5_1 word decode: %s\n", what); exit(1); }
}
...
#undef INVOKE
    ...
    printf("MoE Q5_1 word decode gt=%u dt=%u experts=%u offset=%u PASS\n",gt,dt,experts,64+alignment); fflush(stdout);
}
...
int main(void) {
    ...
    printf("MoE Q5_1 word decode: %u complete buffers, %u graph checks, %u actual replays PASS\n",
