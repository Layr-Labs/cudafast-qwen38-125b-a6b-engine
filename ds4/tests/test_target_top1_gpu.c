...
static void run(uint32_t width,uint32_t rows,int diagnostic) {
 ...
 ds4_gpu_tensor *scores=ds4_gpu_tensor_alloc(count*4),*old=ds4_gpu_tensor_alloc(16),*out=ds4_gpu_tensor_alloc(16),*scratch=ds4_gpu_tensor_alloc(144);
 need(data&&after&&scores&&old&&out&&scratch,"allocations");
 uint32_t sentinel[4]={0xdeadbeef,0xdeadbeef,0xdeadbeef,0xdeadbeef},a[4],b[4];unsigned char canary[144],got[144];memset(canary,0xa5,144);
 for(unsigned mode=0;mode<9;mode++) {
  ...
  need(ds4_gpu_tensor_write(scores,0,data,count*4)&&ds4_gpu_tensor_write(old,0,sentinel,16)&&ds4_gpu_tensor_write(out,0,sentinel,16)&&ds4_gpu_tensor_write(scratch,0,canary,144),"uploads");
  ...
  need(ds4_gpu_tensor_read(scratch,0,got,144),"scratch witness read");
  int active=width>=65536&&rows<=2&&!diagnostic;
  need(active?memcmp(got,canary,rows*64)!=0:!memcmp(got,canary,144),"actual dispatch witness");
  if(active)need(!memcmp(got+rows*64,canary+rows*64,144-rows*64),"scratch tail unchanged");
 }
