...
static const char *ds4_gpu_qwen4exp_down_tune(){
    static std::string report="dnTune["+std::string(247,'T')+"]";calls++;return report.c_str();
}
...
int main(int argc,char **argv){assert(argc==2);mode=argv[1][0]-'0';
    ...
    if(mode==2){assert(result.empty()&&calls==0);}
        ...
        assert(calls==1&&result.size()<576);
        auto first=result.find("dnTune[");assert(first!=std::string::npos);
        assert(result.find(']',first)!=std::string::npos);
        assert(result.find('K')>result.find(']',first));
        char ident[768];std::string prefix(129,'I');
...
'''
with tempfile.TemporaryDirectory(prefix='down-tune-ident-') as d:
