...
for name, templated in (
        ...
        ('qw_gu_coop_raw_load', False), ('qwen4exp_gateup_decode_shape', False)):
    code += definition(name, templated)
kernel = definition('qwen4exp_moe_gateup_split_kernel', True)
code += kernel
with tempfile.TemporaryDirectory(prefix='gateup-shape-') as tmp:
    tmp = Path(tmp)
    ...
    subprocess.run([os.environ.get('CXX', 'c++'), *flags, '-I', str(tmp),
                    '-I', str(root),
                    str(root / 'tests/test_gateup_shape_host.cpp'),
