...
static void kernel_case(unsigned count,unsigned rows,unsigned groups,
    ...
    auto run=[&](bool optimized) {
        ...
        const bool use_pipe=optimized && Wide && Type==7 && (pattern&1u) && !(pattern&2u) && !(groups&3u);
        const unsigned tile_rows=use_pipe?(unsigned)QW_DOWN_RAW_BM:64u;
        for(blockIdx.y=0;blockIdx.y<work;blockIdx.y++)
            for(blockIdx.x=0;blockIdx.x<(rows+tile_rows-1u)/tile_rows;blockIdx.x++) {
                unsigned e=compact?(blockIdx.y<2?(unsigned)active[1+blockIdx.y]:experts):blockIdx.y;
                if(use_pipe && e<experts) {
                    const unsigned row0=blockIdx.x*tile_rows;
                    expected_copies+=(size_t)((counts[e]+31)/32)*(groups/4u)*
                        min(tile_rows,rows-row0)*6u;
                }
                cta(128,[&] {
