...
static void shared_vector_case(unsigned rows, unsigned alignment, unsigned K) {
    enum { D=640, O=2560, CAP=65, GUARD=64, M=2 };
...
#define READ_COMPARE(m, counter) do {for(unsigned j=0;j<3;j++){require_ok(ds4_gpu_tensor_read(t[m][j],0,got[j],sizes[j]),"shared tile output read");if(memcmp(ref[j],got[j],sizes[j])){fprintf(stderr,"SHARED_VECTOR mismatch K=%u rows=%u offset=%u mode=%u buffer=%u\n",K,rows,alignment,m,j);exit(1);}counter++;}}while(0)
    ...
    printf("SHARED_VECTOR K=%u rows=%u weight_offset=%llu eager_buffers=%u graph_buffers=%u replays=%u PASS\n",K,rows,(unsigned long long)go,eager,graphs,replays);fflush(stdout);
...
int main(void){
    ...
    for(unsigned a=0;a<2;a++) {
        for(unsigned w=0;w<sizeof(widths)/sizeof(widths[0]);w++)
            shared_vector_case(widths[w],a*2,2560);
