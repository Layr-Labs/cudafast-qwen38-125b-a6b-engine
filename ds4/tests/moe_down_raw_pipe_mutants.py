...
mutations={
 'wrong_expert_row': ('(uint64_t)(row0 + r) * down_row_bytes +','(uint64_t)(row0 + ((r + 1u) % 32u)) * down_row_bytes +'),
 ...
 'omitted_last_piece': ('i < QW_DOWN_RAW_BM * 6u;','i + 1u < QW_DOWN_RAW_BM * 6u;'),
 ...
 'wrong_token_quadrant': ('const uint32_t bn = (warp >> 1u) * 16u + nt * 8u', 'const uint32_t bn = nt * 8u'),
 'wrong_output_quadrant': ('const uint32_t nn = (warp >> 1u) * 16u + nt * 8u', 'const uint32_t nn = nt * 8u'),
