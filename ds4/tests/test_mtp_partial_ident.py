...
static const char *mtp_partial_tune(){
    static std::string report="partSel["+std::string(183,'T')+"]";calls++;return report.c_str();
}
...
int main(int argc,char **argv){assert(argc==2);mode=argv[1][0]-'0';
    ...
    if(mode==2){assert(result.empty()&&calls==0);}
        ...
        auto first=result.find("partSel[");assert(first!=std::string::npos);
...
'''
with tempfile.TemporaryDirectory(prefix='partial-ident-') as d:
