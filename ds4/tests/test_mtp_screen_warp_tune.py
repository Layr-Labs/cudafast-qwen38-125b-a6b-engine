...
launch=re.search(r'mtp_native_screen_staged_warp_kernel<<<([^,]+),([^,]+), 0, stream>>>',source)
assert launch
source=source.replace(launch[0],f'probe_geometry({launch[1]}, {launch[2]}); mtp_native_screen_staged_warp_kernel')
...
static void launch(bool warp,uint64_t *keys,uint32_t *flag,const unsigned char *w,const int8_t *x,const float *xs,unsigned width,unsigned vocab,unsigned prefix,unsigned tail){
 ...
 double cost=8;unsigned pair=elapsed/2%15,mode=elapsed/30;
 if(scenario=="marginal")cost=9.85;if(scenario=="slower")cost=11;
 if(scenario=="one_slow"&&mode==3)cost=11;
 if(scenario=="noisy")cost=pair==14?7:9.5;
 if(scenario=="large_noisy")cost=pair==14?8.8:5.3;
 if(scenario=="lucky_outlier")cost=pair==14?1:9.9;
 if(scenario=="big_bad_outlier")cost=pair==14?30:5;
 if(scenario=="thirteen_wins")cost=pair>=13?10.01:5;
 if(scenario=="twelve_wins")cost=pair<12?5:10.001;
 if(scenario=="edge")cost=9.8;
 gpu_us+=warp?cost:10;order.push_back(warp);
}
static void mtp_native_screen_staged_warp_kernel(uint64_t *k,uint32_t *f,const unsigned char *w,const int8_t *x,const float *s,uint32_t width,uint32_t v,uint32_t p,uint32_t t){launch(true,k,f,w,x,s,width,v,p,t);}
...
int main(int argc,char **argv){assert(argc==2);scenario=argv[1];auto sep=scenario.find(':');
 ...
 bool success=scenario=="fast"||scenario=="marginal"||scenario=="slower"||scenario=="one_slow"||scenario=="noisy"||scenario=="large_noisy"||scenario=="lucky_outlier"||scenario=="big_bad_outlier"||scenario=="thirteen_wins"||scenario=="twelve_wins"||scenario=="edge";
 if(success){assert(mtp_screen_tuned_device==0&&mtp_screen_tuned_width==4096&&elapsed==120&&allocs==6&&hosts==3&&events==2);
  assert(mtp_screen_prefer_warp==(scenario=="fast"||scenario=="large_noisy"||scenario=="thirteen_wins"));
  for(unsigned mode=0;mode<4;mode++){
   unsigned index=12+mode*962;assert(!order[index]&&order[index+1]);index+=2;
   for(unsigned pair=0;pair<15;pair++)for(unsigned leg=0;leg<2;leg++)for(unsigned repeat=0;repeat<32;repeat++){
    assert(order[index]==bool((pair+leg)&1));index++;
   }
  }
  assert(order.size()==3860);
...
cases=['no_dp4a','fast','marginal','slower','one_slow','noisy','large_noisy','lucky_outlier','big_bad_outlier','thirteen_wins','twelve_wins','edge',
 ...
 'record:1','record:2','record:100','wait:1','elapsed:1','elapsed:120']
