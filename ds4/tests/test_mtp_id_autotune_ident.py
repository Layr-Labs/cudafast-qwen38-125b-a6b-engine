...
static const char *mtp_native_id_sort_tune(){
    static std::string report="idSort18["+std::string(117,'T')+"]";calls++;return report.c_str();
}
...
int main(int argc,char **argv){assert(argc==2);mode=argv[1][0]-'0';
    ...
    if(mode==2){assert(result.empty()&&calls==0);}
        ...
        assert(calls==1&&result.size()<512);
        auto first=result.find("idSort18[");assert(first!=std::string::npos);
...
'''
with tempfile.TemporaryDirectory(prefix='mtp-id-ident-') as d:
