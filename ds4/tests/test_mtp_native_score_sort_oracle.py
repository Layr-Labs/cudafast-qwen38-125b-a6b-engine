...
# word only. The parent already bounds the ascending original-ID sort to
# ceil(log2(vocab)); check its dynamic range, then exercise the equivalence.
...
assert re.search(r'int id_bits = 1;\s*while \(id_bits < 32 && '
    ...
    'dynamic original-ID range changed; revisit its domain proof'
...
def check(prefix, tail, vocab, score_bits):
    ...
    bits = max(1, (vocab - 1).bit_length())
    unpacked = [m32 - (k & m32) for k in word[:cap]]
    assert sorted(unpacked, key=lambda x: x & ((1 << bits) - 1)) == selected(full), \
        'bounded original-ID range changed ascending order'
    return len(keys)
