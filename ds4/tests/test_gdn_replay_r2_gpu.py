"""Actual precise CUDA R2 replay vs unchanged replay and ordinary recurrence.
...
int main(){std::mt19937 rng(620019);unsigned eager=0,replay_count=0;cudaStream_t st;CK(cudaStreamCreate(&st));
 ...
 for(unsigned layout=0;layout<2;layout++)for(unsigned mode=0;mode<8;mode++){
  ...
  auto launch=[&](){
   ...
   qwen4exp_gdn_replay_r2_kernel<<<dim3(nv,16),128,0,st>>>(o1.data(),s1.data(),c1.data(),t1.data(),qkv.data(),alpha.data(),beta.data(),coeff.data(),bias.data(),nk,nv,2,layout,ctl,0);
   CK(cudaGetLastError());
  };
  auto oracle=[&](){
  ...
  auto compare=[&](){
   s0.same(s1,"state R2");c0.same(c1,"checkpoint R2");t0.same(t1,"tape R2");o0.same(o1,"output R2");s0.same(sr,"ordinary state");o0.same(orr,"ordinary output");
 ...
 CK(cudaStreamDestroy(st));printf("PASS replay R2: %u eager + %u changed-input/control graph replays; old replay and ordinary recurrence; state/checkpoint/tape/output/guards/input bits\n",eager,replay_count);
}
'''
with tempfile.TemporaryDirectory(prefix='gdn-replay-r2-') as t:
