...
#define ORIGINAL() ds4_gpu_qwen4exp_qsa_attention_dpos_tensor(a,tq,tk,tv,sel,cnt,rows,H,KV,D,0,CAP,SEL,0.0625f,tp,scratch,CAP)
 ...
 if(!sparse&&rows==1&&pos==1023)need(ds4_gpu_tensor_read(scratch,0,&original_score,4)&&original_score!=0.0f,"nonzero original score witness");
 ...
 if(!sparse&&rows==1&&pos==1023){float r=1;need(ds4_gpu_tensor_read(scratch,0,&r,4)&&r==0.0f,"dead-score rescale witness");}
 setenv("DS4_QWEN4EXP_NO_QSA_RESCALE","1",1);need(FUSED()==1,"legacy fused fold valve");
 identical(oa,ob,obytes,"legacy fused float");identical(qa,qb,pbytes,"legacy fused Q8");unsetenv("DS4_QWEN4EXP_NO_QSA_RESCALE");
