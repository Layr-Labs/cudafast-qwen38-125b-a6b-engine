...
p.add_argument('--raw64', action='store_true')
...
header = a.header.read_text().replace('#pragma once', '')
if a.raw64:
    header = header[:header.index('template <int DownType')]
    header = header.replace('QW_DOWN_RAW_BM = 32', 'QW_DOWN_RAW_BM = 64')
    header += (root / 'ds4_cuda_down_raw64.cuh').read_text().replace('#pragma once', '').replace('qwen4exp_moe_down_raw64_kernel', 'qwen4exp_moe_down_raw_pipe_kernel')
code += header
