/* Native GPU bit comparison for panel-reuse routed-down dispatch.
 * Compare full outputs, untouched intermediates, input/output guards and
 * graph replays with changed routes/activations. Tests production geometry,
 * invalid experts, zero/tiny/large inputs and shape/alignment fallbacks.
 * Build against the normal CUDA library; requires a GPU to execute.
 */
#include "ds4_gpu.h"
#include <stdint.h>
#include <stdio.h>
...
enum { CAP = 8, GUARD = 64 };
static const char *disable = "DS4_QWEN4EXP_NO_DOWN_PANEL_REUSE";
...
static void require(int ok, const char *what) {
    if (!ok) { fprintf(stderr, "MoE down panel reuse: %s\n", what); exit(1); }
}
...
#undef INVOKE
    ...
    printf("MoE down panel reuse gt=%u dt=%u experts=%u offset=%u PASS\n",gt,dt,experts,64+alignment); fflush(stdout);
}

int main(void) {
    require(setenv("DS4_QWEN4EXP_FORCE_DOWN_PANEL_REUSE","1",1) == 0, "force candidate under test");
    ...
    for (unsigned t = 0; t < 2; t++) {
        check_case(types[t],41,0,2560,640,2560); // active plan path at two rows
        check_case(types[t],13,0,2560,640,2560); // insufficient cursor span fallback
        ...
        check_case(types[t],17,0,256,1056,24); // group-count fallback
    }
    printf("MoE down panel reuse: %u complete buffers, %u graph checks, %u actual replays PASS\n",
