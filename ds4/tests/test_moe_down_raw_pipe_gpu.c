/* Complete MoE comparison for the raw-byte pipelined down MMA, including graph
 ...
 * cc -O2 -std=c11 -D_GNU_SOURCE -Ids4 ds4/tests/test_moe_down_raw_pipe_gpu.c \
 *   -L.build/ds4 -lds4qwen -lm -o /tmp/test-moe-down-raw-pipe
 */
#include "ds4_gpu.h"
#include <stdint.h>
#include <stdio.h>
...
enum { K = 256, D = 640, O = 256, CAP = 1024, GUARD = 64 };
static const char *disable = "DS4_QWEN4EXP_NO_DOWN_RAW_PIPE";
...
static void require(int ok, const char *what) {
    if (!ok) { fprintf(stderr, "MoE down raw pipe: %s\n", what); exit(1); }
}
...
#undef INVOKE
    ...
    printf("MoE down raw pipe gt=%u dt=%u experts=%u offset=%u PASS\n",gt,dt,experts,64+alignment); fflush(stdout);
}

int main(int argc, char **argv) {
    require(argc == 2 && (!strcmp(argv[1],"32") || !strcmp(argv[1],"64")), "choose raw tile 32 or 64");
    require(setenv("DS4_QWEN4EXP_FORCE_DOWN_RAW_PIPE",argv[1],1) == 0, "force tested tile");
    ...
    printf("MoE down raw pipe: %u complete buffers, %u graph checks, %u actual replays PASS\n",
