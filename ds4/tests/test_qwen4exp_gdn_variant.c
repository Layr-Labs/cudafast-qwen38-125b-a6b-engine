...
/* The other half of the same argument.  The warm-up's case rests on "parity flips
 * only on a round that SWAPS", and `swap` is decided here, host-side, by
 * ds4_qwen4exp_gdn_replay_plan().  These checks state the plan's contract as
 * implications and drive it over the whole reachable domain, so a change that
 * lets a swap happen without an active replay (or lets a reused row swap) fails
 * here rather than silently invalidating the warm-up's reasoning. */
static void test_swap_requires_an_active_replay(void) {
    printf("a swap requires an active replay and implies it\n");
    int cases = 0;
    for (int enabled = 0; enabled <= 1; enabled++)
        for (int previous = 0; previous <= 1; previous++)
            for (uint32_t width = 1; width <= 3; width++)
                for (uint32_t snap = 0; snap <= 1; snap++)
                    for (uint32_t recurrent = 0; recurrent <= 1; recurrent++)
                        for (uint32_t conv = 0; conv <= 1; conv++) {
                            const ds4_qwen4exp_gdn_replay_step p =
                                ds4_qwen4exp_gdn_replay_plan(
                                    enabled != 0, previous != 0, 3u, 8u, width,
                                    snap, 2u, recurrent, conv);
                            cases++;
                            /* swap is exactly "active and not reusable" */
                            const bool reuse = p.active && previous != 0 &&
                                               recurrent == 1u && conv == 1u;
                            ok(p.swap == (p.active && !reuse),
                               "swap == active && !reuse");
                            ok(!p.swap || p.active, "swap implies active");
                            ok(!(p.swap && reuse), "a reusable round never swaps");
                            /* active needs all three of enabled, width 2, snap 1 */
                            const bool want_active =
                                enabled != 0 && width == 2u && snap == 1u;
                            ok(p.active == want_active,
                               "active == enabled && width==2 && snapshots==1");
                        }
    printf("  %d argument combinations driven\n", cases);
}

static void test_prefix_advances_only_on_reuse_and_wraps(void) {
    printf("the replay prefix advances only on reuse and wraps at capacity\n");
    const uint32_t cap = 4u;
    for (uint32_t old = 0; old <= cap + 1u; old++)
        for (int previous = 0; previous <= 1; previous++) {
            const ds4_qwen4exp_gdn_replay_step p = ds4_qwen4exp_gdn_replay_plan(
                true, previous != 0, old, cap, 2u, 1u, 2u, 1u, 1u);
            const bool reuse = p.active && previous != 0;
            if (!reuse) {
                ok(p.prefix == 0u, "no reuse leaves the prefix at zero");
            } else if (old == cap) {
                ok(p.prefix == 0u, "reuse at capacity wraps to zero");
            } else {
                ok(p.prefix == old + 1u, "reuse advances the prefix by one");
            }
            /* A replay that is not active must never carry a prefix forward. */
            const ds4_qwen4exp_gdn_replay_step off = ds4_qwen4exp_gdn_replay_plan(
                false, previous != 0, old, cap, 2u, 1u, 2u, 1u, 1u);
            ok(!off.active && off.prefix == 0u && !off.swap,
               "an inert plan is inert in every field");
        }
}

/* The plan's third output.  `settle` is the field that says a round must
 * reconcile the two parities before the next identity is safe -- the condition
 * the warm-up's argument leans on when it claims a parity can only be captured
 * on a round that swaps.  The predicate is recomputed here rather than trusted,
 * and driven over the whole domain, so a change to any single clause fails. */
static void test_settle_fires_exactly_on_the_documented_conditions(void) {
    printf("settle fires exactly on the documented conditions\n");
    const uint32_t max_adopt = 2u;
    int cases = 0;
    uint32_t settle_true = 0;
    for (int enabled = 0; enabled <= 1; enabled++)
        for (int previous = 0; previous <= 1; previous++)
            for (uint32_t width = 1; width <= 3; width++)
                for (uint32_t snap = 0; snap <= 1; snap++)
                    for (uint32_t recurrent = 0; recurrent <= 1; recurrent++)
                        for (uint32_t conv = 0; conv <= 1; conv++) {
                            const ds4_qwen4exp_gdn_replay_step p =
                                ds4_qwen4exp_gdn_replay_plan(
                                    enabled != 0, previous != 0, 3u, 8u, width,
                                    snap, max_adopt, recurrent, conv);
                            const bool active =
                                enabled != 0 && width == 2u && snap == 1u;
                            const bool reuse = active && previous != 0 &&
                                               recurrent == 1u && conv == 1u;
                            const bool want =
                                (recurrent != 0u || conv != 0u) &&
                                (width > max_adopt || recurrent != conv ||
                                 (active && !reuse) ||
                                 (!active && previous != 0 &&
                                  recurrent != 0u));
                            cases++;
                            if (p.settle) settle_true++;
                            ok(p.settle == want,
                               "settle matches the documented predicate");
                            /* The two clauses the warm-up depends on, as
                             * implications on their own. */
                            if (recurrent != conv &&
                                (recurrent != 0u || conv != 0u))
                                ok(p.settle, "disagreeing parities always settle");
                            if (width > max_adopt && (recurrent || conv))
                                ok(p.settle, "an over-wide row with work settles");
                        }
    ok(settle_true > 0u && settle_true < (uint32_t)cases,
       "settle is neither always on nor always off over the domain");
    printf("  %d combinations driven, settle true in %u\n", cases, settle_true);
}
...
int main(void) {
    ...
    test_swap_requires_an_active_replay();
    test_prefix_advances_only_on_reuse_and_wraps();
    test_settle_fires_exactly_on_the_documented_conditions();
