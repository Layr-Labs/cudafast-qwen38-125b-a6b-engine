...
template<int Type> static void trial(unsigned groups,unsigned nt,unsigned slots,
                                     unsigned pattern,unsigned rows) {
    ...
    auto run=[&](unsigned mode) {
        for(unsigned b=0;b<rows/8;b++) for(unsigned t=0;t<(nt+1)/2;t++) {
            ...
            cta(mode==1?128:256,[&] {
                if(mode==1) qwen4exp_moe_down_halfwarp_kernel<Type>(
                    got.data()+4,(const char*)w,selected.data(),mq,ms.data(),
                    sums.data(),expert_bytes,row_bytes,groups,rows,nt,experts,slots);
...
int main() {
    (void)barriers;
    for(unsigned groups: {17u,20u,24u,31u,32u})
    ...
    std::printf("MoE down halfwarp: %u cases, staged/unstaged/halfwarp bits and guards PASS\n",cases);
}
