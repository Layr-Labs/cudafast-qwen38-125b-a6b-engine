/* Native GPU bit comparison for shape gate/up dispatch.
...
enum { CAP = 8, GUARD = 64 };
static const char *disable = "DS4_QWEN4EXP_NO_GATEUP_DECODE_SHAPE";
...
static void require(int ok, const char *what) {
    if (!ok) { fprintf(stderr, "MoE gate/up shape: %s\n", what); exit(1); }
}
...
static void check_case(unsigned dt, unsigned experts, unsigned alignment,
    ...
    const unsigned stride = used * D + ((K==2560 && D==640) ? 0 : 16);
...
#undef INVOKE
    ...
    printf("MoE gate/up shape gt=%u dt=%u experts=%u offset=%u PASS\n",gt,dt,experts,64+alignment); fflush(stdout);
}
...
int main(void) {
    ...
    printf("MoE gate/up shape: %u complete buffers, %u graph checks, %u actual replays PASS\n",
