/* Complete MoE comparison for the Q8 down activation-bias staging, including graph
...
static const char *disable = "DS4_QWEN4EXP_NO_DOWN_Q8_BIAS";
...
static void require(int ok, const char *what) {
    if (!ok) { fprintf(stderr, "MoE Q8 bias: %s\n", what); exit(1); }
}
...
#undef INVOKE
    ...
    printf("MoE Q8 bias gt=%u dt=%u experts=%u offset=%u PASS\n",gt,dt,experts,64+alignment); fflush(stdout);
}
...
int main(void) {
    ...
    printf("MoE Q8 bias: %u complete buffers, %u graph checks, %u actual replays PASS\n",
