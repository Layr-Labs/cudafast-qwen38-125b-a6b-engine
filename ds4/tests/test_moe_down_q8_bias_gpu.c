/* Complete MoE comparison for the down MMA shared-memory layout, including graph
 ...
 * cc -O2 -std=c11 -D_GNU_SOURCE -Ids4 ds4/tests/test_moe_down_q8_bias_gpu.c \
 *   -L.build/ds4 -lds4qwen -lm -o /tmp/test-moe-q8-bias
 */
#include "ds4_gpu.h"
#include <stdint.h>
#include <stdio.h>
...
static const char *disable = "DS4_QWEN4EXP_NO_DOWN_MMA_SWIZZLE";
...
static void require(int ok, const char *what) {
    if (!ok) { fprintf(stderr, "MoE down swizzle: %s\n", what); exit(1); }
}
...
#undef INVOKE
    ...
    printf("MoE down swizzle gt=%u dt=%u experts=%u offset=%u PASS\n",gt,dt,experts,64+alignment); fflush(stdout);
}
...
int main(void) {
    ...
    printf("MoE down swizzle: %u complete buffers, %u graph checks, %u actual replays PASS\n",
