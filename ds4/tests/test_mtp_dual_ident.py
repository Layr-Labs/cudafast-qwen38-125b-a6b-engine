"""Compile the production hello-limits formatter with padded CUDA stubs."""
...
static int mode,calls,partial_calls;
...
static const char *mtp_screen_warp_tune(){
    static std::string report="scrWarp["+std::string(183,'T')+"]";calls++;return report.c_str();
}
static const char *mtp_partial_tune(){static std::string report="partSel["+std::string(183,'P')+"]";partial_calls++;return report.c_str();}
...
int main(int argc,char **argv){assert(argc==2);mode=argv[1][0]-'0';
    ...
    if(mode==2){assert(result.empty()&&calls==0&&partial_calls==0);}
    else{
        assert(calls==1&&partial_calls==1&&result.size()<1024);
        auto first=result.find("scrWarp[");assert(first!=std::string::npos);
        assert(result.find(']',first)!=std::string::npos);
        auto second=result.find("partSel[");assert(second>first&&second!=std::string::npos);
        assert(result.find(']',second)!=std::string::npos);
        assert(result.find('K')>result.find(']',second));
        assert(result.rfind(std::string(383,'K'))==result.size()-383);
...
'''
with tempfile.TemporaryDirectory(prefix='screen-warp-ident-') as d:
