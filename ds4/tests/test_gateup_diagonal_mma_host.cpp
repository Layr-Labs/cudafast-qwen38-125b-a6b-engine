...
static void kernel_case(unsigned nt,unsigned slots,unsigned rows,
    ...
    auto run=[&](unsigned mode) {
        ...
        const unsigned tile = mode == 1 ? 2u : 4u;
        for(unsigned b=0;b<(rows+tile-1)/tile;b++)for(unsigned e=0;e<experts;e++) {
...
#define ARGS out,(const char *)gate.data(),(const char *)up.data(),xq, \
    ...
    expert_bytes,row_bytes,12,12,groups,rows,stride,slots
                ...
                else if(mode==1)qwen4exp_moe_gateup_diagonal_mma_kernel(ARGS);
...
int main() {
    ...
    std::printf("PASS: %u actual DP4A/staged/MMA kernel fiber cases\n",kernel_checks);
}
