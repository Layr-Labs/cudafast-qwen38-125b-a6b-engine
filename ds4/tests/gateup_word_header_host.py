...
Header bytes, scale products, payload words, reductions, panel row indexing,
...
for name, templated in (
        ...
        ('qw_gu_coop_raw_load', False), ('qw_gu_coop_decode', False)):
    code += definition(name, templated)
kernel = definition('qwen4exp_moe_gateup_split_kernel', True)
# Restore the original byte accessor at the single changed call site. All
# other statements are the actual current CUDA body. Comparing the noncoop
# body too provides a separately indexed, unstaged reference for the panel.
original = kernel.replace('qwen4exp_moe_gateup_split_kernel(', 'gateup_parent(')
needle = 'qw_gu_coop_decode(wsh, wrow, g_, (RAWP), wq, wa, wb);'
assert original.count(needle) == 1
original = original.replace(needle, '''dev_qwen4exp_group_decode_w((uint32_t)Type, \\
                        (const char *)(const void *) \\
                            &wsh[wrow * QW_GU_COOP_ROW_U4], g_, \\
                        (RAWP), wq, wa, wb);''')
code += original + kernel
with tempfile.TemporaryDirectory(prefix='gateup-word-header-') as tmp:
    tmp = Path(tmp)
    ...
    subprocess.run([os.environ.get('CXX', 'c++'), *flags, '-I', str(tmp),
                    '-I', str(root),
                    str(root / 'tests/test_gateup_word_header_host.cpp'),
