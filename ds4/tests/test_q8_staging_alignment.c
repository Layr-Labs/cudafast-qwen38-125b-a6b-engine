/* Q8 staging alignment regression. Link against libds4qwen.
 * Compare optimized staging with its unstaged oracle on identical Q8 bytes
 * and quantized activations, including output tails and unaligned slabs. */
#include "ds4_gpu.h"
#include <math.h>
#include <stdint.h>
#include <stdio.h>
...
static void require(int ok, const char *what) {
    if (!ok) { fprintf(stderr, "Q8 staging alignment: %s\n", what); exit(1); }
}
...
static void check_shape(uint64_t in, uint64_t out0, uint64_t out1,
    ...
    const uint32_t widths[] = {1, 2, 3, 4, 7, 4, 3};
    for (size_t c = 0; c < sizeof(widths) / sizeof(widths[0]); c++) {
        ...
        for (int branch = 0; branch < 2; branch++) {
            ...
            require(setenv("DS4_QWEN4EXP_NO_PAIR_LANES_STAGE", "1", 1) == 0,
                    "disable panel staging");
            require(setenv("DS4_QWEN4EXP_NO_PAIR_LANES_ROLL", "1", 1) == 0,
                    "disable rolling staging");
            ...
            require(unsetenv("DS4_QWEN4EXP_NO_PAIR_LANES_STAGE") == 0,
                    "enable panel staging");
            require(unsetenv("DS4_QWEN4EXP_NO_PAIR_LANES_ROLL") == 0,
                    "enable rolling staging");
            require(ds4_gpu_matmul_q8_0_preq_rows_exact_tensor(
                yt, model, bytes, offsets[branch], in, out, qt, 0, soff, rows),
            ...
            for (size_t i = 0; i < live_ybytes / sizeof(float); i++) {
                if (!isfinite(reference[i]) || !isfinite(got[i]) ||
                    ...
                    fprintf(stderr, "wide Q8 %llu->%llu rows %u branch %d "
                                    "at %zu: %.9g != %.9g\n",
    ...
    printf("wide Q8 %llu->(%llu,%llu), offset %llu: 14 projections exact\n",
...
int main(void) {
    ...
    check_shape(6144, 2560, 1537, 64);
    check_shape(6144, 2560, 1537, 66);
    puts("Q8 staging alignment: all 70 projections passed");
    return 0;
}
