/* Actual staged DP4A CUDA body with host fibers and warp synchronization.
 * Independent raw scalar dots, float tree and key generation are the oracle. */
#include <ucontext.h>
#include <algorithm>
#include <array>
#include <cmath>
...
static void __syncwarp(uint32_t mask=0xffffffffu){warp_barrier(mask);}
...
alignas(16) static unsigned char host_shared_storage[16u + 4u*816u + 16u];
...
static void host_fragment_bounds(const unsigned char *payload,unsigned byte) {
    ...
    need(payload>=first+2u && payload+32u<=first+4u*816u,"fragment must read shared slab");
...
static unsigned __cvta_generic_to_shared(const void *ptr) {
    const auto *p=(const unsigned char *)ptr;
    need(p>=host_shared_storage+16u && p<host_shared_storage+16u+4u*816u,
...
static void cta(int nth,std::function<void()> f) {
    ...
    for(unsigned i=0;i<16u;i++) {
        ...
        need(host_shared_storage[16u+4u*816u+i]==0xad,"shared suffix guard");
    }
}
...
static void one(unsigned prefix,unsigned tail,unsigned offset,unsigned pattern){
 ...
 for(unsigned b:blocks){
  blockIdx.x=b;cta(128,[&](){mtp_native_screen_staged_warp_kernel(out.data(),&invalid,w,x.data(),xs.data(),width,vocab,prefix,tail);});
...
int main(){unsigned cases=0;
 ...
 std::printf("MTP staged one-warp actual-body PASS: %u cases; exact keys/flags, mapped prefix-only copy bounds, shared guards, exact byte count and both alignment paths\n",cases);
}
