...
gate=definition(partial,'static bool mtp_partial_eligible(')
...
#include <string>
#include <vector>
constexpr uint32_t MTP_NATIVE_DIM=2560,MTP_NATIVE_CAP=2048,MTP_NATIVE_MAX_WIDTH=1<<20,MTP_PARTIAL_LIMIT=4096;
...
int main(){
 ...
 for(const char *s:{"device","capture","clear","quant","fused","pivot","filter","read","partial_ids","refine"}){failure=s;run(-1,-1);}
 supplied_count=4097;
