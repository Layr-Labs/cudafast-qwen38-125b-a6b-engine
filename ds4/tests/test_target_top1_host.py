...
for(uint32_t width:{65536u,65537u,65543u,248320u})for(uint32_t rows:{1u,2u})for(unsigned mode=0;mode<10;mode++){
 std::vector<float>x((uint64_t)width*rows);std::vector<uint32_t>out(rows,UINT32_MAX),expected(rows);std::vector<target_top1_pair>p(rows*32);
 ...
 if(mode==6){std::fill(x.begin(),x.end(),-1.f);for(unsigned i:{255u,256u,1023u,1024u,8191u,8192u})x[i]=7.f;}
 ...
 for(unsigned r=0;r<rows;r++){
  ...
  for(unsigned part=0;part<32;part++){blockIdx={part,r};std::vector<task>v;for(unsigned t=0;t<256;t++)v.push_back(target_top1_partition_kernel(p.data(),x.data(),width,rows));execute(v);}
