    ...
    for(unsigned replay=0;replay<(adversarial?1u:3u);replay++) {
        ...
        if(adversarial>=2 && adversarial<=4) {
            ...
            for(unsigned i=0;i<CAP;i++) {
                need(found[i]<VOCAB&&!seen[found[i]]&&(!i||found[i]>found[i-1]),"invalid-case IDs bounded, unique and ascending");seen[found[i]]=1;
            }
        ...
        need(!memcmp(found,async_ids,sizeof found)&&!memcmp(selected,async_logits,sizeof selected),"async retains ascending IDs and row-for-row scores");
