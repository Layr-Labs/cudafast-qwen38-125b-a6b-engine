...
static int compare_key_paths(ds4_gpu_tensor *out,ds4_gpu_tensor *ids,
    ...
    for(unsigned mode=0;mode<4;mode++) {
        ...
        if(mode&2)unsetenv("DS4_MTP_NO_PARTIAL_SELECT");else setenv("DS4_MTP_NO_PARTIAL_SELECT","1",1);
        ...
        if(mode==3 && status[mode]>0) need(ds4_gpu_tensor_read(scratch,flag_at+4u,&selected_count,4),"partial count witness");
        const int partial_taken=mode==3 && selected_count>=CAP && selected_count<=4096u;
        need(partial_taken||status[mode]==0 ? !memcmp(score_canary,score_after,CAP*4u)
            : memcmp(score_canary,score_after,CAP*4u)!=0,"actual partial/full sort dispatch witness");
...
int main(void){need(setenv("DS4_MTP_FORCE_PARTIAL_SELECT","1",1)==0,"force partial candidate for native test");setenv("DS4_CUDA_DECODE_GRAPHS","1",1);run_case(0,0);run_case(0,2);run_case(1,0);run_case(2,0);run_case(3,0);run_case(3,2);puts("native screen contracts pass (selection deliberately approximate)");return 0;}
