...
code = '''static int qwen4exp_down_tuned_device = -1;
static bool qwen4exp_down_prefer_reuse = false;
static int g_n_gpus = 1, probe_device = 0, probe_device_error = 0;
static constexpr int cudaSuccess = 0;
static int cudaGetDevice(int *p) { *p = probe_device; return probe_device_error; }
'''
...
for name, templated in (
        ('warp_sum_f32', False),
        ('qwen4exp_moe_group_small_block', False),
        ('qwen4exp_down_reuse_plan', True),
        ('qwen4exp_moe_group_small_kernel', False),
        ('qwen4exp_moe_group_plan_kernel', False), ('dev_q4_K_get_scale_min', False),
        ...
        ('qwen4exp_down_autotune_selected', False),
        ('qwen4exp_down_panel_reuse', False),
        ('qwen4exp_down_plan_enabled', False),
        ('qwen4exp_moe_down_q_kernel', True),
        ('qwen4exp_moe_down_reuse_kernel', True)):
    code += definition(name, templated).replace('__shared__ int32_t', 'static int32_t')
