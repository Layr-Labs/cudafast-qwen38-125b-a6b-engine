...
for name, templated in (
        ...
        ('qw_load_words8', False),
        ('qw_load_words6', False),
        ('qw_pack4', False),
        ('qw_tile_store_group', False),
        ('dev_qwen4exp_group_decode', False),
        ('dev_qwen4exp_group_decode_w', False),
...
with tempfile.TemporaryDirectory(prefix='moe-q51-worddecode-') as tmp:
    tmp = Path(tmp)
    ...
    subprocess.run([os.environ.get('CXX', 'c++'), *flags, '-I', str(tmp),
                    '-I', str(root),
                    str(root / 'tests/test_moe_q51_worddecode_host.cpp'),
