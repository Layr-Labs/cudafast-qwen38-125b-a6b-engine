#!/usr/bin/env python3
"""Execute both actual down-MMA specializations using host CUDA fibers.
...
for name, templated in (
        ...
        ('qw_down_mma_word', False), ('qw_tile_store_zero', False), ('qw_tile_copy_group', False),
...
with tempfile.TemporaryDirectory(prefix='moe-down-q8-bias-') as tmp:
    tmp = Path(tmp)
    (tmp / 'down_q8_bias_bodies.inc').write_text(code)
    ...
    subprocess.run([os.environ.get('CXX', 'c++'), *flags, '-I', str(tmp),
                    '-I', str(root),
                    str(root / 'tests/test_moe_down_q8_bias_host.cpp'),
