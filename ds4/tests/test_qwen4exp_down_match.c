/* Complete MoE comparison for the same-slot Q5_1 down decode reuse, including graph
 ...
 * cc -O2 -std=c11 -D_GNU_SOURCE -Ids4 ds4/tests/test_qwen4exp_down_match.c \
 *   -L.build/ds4 -lds4qwen -lm -o /tmp/test-down-match
 */
#include "ds4_gpu.h"
#include <stdint.h>
#include <stdio.h>
...
enum { K = 2560, D = 640, O = 2560, CAP = 4, GUARD = 64 };
static const char *disable = "DS4_QWEN4EXP_NO_DOWN_MATCH_REUSE";
...
static void require(int ok, const char *what) {
    if (!ok) { fprintf(stderr, "MoE down match: %s\n", what); exit(1); }
}
static unsigned block_bytes(unsigned type) {
    return type == 7 ? 24 : type == 8 ? 34 : type == 12 ? 144 :
           type == 13 ? 176 : 210;
}
static unsigned row_bytes(unsigned type, unsigned dim) {
    return block_bytes(type) * ((type == 7 || type == 8) ? dim / 32 : dim / 256);
}
...
#define POISON(M) do { for(unsigned pj=0;pj<3;pj++) \
    ...
    const unsigned widths[] = {1, 2, 3, CAP};
        ...
        for (unsigned trial = 0; trial < 12; trial++) {
            ...
            for (unsigned t = 0; t < CAP; t++) {
                ...
                for (unsigned slot = 0; slot < used; slot++) {
                    /* Same-set overlap alone must not trigger reuse. */
                    int e = (int)((slot + (t & 1u) * used) % experts);
                    const unsigned pattern = trial % 6;
                    const unsigned matches = pattern == 0 ? 0 : pattern == 1 ? 1 :
                                             pattern == 2 ? used / 2 : used;
                    if (slot < matches) e = (int)(slot % experts);
                    if (pattern == 4) e = (int)((slot + (t & 1u)) % used % experts);
                    if (pattern == 5) e = (int)(slot % 2 % experts); /* duplicates */
                    if (trial >= 6 && slot % 3 == 0) e = -1;
                    if (trial >= 6 && slot % 3 == 1) e = (int)experts;
...
#undef INVOKE
    ...
    printf("MoE down match gt=%u dt=%u experts=%u offset=%u PASS\n",gt,dt,experts,64+alignment); fflush(stdout);
}

int main(void) {
    require(setenv("DS4_CUDA_COPY_MODEL","1",1) == 0 && setenv("DS4_CUDA_DECODE_GRAPHS","1",1) == 0, "test environment");
    const unsigned types[][2] = {{12,7},{8,7},{8,8}};
    for (unsigned t = 0; t < sizeof(types)/sizeof(types[0]); t++)
        for (unsigned a = 0; a < 2; a++) check_case(types[t][0],types[t][1],32,a*2);
    check_case(12,7,1,0); check_case(8,7,17,2);
    printf("MoE down match: %u complete buffers, %u graph checks, %u actual replays PASS\n",
