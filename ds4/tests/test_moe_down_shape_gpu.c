/* Native GPU bit comparison for shape routed-down dispatch.
...
enum { CAP = 8, GUARD = 64 };
static const char *disable = "DS4_QWEN4EXP_NO_DOWN_DECODE_SHAPE";
...
static void require(int ok, const char *what) {
    if (!ok) { fprintf(stderr, "MoE down shape: %s\n", what); exit(1); }
}
...
#undef INVOKE
    ...
    printf("MoE down shape gt=%u dt=%u experts=%u offset=%u PASS\n",gt,dt,experts,64+alignment); fflush(stdout);
}
...
int main(void) {
    ...
    printf("MoE down shape: %u complete buffers, %u graph checks, %u actual replays PASS\n",
