...
static void kernel_case(unsigned count,unsigned rows,unsigned groups,
    ...
    const size_t row_bytes=groups*34u, expert_bytes=rows*row_bytes;
    ...
    for(unsigned e=0;e<experts;e++) for(unsigned r=0;r<rows;r++)
        for(unsigned g=0;g<groups;g++) {
            const size_t at=e*expert_bytes+r*row_bytes+g*34u;
            const uint16_t h=hs[(pattern+e+r+g)%7];
            std::memcpy(w+at,&h,2);
            for(unsigned k=0;k<32;k++) w[at+2+k]=(uint8_t)random_word();
        }
    ...
    auto run=[&](bool optimized) {
        for(blockIdx.y=0;blockIdx.y<(compact?4:experts);blockIdx.y++)
            for(blockIdx.x=0;blockIdx.x<(rows+63)/64;blockIdx.x++) {
                cta(128,[&] {
#define CALL(B,P) qwen4exp_moe_down_mma_kernel<DS4_QWEN4EXP_TY_q8_0,false,B>( \
                    P.data(),(const char *)w,mq.data(),ms.data(),sums.data(), \
                    pairs.data(),counts.data(),offsets.data(),compact?active.data():nullptr, \
                    expert_bytes,row_bytes,8,groups,rows,1)
...
int main() {
    ...
    std::printf("down Q8 bias: %u actual-kernel cases, 3200000 scalar recurrence steps PASS\n",cases);
}
