...
#include "down_q8_bias_bodies.inc"
...
static void kernel_case(unsigned count,unsigned rows,unsigned groups,
    ...
    std::vector<int32_t> counts={int(count),0,int(count)};
    ...
    auto run=[&](bool optimized) {
        for(blockIdx.y=0;blockIdx.y<(compact?4:experts);blockIdx.y++)
            for(blockIdx.x=0;blockIdx.x<(rows+63)/64;blockIdx.x++) {
                cta(128,[&] {
#define CALL(B,P) qwen4exp_moe_down_mma_kernel<Type,Wide,B && Type==8,B>( \
...
int main() {
    // Every logical word has exactly one slot. Both staging and fragment
    // instructions touch each 32-bit shared bank once across a warp.
    for(unsigned rows: {32u,64u}) {
        std::vector<unsigned> seen(rows*32);
        for(unsigned r=0;r<rows;r++) for(unsigned g=0;g<4;g++)
            for(unsigned k=0;k<8;k++) {
                unsigned at=qw_down_mma_word(r,g,k);
                need(at<seen.size() && seen[at]++==0,"tile layout bijection");
            }
    }
    for(unsigned k=0;k<8;k++) {
        uint32_t banks=0;
        for(unsigned t=0;t<32;t++) banks|=1u<<(qw_down_mma_word(t/4,t%4,k)%32);
        need(banks==0xffffffffu,"staging store bank conflict");
    }
    for(unsigned g=0;g<4;g++) for(unsigned r=0;r<64;r+=8)
        for(unsigned k=0;k<8;k+=4) {
            uint32_t banks=0;
            for(unsigned t=0;t<32;t++) banks|=1u<<(qw_down_mma_word(r+t/4,g,k+t%4)%32);
            need(banks==0xffffffffu,"MMA fragment bank conflict");
        }
    // Starting at +0 is essential. Compare every intermediate accumulator
    // of random integer-dot recurrences, rather than assuming arbitrary -0
    // accumulators are reachable. Check nonfinite classification separately.
    for(unsigned i=0;i<100000;i++) {
        float ref=0.0f,got=0.0f;
        for(unsigned g=0;g<32;g++) {
            float sc=from_bits(random_word()),wa=from_bits(random_word());
            int dot=int(random_word()%1048577)-524288;
            int sum=int(random_word()%8193)-4096;
            ref=std::fma(wa*sc,(float)dot,ref);
            ref=std::fma(0.0f*sc,(float)sum,ref);
            volatile float staged=sc+0.0f*sc;
            got=std::fma(wa*staged,(float)dot,got);
            need((std::isnan(ref)&&std::isnan(got)) || bits(ref)==bits(got),
                 "integer-dot recurrence zero-bias identity");
        }
    }
    for(unsigned count: {1u,7u,8u,9u,16u,17u,31u,32u,33u,65u})
    ...
    std::printf("down MMA swizzle: %u actual-kernel cases, 3200000 scalar recurrence steps PASS\n",cases);
}
