...
static void replay_case(void *model, const weight_set *ws, bool captured, bool deferred) {
    const uint64_t sb=(uint64_t)STATE_ELEMENTS*sizeof(float);
    const uint64_t cb=(uint64_t)HISTORY*CONV_DIM*sizeof(float);
    const uint64_t tb=(deferred?DS4_QWEN4EXP_GDN_DEFER_TAPE_ROWS:DS4_QWEN4EXP_GDN_REPLAY_ROWS)*
    ...
    ds4_gpu_qwen4exp_gdn_replay replay={checkpoint,tape,control,deferred};
    ...
    uint32_t pending=0,prefix=0,start=0,phase=0;
    void *state_before=require_alloc(sb,"state write guard");
    void *state_after=require_alloc(sb,"state write guard");
    bool previous=false;
    const char *pattern="RRRAARRRRRARARAAA";
    unsigned replays=0;
    for(unsigned round=0;round<32;round++) {
        ds4_qwen4exp_gdn_replay_step p=deferred?ds4_qwen4exp_gdn_deferred_plan(
            true,previous,prefix,start,2,1,7,pending,pending):ds4_qwen4exp_gdn_replay_plan(
        ...
        require_ok(ds4_gpu_qwen4exp_update_dpos(control,deferred?
            ds4_qwen4exp_gdn_deferred_control(start,prefix):prefix),"prefix upload");
        for(unsigned j=0;j<2;j++) {
        ...
        if(deferred) require_ok(ds4_gpu_tensor_read(b[1].state,0,state_before,sb),"canonical before");
        require_ok(ds4_gpu_begin_commands(),"candidate commands");
        ds4_decode_graph_key key={0};
        key.variant=ds4_qwen4exp_gdn_graph_variant(2,1,phase,true,deferred);
        ...
        if(deferred) {
            require_ok(ds4_gpu_tensor_read(b[1].state,0,state_after,sb),"canonical after");
            require_ok(memcmp(state_before,state_after,sb)==0,"verify omitted final state write");
            require_ok(ds4_gpu_qwen4exp_gdn_deferred_materialize(materialized,checkpoint,tape,
                ds4_qwen4exp_gdn_deferred_after(start,prefix,2),
                KEY_HEADS,VALUE_HEADS,ws->layout),"materialize final state");
            compare_tensor("final recurrent state bits",b[0].state,materialized,sb);
        } else compare_tensor("final recurrent state bits",b[0].state,b[1].state,sb);
        ...
        if(deferred) require_ok(ds4_gpu_qwen4exp_gdn_deferred_materialize(materialized,checkpoint,tape,
            ds4_qwen4exp_gdn_deferred_after(start,prefix,1),
            KEY_HEADS,VALUE_HEADS,ws->layout),"materialize row zero");
        else require_ok(ds4_gpu_qwen4exp_gdn_replay_materialize(materialized,checkpoint,tape,
    ...
    require_ok(!ds4_gpu_qwen4exp_gdn_deferred_materialize(materialized,checkpoint,tape,16,
        KEY_HEADS,VALUE_HEADS,ws->layout),"oversized descriptor refused");
    require_ok(!ds4_gpu_qwen4exp_gdn_deferred_materialize(checkpoint,checkpoint,tape,0,
        KEY_HEADS,VALUE_HEADS,ws->layout),"deferred checkpoint alias refused");
    printf("PASS CUDA replay layout=%u captured=%u deferred=%u rounds=32 graph_replays=%u\n",ws->layout,captured,deferred,replays);
    ds4_gpu_decode_graphs_invalidate();
    free(qkv);free(alpha);free(beta);free(gate);
    free(state_before);free(state_after);
...
int main(void) {
    ...
    for(unsigned deferred=0;deferred<2;deferred++) for(unsigned captured=0;captured<2;captured++) {
        replay_case(model,&g_grouped,captured,deferred);
        replay_case(model,&g_tiled,captured,deferred);
        replay_case(model,&g_fast_decay,captured,deferred);
    }
