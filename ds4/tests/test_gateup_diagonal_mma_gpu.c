/* Native GPU bit comparison for diagonal four-group routed gate/up MMA.
...
enum { CAP = 8, GUARD = 64 };
static const char *disable = "DS4_QWEN4EXP_NO_GATEUP_DIAGONAL_MMA";
...
static void require(int ok, const char *what) {
    if (!ok) { fprintf(stderr, "MoE gate/up diagonal MMA: %s\n", what); exit(1); }
}
...
#undef INVOKE
    ...
    printf("MoE gate/up diagonal MMA gt=%u dt=%u experts=%u offset=%u PASS\n",gt,dt,experts,64+alignment); fflush(stdout);
}
...
int main(void) {
    ...
    printf("MoE gate/up diagonal MMA: %u complete buffers, %u graph checks, %u actual replays PASS\n",
