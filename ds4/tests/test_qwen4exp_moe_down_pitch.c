/* Complete MoE comparison for the down-projection temporary shared pitch, including graph
 * replays, output guards, invalid routes and the shared-scratch growth seam.
 * Synthetic weights only. The input is shortened to 256 while the down
 * projection retains the target 640-to-2560 shape. Build against the normal CUDA library:
 * cc -O2 -std=c11 -D_GNU_SOURCE -Ids4 ds4/tests/test_qwen4exp_moe_down_pitch.c \
 *   -L.build/ds4 -lds4qwen -lm -o /tmp/test-moe-down-pitch
 */
#include "ds4_gpu.h"
#include <stdint.h>
#include <stdio.h>
...
enum { K = 256, D = 640, O = 2560, CAP = 1024, GUARD = 64 };
static const char *disable = "DS4_QWEN4EXP_NO_DOWN_MMA_PADDING";
...
static void require(int ok, const char *what) {
    if (!ok) { fprintf(stderr, "MoE down pitch: %s\n", what); exit(1); }
}
...
int main(void) {
    ...
    printf("MoE down pitch: %u complete buffers, %u graph checks, %u actual replays PASS\n",
