...
launch=re.search(r'mtp_native_screen_staged_warp_kernel<<<([^,]+),([^,]+),0,cuda_decode_stream\(\)>>>',body)
assert launch
body=body.replace(launch.group(0),f'record_grid({launch[1]}, {launch[2]}); mtp_native_screen_staged_warp_kernel')
...
template<class... A>static void mtp_native_screen_staged_warp_kernel(A...){mark("warp");}
