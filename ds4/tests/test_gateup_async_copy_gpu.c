/* Native GPU comparison of cooperative asynchronous gate/up panel delivery with the original
 * global-register-shared copy path (DS4_QWEN4EXP_NO_GATEUP_ASYNC_COPY=1). The down projection is the
 * same in both arms. Compares all outputs/guards and graph replays with new
 * activations/routes at production dimensions and shape/alignment fallbacks.
 * Compile against the normal CUDA library; execution requires a CUDA GPU.
 * Host correctness checks are separate: gateup_async_copy_host.py.
 */
#include "ds4_gpu.h"
#include <stdint.h>
#include <stdio.h>
...
enum { CAP = 8, GUARD = 64 };
static const char *disable = "DS4_QWEN4EXP_NO_GATEUP_ASYNC_COPY";
...
static void require(int ok, const char *what) {
    if (!ok) { fprintf(stderr, "MoE gate/up async copy: %s\n", what); exit(1); }
}
static unsigned block_bytes(unsigned type) {
    return type == 7 ? 24 : type == 8 ? 34 : type == 12 ? 144 :
           type == 13 ? 176 : 210;
}
static unsigned row_bytes(unsigned type, unsigned width) {
    return block_bytes(type) * ((type == 7 || type == 8) ? width / 32 : width / 256);
}
...
static void check_case(unsigned dt, unsigned experts, unsigned alignment,
                       unsigned K, unsigned D, unsigned O) {
    const unsigned gt=12;
    ...
    const size_t db = (size_t)experts * O * row_bytes(dt,D);
    ...
    ds4_gpu_qwen4exp_slab down = {image, image_bytes, d_o, O * row_bytes(dt,D), row_bytes(dt,D), dt};
...
#define POISON(M) do { for(unsigned pj=0;pj<3;pj++) \
    ...
    const unsigned widths[] = {1, 2, 3, 4, CAP};
...
#undef INVOKE
    ...
    printf("MoE gate/up async copy gt=%u dt=%u experts=%u offset=%u PASS\n",gt,dt,experts,64+alignment); fflush(stdout);
}

int main(void) {
    require(setenv("DS4_QWEN4EXP_FORCE_GATEUP_ASYNC_COPY","1",1)==0,"test candidate even without startup tuner");
    require(setenv("DS4_CUDA_COPY_MODEL","1",1) == 0 && setenv("DS4_CUDA_DECODE_GRAPHS","1",1) == 0, "test environment");
    const unsigned types[] = {7,8};
    for (unsigned t = 0; t < 2; t++) {
        check_case(types[t],13,0,2560,640,2560);
        check_case(types[t],13,2,256,640,24); // weight alignment fallback
        for(unsigned D=512;D<=544;D+=32) check_case(types[t],1,0,256,D,24);
        check_case(types[t],17,0,256,1024,24);
        check_case(types[t],17,0,256,1056,24); // multiple groups per lane
    }
    printf("MoE gate/up async copy: %u complete buffers, %u graph checks, %u actual replays PASS\n",
