...
static int mtp_head_forward_impl(ds4_qwen4exp_mtp_head *h,
    ...
    if (ok) {
        ...
        if (device_second) {
            ok = ds4_gpu_mtp_top2_policy_tensor(
                     h->t_top1, h->t_logits, draft_width, 1u, 0u,
    ...
    if (ok && collect_confidence) {
        ...
        if (ok) {
            ...
            if (screened) {
                if (second_id >= draft_width) {
                    ok = false;
                } else {
                    stage = "native second winner mapping";
                    ok = ds4_gpu_tensor_read(
                             h->t_native_ids,
                             (uint64_t)second_id * sizeof(uint32_t),
                             &second_id, sizeof(second_id)) != 0;
                }
