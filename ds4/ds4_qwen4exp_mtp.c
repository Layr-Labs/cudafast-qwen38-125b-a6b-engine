...
static int mtp_head_forward_impl(ds4_qwen4exp_mtp_head *h,
    ...
    if (ok) {
        /* Draft steps always use logit_rows == 1.  Project the prefix straight
         * into t_logits and only D2D-copy the (tiny) tail range behind it —
         * that drops the 393 KB prefix copy on every draft step.  Multi-row
         * diagnostic / verify paths keep the prior prefix staging. */
        if (logit_rows == 1u) {
