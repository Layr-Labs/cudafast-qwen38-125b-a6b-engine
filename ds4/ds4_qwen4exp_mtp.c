...
static int mtp_head_forward_impl(ds4_qwen4exp_mtp_head *h,
    ...
    if (ok) {
        if (logit_rows == 1u) {
            /* One draft row: project the prefix straight into the packed
             * shortlist row and copy only the tail into the slot behind it.
             * The bytes written are the same bytes the staged path would copy
             * into the same addresses, so the pack is elided, not changed. */
            ...
            if (ok && draft_tail) {
                /* Pack each row's two ranges into one contiguous shortlist
                 * row: prefix first, tail behind it.  Sources are other
                 * tensors and the destinations do not overlap, so plain
                 * stream-ordered copies are enough -- no in-place shuffle. */
