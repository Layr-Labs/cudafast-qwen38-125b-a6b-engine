    ...
    if (ok) {
        ...
        for (uint32_t t = 0; ok && t < n_tokens; t++) {
            for (uint32_t s = 0; ok && s < n_hc; s++) {
                const uint64_t dst = ((uint64_t)t * n_hc + s) * 2ull * embd_bytes;
                ok = ds4_gpu_tensor_copy(h->t_ehx, dst,
                                         h->t_e_normed, (uint64_t)t * embd_bytes,
                                         embd_bytes) != 0 &&
                     ds4_gpu_tensor_copy(h->t_ehx, dst + embd_bytes,
                                         h->t_h_normed,
