    ...
    if (ok) {
        if (logit_rows == 1u) {
            ...
            if (ok && draft_tail) {
                ...
                ds4_gpu_tensor *ov_tail = ds4_gpu_tensor_view(
                        h->t_logits, (uint64_t)draft_prefix * f,
                        (uint64_t)draft_tail * f);
                ok = ov_tail && h->hooks.matmul_q8_0(
                        ov_tail, h->target_map, h->target_size,
                        h->output_offset + (uint64_t)(h->n_vocab - draft_tail) *
                            ds4_qwen4exp_q8_0_row_bytes(n_embd),
                        n_embd, draft_tail, h->t_sample, 1u) != 0;
                if (ov_tail) ds4_gpu_tensor_free(ov_tail);
            }
        } else {
            stage = "borrowed lm head";
            ok = h->hooks.matmul_q8_0(
