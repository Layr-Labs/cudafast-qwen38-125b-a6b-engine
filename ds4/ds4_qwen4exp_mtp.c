...
void ds4_qwen4exp_mtp_state_set_dev_hyper(ds4_qwen4exp_mtp_state *st,
    ...
    if (!st) return;
    /* Opt-in: on the organizer pool the device-to-device copy route measured
     * -2.75%/-8.88% (escalating with coverage) while every local box measured
     * it faster and bit-exact, so the transport is not provably safe there.
     * DS4_MTP_DEVICE_MULTI=1 re-enables it for local work. */
    st->dev_hc = (hyper && getenv("DS4_MTP_DEVICE_MULTI") != NULL)
        ? hyper : NULL;
}
