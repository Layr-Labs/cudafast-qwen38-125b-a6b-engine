...
static int mtp_head_forward_impl(ds4_qwen4exp_mtp_head *h,
    ...
    bool screened = false;
    bool asynchronous_policy = false;
    const bool collect_confidence =
        logit_rows == 1u && h->collect_draft_confidence;
    const bool device_second =
        logit_rows == 1u && h->device_second_policy;
    if (ok && logit_rows == 1u && h->t_native_scratch && h->t_native_ids &&
        ...
        asynchronous_policy = device_second && !collect_confidence &&
            h->device_second_map && h->hooks.native_screen_policy &&
            getenv("DS4_MTP_NO_ASYNC_NATIVE_POLICY") == NULL;
        const int rc = asynchronous_policy
            ? h->hooks.native_screen_policy(h->t_top1, h->t_logits,
                h->t_native_ids, h->t_native_scratch,
                h->target_map, h->target_size, h->output_offset,
                n_embd, h->n_vocab, draft_prefix, draft_tail, h->t_sample,
                h->second_margin_threshold, h->second_logit_threshold)
        ...
        if (rc <= 0) asynchronous_policy = false;
    }
...
output_projection:
    ...
    const bool map_in_device_policy =
        device_second && screened && h->device_second_map;
    h->last_draft_margin_valid = false;
    if (ok && !asynchronous_policy) {
        stage = "gpu top-1";
         ...
         * rebased below. */
        if (device_second) {
            ok = ds4_gpu_mtp_top2_policy_tensor(
                     h->t_top1, h->t_logits,
                     map_in_device_policy ? h->t_native_ids : NULL,
                     draft_width, 1u, 0u, h->n_vocab,
                     h->second_margin_threshold,
                     h->second_logit_threshold) != 0;
        } else if (collect_confidence) {
            ok = ds4_gpu_indexer_top2_value_tensor(
                     h->t_top1, h->t_top_values, h->t_logits,
                     draft_width, 1u, 0u) != 0;
        } else {
    ...
    if (ok && screened && !map_in_device_policy && !asynchronous_policy) {
    ...
    if (ok) {
        ...
        ok = ds4_gpu_tensor_read(h->t_top1,
                                 ...
                                 h->top1_host,
                                 (uint64_t)(collect_confidence ? 2u : out_rows) *
                                     sizeof(uint32_t)) != 0;
    }
    if (ok && asynchronous_policy &&
        h->top1_host[0] == DS4_MTP_NATIVE_SCREEN_FALLBACK) {
        /* Only a nonfinite coarse score requests this fallback. The chosen
         * sentinel differs from invalid-ID UINT32_MAX. Keep the already
         * computed t_sample and t_hyper: replaying the head would append its
         * QSA/MoE state twice. Restore the full static range width and rerun
         * only its stateless projection/reduction, including the NaN0 rule. */
        screened = false;
        asynchronous_policy = false;
        draft_width = draft_prefix ? draft_prefix + draft_tail : h->n_vocab;
        goto output_projection;
    }
    if (ok && collect_confidence) {
