...
#define DS4_QWEN4EXP_GDN_DEFER_FLUSH_ROWS 1u

/* A four-slot input ring. At most two committed transitions follow the
 * checkpoint. A nonempty prefix publishes a new checkpoint after row zero
 * and records only row one's input, limiting repeated recurrence work. */
...
static inline uint32_t ds4_qwen4exp_gdn_deferred_after(
    ...
    if (prefix >= DS4_QWEN4EXP_GDN_DEFER_FLUSH_ROWS) {
