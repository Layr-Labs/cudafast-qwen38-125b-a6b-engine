...
int ds4_gpu_mtp_native_screen(ds4_gpu_tensor *out, ds4_gpu_tensor *ids,
    ...
    const ds4_gpu_tensor *x);
/* Async screen plus exact top-2 policy/map. The screen's nonfinite flag is
 * consumed on device, not read between coarse projection and sorting.
 * Positive return means a candidate-count result is queued; selected becomes
 * a vocabulary id, or FALLBACK when the caller must rerun only the stateless
 * full-static output projection. UINT32_MAX remains an invalid-ID error.
 * This entry declines vocabularies large enough to contain the sentinel. */
#define DS4_MTP_NATIVE_SCREEN_FALLBACK (UINT32_MAX - 1u)
int ds4_gpu_mtp_native_screen_policy(ds4_gpu_tensor *selected,
    ds4_gpu_tensor *out, ds4_gpu_tensor *ids, ds4_gpu_tensor *scratch,
    const void *map, uint64_t bytes, uint64_t offset,
    uint32_t dim, uint32_t vocab, uint32_t prefix, uint32_t tail,
    const ds4_gpu_tensor *x, float margin_threshold, float second_logit_threshold);
...
/* Float-input BF16 matvec arithmetic on a single prefill row grid. */
