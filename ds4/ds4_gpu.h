...
/* Pin a host range so D2H/H2D copies take the fast path instead of the
 * driver's pageable staging.  Returns nonzero on success; callers treat
 * failure as "stay pageable", never fatal. */
int ds4_gpu_host_register(void *ptr, uint64_t bytes);
int ds4_gpu_host_unregister(void *ptr);
