...
/* The attention above with the QSA output gate folded into its store: `out`
 * holds `attn * sigmoid(gate)` on return, byte for byte what the separate
 * attention + ds4_gpu_qwen4exp_qsa_output_gate_tensor chain produced.  The
 * CUDA backend computes it in one launch; the DS4_QWEN4EXP_NO_QSA_GATE_FUSE
 * valve runs the two-launch chain. */
int ds4_gpu_qwen4exp_qsa_attention_gate_dpos_tensor(
        ds4_gpu_tensor       *out,
        const ds4_gpu_tensor *q,
        const ds4_gpu_tensor *k_cache,
        const ds4_gpu_tensor *v_cache,
...
/* The mixer above with a PENDING inject apply in front of it: the apply
 * ds4_gpu_qwen4exp_hc_inject_tensor performs (hyper += block_out * inject,
 * in place), then the mixer over the updated stream.  The CUDA backend folds
 * the apply into the mixer's first kernel, which stores the apply's bytes
 * into `hyper` exactly as the separate kernel did; every declined shape,
 * other backend and the DS4_QWEN4EXP_NO_HC_INJECT_FUSE valve run the two
 * steps as two launches. */
int ds4_gpu_qwen4exp_hc_mixer_apply_tensor(
        ds4_gpu_tensor       *mixed,
        ds4_gpu_tensor       *inject,
        ds4_gpu_tensor       *normed_scratch,
        ds4_gpu_tensor       *lowrank_scratch,
