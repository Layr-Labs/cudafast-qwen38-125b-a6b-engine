...
/* Writes selected[2*t+{0,1}] and values[2*t+{0,1}] with the two highest-scoring
 * entries of row t, ordered by (value desc, index asc).  The reduction is a pure
 * comparison over that total order, so the result does not depend on traversal
 * order and matches a serial strict-greater host scan exactly. */
