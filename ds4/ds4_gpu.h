...
/* Pinned staging for the per-token uploads.  A synchronous pageable
 * cudaMemcpy stages through a driver bounce buffer and serializes on the
 * legacy stream; a pinned source on the decode stream is a true async DMA
 * that overlaps the launch train that follows it.  ds4_gpu_host_pin returns
 * nonzero when the range registered; callers keep the synchronous write as
 * the fallback, so a backend without a decode stream loses nothing. */
int  ds4_gpu_host_pin(void *ptr, uint64_t bytes);
void ds4_gpu_host_unpin(void *ptr);
