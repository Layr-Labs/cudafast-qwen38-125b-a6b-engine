...
typedef struct {
    ...
    /* 0: the shipped bounded log (control = prefix <= ROWS, tape of ROWS
     * rows, `recurrent_state` written every verify).  N in [1, 255]: deferred
     * materialisation (ds4_qwen4exp_gdn_defer_plan): control = prefix |
     * parity << 8, tape of 2 * N rows, `recurrent_state` never written. */
    uint32_t defer_rows;
...
int ds4_gpu_qwen4exp_gdn_replay_materialize(
        ...
        uint32_t head_layout);
/* state = checkpoint . tape rows [first_row, first_row + rows) of a deferred
 * 2 * defer_rows row tape; first_row is 0 or defer_rows, rows <= defer_rows. */
int ds4_gpu_qwen4exp_gdn_defer_materialize(
        ds4_gpu_tensor *state, ds4_gpu_tensor *checkpoint, ds4_gpu_tensor *tape,
        uint32_t defer_rows, uint32_t first_row, uint32_t rows,
        uint32_t n_key_head, uint32_t n_value_head, uint32_t head_layout);
