...
static const char *mtp_partial_tune(void) {
    ...
    double ratio[4]={}, mean_a=0,mean_b=0,worst_sd=0;
    ...
    snprintf(report,sizeof report,"partSel[w=%u a_us=%.4g b_us=%.4g r=%.3g,%.3g,%.3g,%.3g sd=%.3g win=%u use=%u]",
        width,mean_a,mean_b,ratio[0],ratio[1],ratio[2],ratio[3],worst_sd,least_wins,(unsigned)prefer);
    return report;
}
