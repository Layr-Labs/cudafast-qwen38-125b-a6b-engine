...
static const char *mtp_screen_warp_tune(void) {
    ...
    do {
        ...
        auto launch = [&](bool warp) -> bool {
            ...
            if (warp) {
                mtp_native_screen_warp_kernel<<<(width + 3u) / 4u, 128, 0, stream>>>(
        ...
        for (unsigned pattern = 0; pattern < 4u && ok; pattern++) {
            ...
            for (unsigned pair = 0; pair < 7u && ok; pair++) {
                for (unsigned leg = 0; leg < 2u && ok; leg++) {
                    ...
                    for (unsigned repeat = 0; repeat < 16u && ok; repeat++) ok = launch(warp);
                    ...
                    (warp ? b : a)[pattern][pair] = (double)ms * 1000.0 / 16.0;
                }
            }
        }
        if (!ok) break;
