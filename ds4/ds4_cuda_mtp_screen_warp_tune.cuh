...
static const char *mtp_screen_warp_tune(void) {
    static char report[192] = "scrWarp[unmeasured]";
    static bool tried = false;
    if (tried) return report;
    tried = true;
    if (getenv("DS4_MTP_NO_SCREEN_WARP") || getenv("DS4_MTP_NO_SCREEN_WARP_TUNE") ||
        getenv("DS4_MTP_NO_FUSED_SCREEN_KEYS") || getenv("DS4_QWEN4EXP_NO_ROW_TILE") ||
        getenv("DS4_QWEN4EXP_PAIR_LANES_R2")) {
        snprintf(report, sizeof report, "scrWarp[disabled]"); return report;
    }
    ...
    if (MTP_NATIVE_SCREEN_GROUPS != 24u || MTP_NATIVE_DIM != 2560u || !cuda_q8_use_dp4a() ||
    ...
    constexpr unsigned pairs = 15u, repeats = 32u, required_wins = 13u;
    double a[4][pairs] = {}, b[4][pairs] = {};
    do {
        ...
        auto launch = [&](bool warp) -> bool {
            if (cudaMemsetAsync(flag, 0, 4u, stream) != cudaSuccess) return false;
            if (warp) {
                mtp_native_screen_staged_warp_kernel<<<(width + 3u) / 4u, 128, 0, stream>>>(
         ...
         * width. Four input patterns, fifteen AB/BA pairs each, thirty-two complete
        ...
        for (unsigned pattern = 0; pattern < 4u && ok; pattern++) {
            ...
            for (unsigned pair = 0; pair < pairs && ok; pair++) {
                for (unsigned leg = 0; leg < 2u && ok; leg++) {
                    const bool warp = ((pair + leg) & 1u) != 0;
                    if (cudaEventRecord(begin, stream) != cudaSuccess) {ok = false; break;}
                    for (unsigned repeat = 0; repeat < repeats && ok; repeat++) ok = launch(warp);
                    ...
                    (warp ? b : a)[pattern][pair] = (double)ms * 1000.0 / repeats;
                }
            }
        }
        if (!ok) break;
    ...
    if (!success) {
        ...
        snprintf(report, sizeof report, "scrWarp[failed=%s]", failure); return report;
    }
    double ratios[4] = {}, sum_a = 0.0, sum_b = 0.0, max_upper = 0.0;
    unsigned min_wins = pairs;
    bool prefer = true;
    for (unsigned pattern = 0; pattern < 4u; pattern++) {
        ...
        mean /= pairs;
        for (unsigned i = 0; i < pairs; i++) {const double delta = b[pattern][i] / a[pattern][i] - mean; variance += delta * delta;}
        const double upper = mean + 3.0 * sqrt(variance / (pairs - 1u)) / sqrt((double)pairs);
        ratios[pattern] = sb / sa;
        prefer = prefer && ratios[pattern] < 0.98 && wins >= required_wins && upper < 0.98;
        if (upper > max_upper) max_upper = upper;
        if (wins < min_wins) min_wins = wins;
        sum_a += sa; sum_b += sb;
    }
    mtp_screen_tuned_device = device; mtp_screen_tuned_width = width; mtp_screen_prefer_warp = prefer;
    snprintf(report, sizeof report,
             "scrWarp[w=%u a_us=%.4g b_us=%.4g r=%.3g,%.3g,%.3g,%.3g upper=%.3g win=%u use=%u]",
             width, sum_a / (4u * pairs), sum_b / (4u * pairs), ratios[0], ratios[1], ratios[2], ratios[3], max_upper, min_wins, (unsigned)prefer);
    return report;
}
