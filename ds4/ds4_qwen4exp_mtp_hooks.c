...
#if !defined(__APPLE__) && (defined(__GNUC__) || defined(__clang__))
/* Head-owned allocations are immutable until head_free. Generic in-place
 * retargeting/recycling must explicitly release this state before mutation. */
typedef struct {
    const void *head_map, *target_map;
    ds4_gpu_tensor *tensor[7];
    uint64_t bytes[7], head_size, target_size, embed, enorm, hnorm, proj;
    uint32_t embd, hc, vocab, type, max_tokens, block;
    float eps, bias;
    int round;
} mtp_stem_binding;

static void mtp_stem_graph_release(ds4_qwen4exp_mtp_head *h) {
    if (h->stem_graph_state) {
        (void)ds4_gpu_synchronize();
        ds4_gpu_decode_graphs_invalidate();
        free(h->stem_graph_state);
    }
    h->stem_graph_state = NULL;
    h->stem_graph_release = NULL;
}

static int mtp_stem(ds4_qwen4exp_mtp_head *h, uint32_t rows) {
    /* H2D uploads precede this hook. The transformer/cache block follows it.
     * Default identities keep custom hook side effects outside capture. */
    if (!h || h->hooks.embed != ds4_gpu_qwen4exp_embed_tokens_hc_tensor ||
        h->hooks.rms_norm != ds4_gpu_qwen4exp_rms_norm_tensor ||
        h->hooks.ehx_pack != ds4_gpu_qwen4exp_ehx_pack_tensor ||
        !h->hooks.ehx_pack || h->hooks.matmul_q8_0 != mtp_matmul_q8_0_decode_rows ||
        h->block_index != 48u || rows < 1u || rows > 2u ||
        rows > h->max_tokens || h->n_embd != 2560u || h->n_hc != 4u ||
        h->token_embd_type != 8u ||
        getenv("DS4_MTP_NO_STEM_GRAPH") != NULL ||
        getenv("DS4_QWEN4EXP_NO_ROW_TILE") != NULL ||
        getenv("DS4_CUDA_NO_Q8_DP4A") != NULL ||
        getenv("DS4_CUDA_NO_Q8_MMA") != NULL ||
        !ds4_gpu_decode_graphs_supported())
        return ds4_qwen4exp_mtp_head_stem_eager(h, rows);

    mtp_stem_binding binding;
    memset(&binding, 0, sizeof(binding));
    binding.head_map = h->head_map; binding.head_size = h->head_size;
    binding.target_map = h->target_map; binding.target_size = h->target_size;
    binding.embed = h->token_embd_offset; binding.enorm = h->enorm_offset;
    binding.hnorm = h->hnorm_offset; binding.proj = h->eh_proj_offset;
    binding.embd = h->n_embd; binding.hc = h->n_hc;
    binding.vocab = h->n_vocab; binding.type = h->token_embd_type;
    binding.max_tokens = h->max_tokens; binding.block = h->block_index;
    binding.eps = h->rms_eps; binding.bias = h->weight_bias;
    binding.round = h->round_bf16;
    ds4_gpu_tensor *tensor[7] = {h->t_tokens, h->t_embed_rows, h->t_embed_out,
        h->t_e_normed, h->t_h_normed, h->t_ehx, h->t_hyper};
    for (unsigned i = 0; i < 7u; ++i) {
        binding.tensor[i] = tensor[i];
        binding.bytes[i] = ds4_gpu_tensor_bytes(tensor[i]);
    }
    if (h->stem_graph_state &&
        memcmp(h->stem_graph_state, &binding, sizeof(binding)) != 0) {
        if (!ds4_gpu_synchronize()) return 0;
        ds4_gpu_decode_graphs_invalidate();
        memcpy(h->stem_graph_state, &binding, sizeof(binding));
    } else if (!h->stem_graph_state) {
        h->stem_graph_state = malloc(sizeof(binding));
        if (!h->stem_graph_state)
            return ds4_qwen4exp_mtp_head_stem_eager(h, rows);
        memcpy(h->stem_graph_state, &binding, sizeof(binding));
        h->stem_graph_release = mtp_stem_graph_release;
    }
    ds4_decode_graph_key key;
    memset(&key, 0, sizeof(key));
    key.il = 48u; key.island = 0u; key.variant = rows;
    key._pad = 0x4d545053u; /* MTPS: separate from target/head block/tail. */
    key.cur_hc = h; key.after_attn_hc = h->stem_graph_state;
    key.after_ffn_hc = h->t_hyper; key.attn_norm = h->t_ehx;
    const int mode = ds4_gpu_decode_graph_begin(&key);
    if (mode == 1) return 1;
    const int ok = ds4_qwen4exp_mtp_head_stem_eager(h, rows);
    if (mode != 0) return ok;
    if (!ok) {
        ds4_gpu_decode_graph_abort(&key);
        return 0;
