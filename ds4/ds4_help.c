...
static void print_sampling(FILE *fp, const help_colors *c, bool full) {
    ...
    if (full) {
        ...
        opt(fp, c, "--prompt-tokens N", "Send only the first N prompt tokens, as a golden case does.");
    }
    fputc('\n', fp);
}
...
static void print_cli_diagnostics(FILE *fp, const help_colors *c) {
    ...
    opt(fp, c, "--dump-tokens", "Print the exact CLI prompt token stream, then exit. Use --raw-prompt for literal text.");
