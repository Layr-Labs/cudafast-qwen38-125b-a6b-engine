...
static int run_sampled_generation(ds4_engine *engine, const cli_config *cfg, const ds4_tokens *prompt) {
    ...
    while (generated < max_tokens && !cli_interrupt_requested()) {
        ...
        if (ds4_token_is_stop_for_think_mode(engine, token, think_mode)) {
            /* NAME THE TOKEN THAT STOPPED IT.  A stop on the very first
             * sample is reported as "generation: 0.00 t/s" and nothing else,
             * which reads like a broken decode rather than a model ending the
             * turn.  That cost a boot investigation a day: this leg printed
             * nothing while the same weights through the protocol adapter
             * printed coherent text, because the adapter passes eos_token -1
             * and never stops at all (ds4_shim.c) while the CLI stops on the
             * model's own end of text. */
            if (generated == 0) {
                size_t stop_len = 0;
                char *stop_text = ds4_token_text(engine, token, &stop_len);
                ds4_log(stderr, DS4_LOG_WARNING,
                        "ds4: the first sampled token is a stop token (id %d, "
                        "\"%.*s\"), so nothing was generated\n",
                        token, (int)stop_len, stop_text ? stop_text : "");
                free(stop_text);
            }
            break;
        }
    ...
    /* Say what the rates are made of.
     *
     * A rate alone cannot be read.  "generation: 0.00 t/s" is what this line
     * prints when the decode window is empty AND when the window ran but no
     * token came out of it, and those are different failures: the first is a
     * broken timer, the second is a model that answered a 1024-token prompt
     * with a stop token.  The gate driver reads this line for its decode_tok_s
     * and reported the second as the first for a whole afternoon.  The counts
     * and the windows settle it, and the leading "N t/s" text is unchanged so
     * the driver's parse still matches. */
    ds4_log(stderr,
            DS4_LOG_TIMING,
            "ds4: prefill: %.2f t/s, generation: %.2f t/s "
            "(%d prompt tokens in %.3f s, %d generated in %.3f s)\n",
            prefill_s > 0.0 ? (double)prompt->len / prefill_s : 0.0,
            decode_s > 0.0 ? (double)generated / decode_s : 0.0,
            (int)prompt->len, prefill_s, generated, decode_s);
...
static int run_generation(ds4_engine *engine, const cli_config *cfg) {
    ...
    /* --prompt-tokens N sends the first N tokens and drops the rest.
     *
     * A golden case is authored over an exact token count -- the qwen4exp
     * correctness goldens all carry 1024 prompt tokens, which is a PREFIX of
     * the prompt file and ends part way through a word.  Without this the CLI
     * sends the whole file, the harness sends the prefix, and the two are
     * compared on different text: the prefix ends mid-sentence and has to be
     * continued, the whole file ends on a finished instruction and the model
     * ends the turn.  That is not a decode difference, and no amount of
     * looking at the decoder finds it. */
    if (cfg->gen.prompt_tokens > 0 && prompt.len > cfg->gen.prompt_tokens) {
        ds4_log(stderr, DS4_LOG_PREFILL,
                "ds4: prompt truncated to the first %d of %d tokens\n",
                cfg->gen.prompt_tokens, prompt.len);
        prompt.len = cfg->gen.prompt_tokens;
    }
...
static int run_chat_turn(ds4_engine *engine, cli_config *cfg, repl_chat *chat,
    ...
    ds4_log(stderr,
            ...
            decode_s > 0.0 ? (double)generated / decode_s : 0.0,
            suffix, prefill_s, generated, decode_s);
    return 0;
}
...
static cli_config parse_options(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (!strcmp(arg, "-p") || !strcmp(arg, "--prompt")) {
            ...
            c.gen.prompt_tokens = parse_int(need_arg(&i, argc, argv, arg), arg);
