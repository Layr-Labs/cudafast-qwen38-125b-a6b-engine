...
/* ROCm keeps the single-fd route; a split set on gfx1151 is not a shape this
 * build serves, and the fd route falls back to the mapping when it cannot
 * name the file. */
extern "C" int ds4_gpu_set_model_shard_fd(int fd, const void *model_map) {
    (void)fd;
    (void)model_map;
    return 1;
}
