...
// ---------------------------------------------------------------------------
// Row-tiled twins of the two matvecs the Qwen4-Exp tower runs at prefill.
//
// The kernels above give one threadgroup to one activation row, so a weight row
// crosses the memory system once per TOKEN.  A prefill of 512 rows therefore
// reads the whole active weight set 512 times.  The twins below give one
// threadgroup a TILE of activation rows and read the weight once for the tile.
//
// Nothing about a row's arithmetic changes.  Each row keeps the same lane walk
// over the same blocks in the same order, the same float accumulator, the same
// multiply and add, and the same two-stage simd_sum reduction, so a row in a
// tile produces the bits it produces alone.  The weight is simply held in
// registers across the tile instead of being fetched again: `xw` and `d` for
// Q8_0, `xl4` for F32.  The activation is re-read per row, which is cheap --
// it is kilobytes against the megabytes of a weight row.
//
// A tile that runs off the end of the batch writes only its real rows; the
// bound is uniform across the threadgroup, so every lane still reaches every
// barrier and every simd_sum.

#define DS4_QWEN4EXP_MV_TILE 8

template<short NR0, short NT>
static inline void helper_mv_reduce_and_write_tile(
        device float * dst,
        const uint64_t dst_row_stride,
        const int r1_base,
        const int n_rows,
        thread float (&sumf)[NR0][NT],
        const int r0,
        const int ne01,
        ushort tiisg,
        ushort sgitg,
    ...
    threadgroup float * sh[NR0*NT];

    for (short row = 0; row < NR0; ++row) {
        for (short t = 0; t < NT; ++t) {
            const short s = row*NT + t;
            sh[s] = (threadgroup float *) shmem + NW*s;
    ...
    for (short row = 0; row < NR0 && r0 + row < ne01; ++row) {
        for (short t = 0; t < NT; ++t) {
            const float tot = simd_sum(sh[row*NT + t][tiisg]);

            if (tiisg == 0 && sgitg == 0 && r1_base + t < n_rows) {
                dst[(uint64_t)(r1_base + t)*dst_row_stride + r0 + row] = tot;
            }
        }
    }
}

// Q8_0, tiled over activation rows.  Compare kernel_mul_mv_q8_0_f32_impl: the
// lane's block walk (ib0 = sgitg*NQ + ix, ib += NSG*NQ), the eight-element
// partial `sumq`, and `sumf += sumq*d` are the same expressions on the same
// operands.
[[host_name("kernel_qwen4exp_mul_mv_q8_0_f32_tile")]]
kernel void kernel_qwen4exp_mul_mv_q8_0_f32_tile(
        constant ds4_metal_args_mul_mv & args,
        device const char * src0,
        device const char * src1,
        device       char * dst,
    ...
    constexpr short NT  = DS4_QWEN4EXP_MV_TILE;

    const int nb = args.ne00/QK8_0;

    const int r0 = tgpig.x*NR0;
    const int r1_base = tgpig.y*NT;
    ...
    for (int ib = ib0; ib < nb; ib += NSG*NQ) {
        for (short row = 0; row < NR0; row++) {
            ...
            for (short t = 0; t < NT; ++t) {
                if (r1_base + t >= args.ne1) {
                    break;
                }
                device const float * yb =
                    (device const float *) (src1 + (r1_base + t)*args.nb11) +
                    ib*QK8_0 + il*NQ;
    ...
    helper_mv_reduce_and_write_tile<NR0, NT>(
            (device float *) dst, (uint64_t) args.ne0, r1_base, args.ne1,
            sumf, r0, args.ne01, tiisg, sgitg, shmem);
}

// F32 weights, tiled over activation rows.  Compare
// kernel_mul_mv_t_t_4_impl<float, float4, float, float4>: the same NF-element
// stripe per lane, the same four `dot(float4, float4)` partials, the same
// scalar tail past the last whole block.
template<short NR0>
void kernel_qwen4exp_mul_mv_f32_f32_4_tile_impl(
        constant ds4_metal_args_mul_mv & args,
        device const char * src0,
        device const char * src1,
        device       char * dst,
    ...
    FOR_UNROLL (short row = 0; row < NR0; ++row) {
        ax [row] = (device const float  *) ((device char *) src0 + (r0 + row)*args.nb01);
        ax4[row] = (device const float4 *) ((device char *) src0 + (r0 + row)*args.nb01);
    }
        ...
        for (short row = 0; row < NR0; row++) {
            ...
            for (short t = 0; t < NT; ++t) {
                ...
                device const float4 * yb4 =
                    (device const float4 *) (src1 + (r1_base + t)*args.nb11) +
                    (ib*NB + il*NF)/4;

                float sumq = 0.f;
                FOR_UNROLL (short i = 0; i < NF4; ++i) {
                    sumq += dot(float4(xl4[i]), float4(yb4[i]));
                }
...
[[host_name("kernel_qwen4exp_mul_mv_f32_f32_4_tile")]]
kernel void kernel_qwen4exp_mul_mv_f32_f32_4_tile(
        constant ds4_metal_args_mul_mv & args,
        device const char * src0,
        device const char * src1,
        device       char * dst,
        ...
        ushort sgitg[[simdgroup_index_in_threadgroup]]) {
    switch (args.nr0) {
        case 2: kernel_qwen4exp_mul_mv_f32_f32_4_tile_impl<2>(args, src0, src1, dst, shmem, tgpig, tiisg, sgitg); break;
        case 4: kernel_qwen4exp_mul_mv_f32_f32_4_tile_impl<4>(args, src0, src1, dst, shmem, tgpig, tiisg, sgitg); break;
    };
}
