...
static inline void ds4_ple_row_ids_impl(const ds4_ple_constants *__restrict c, ds4_ple_history *h,
    ...
    /* Both of these are fixed for the whole call: the first multiplier and the
     * history ring. Hold them in registers rather than reloading through `c`
     * and `h` on every token. Same loads, same values. */
    const uint64_t mult0 = mult[0];
    const int32_t *__restrict previous = h->previous;
    ...
    for (size_t t = 0; t < count; t++) {
        ...
        uint64_t mixed = (uint64_t)(uint32_t)cur * mult0;

        for (uint32_t n = 2; n <= ngram; n++) {
            mixed ^= (uint64_t)(uint32_t)previous[n - 1] * mult[n - 1];
