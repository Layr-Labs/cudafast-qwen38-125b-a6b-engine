...
typedef struct {
    ...
    /* Optional asynchronous depth-1 screen and policy. A nonfinite coarse
     * value writes DS4_MTP_NATIVE_SCREEN_FALLBACK to the selected-id tensor;
     * the head then repeats only its stateless LM projection/reduction. */
    int (*native_screen_policy)(ds4_gpu_tensor *, ds4_gpu_tensor *,
                                ds4_gpu_tensor *, ds4_gpu_tensor *, const void *,
