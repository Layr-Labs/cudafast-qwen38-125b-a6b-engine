...
enum {
     ...
     * pure schedule knob.
     *
     * The gradient is MEASURED and steep.  The whole reason this kernel exists
     * is that carrying four rows per warp beat qwen4exp_gdn_recurrence_kernel's
     * one, and four -> eight then measured 4.91% faster on the prefill leg
     * (d840e029, spark-2: normalized prefill 0.324217 against the promoted
     * tree's 0.340950, with max_abs_diff 0).  Each doubling halves the redundant
     * per-token q, k and gate traffic -- a head's 128 value rows are split
     * across QWEN4EXP_GDN_DIM / (4 * this) blocks, and every one of them reads
     * the same q4 and k4 -- and doubles the independent warp-reduction chains,
     * which is the only lever a 1024-step serial loop has for latency, since
     * block count cannot supply instruction-level parallelism.
     *
     * The cost is resident blocks and carried registers: 96 blocks over 48 SMs
     * at sixteen rows (8 warps an SM) against 192 at eight and 384 at four, and
     * 64 carried floats a lane against 32 and 16.  Sixteen is the last doubling
     * that keeps at least two blocks an SM; thirty-two would leave 48 blocks,
     * one per SM, with no slack for imbalance. */
    QWEN4EXP_GDN_VALUE_ROWS = 16
};
