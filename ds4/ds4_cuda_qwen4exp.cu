 ...
 * lane holds instead of on four lanes.  Explicit round-to-nearest adds keep
 * nvcc from contracting a dot product into the fold. */
__device__ __forceinline__ static float qwen4exp_gdn_fold4(
        float d0, float d1, float d2, float d3) {
    return __fadd_rn(__fadd_rn(d0, d2), __fadd_rn(d1, d3));
}

/* The three outer levels stay inside an eight-lane value-row group. */
...
/* Four value rows per warp, one per eight-lane group.  Each lane owns the
 * columns of four lanes from the original warp-wide schedule, so fold4 plus
 * the three-step group butterfly preserves the exact reduction tree while
 * performing the four row reductions concurrently.  Prefill only. */
