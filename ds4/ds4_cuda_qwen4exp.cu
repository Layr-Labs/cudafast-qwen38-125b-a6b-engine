...
#pragma unroll
     ...
     * makes for its own chunk stream, where an L2 prefetch three chunks ahead
     * of the cp.async is worth real time.  This asks the L2 for the panel that
     * step k+3 will copy while step k+1's cp.async is in flight, so each panel
     * is requested exactly once and three steps before it is needed.
     *
...
#pragma unroll
    ...
    if (Stage) {
        ...
        /* Step 1's fill is issued inside step 0's body; asking for it here
         * gives it a head start.  Both reads are of `down` and of `route`,
        ...
        for (int r = 0; r < R; r++) {
            if ((uint32_t)r < take) {
                ...
                if (Stage) {
                    ...
                    /* Panels 1 and 2 were asked for in the prologue, so
                     * issuing only step+3 here asks for each panel exactly
                     * once, three steps before the step that copies it. */
                    qw_l2_ahead(step + 3u);
                }
