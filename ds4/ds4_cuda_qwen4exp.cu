...
__global__ static void qwen4exp_qsa_indexer_select_kernel(
    ...
    if (rank_sort) {
        /* ONE-PASS RANK SORT.  The bitonic network below spends
         * log2(w) * (log2(w) + 1) / 2 block barriers on a w-entry row
         * (21 at w = 64) on every full-attention layer of every decode
         * round.  The ascending order of a multiset of integers is unique,
         * so ANY correct sort leaves the same row: entry i goes to
         * rank(i) = #{j : ids[j] < ids[i]} + #{j < i : ids[j] == ids[i]},
         * a permutation of 0..w-1 (equal values -- the sentinels -- keep
         * their relative order, and are identical anyway).  Every thread
         * reads the same ids[j] at the same time, a shared-memory
         * broadcast; the result is written to the second half of the
         * buffer and read back after one barrier.  Integer compares only.
         * DS4_QWEN4EXP_IDX_BITONIC=1 restores the network. */
        int32_t *sorted = ids + sort_width;
        for (uint32_t i = tid; i < sort_width; i += nth) {
            const int32_t v = ids[i];
            uint32_t rank = 0;
            for (uint32_t j = 0; j < sort_width; j++) {
                const int32_t u = ids[j];
                rank += (u < v || (u == v && j < i)) ? 1u : 0u;
            }
            sorted[rank] = v;
        }
        __syncthreads();
...
extern "C" int ds4_gpu_qwen4exp_qsa_indexer_select_tensor(
    ...
    /* The rank sort writes its result to a second row of the same width. */
    const uint32_t rank_sort =
        getenv("DS4_QWEN4EXP_IDX_BITONIC") == NULL ? 1u : 0u;
    qwen4exp_qsa_indexer_select_kernel<<<n_tokens, nth,
        (size_t)(rank_sort ? 2u : 1u) * sort_width * sizeof(int32_t),
        cuda_decode_stream()>>>(
