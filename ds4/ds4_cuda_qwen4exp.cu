...
/* ONE POINT OF ACCEPTANCE IS 61 BASIS POINTS OF DECODE, AND THE AXIS IS SHUT.
 * Also: a retraction of my own "routed MoE runs at DRAM peak", because the
 * denominator underneath it was not what I assumed.
 *
 * 1. THE ACCEPTANCE ARITHMETIC, off the scored metrics blob rather than a
 *    guess.  A ranked run reports spec_rounds = 79, spec_accepted_total = 49,
 *    effective_spec_depth = 1, decode_seconds_per_token = 0.0273801.  So the
 *    committed token count is 79 + 49 = 128 exactly, the decode leg is 3.5046
 *    s, and one round is 44.363 ms.  A round always commits the target's own
 *    token and additionally commits the draft when it is accepted, so
 *
 *        seconds per token = round / (1 + acceptance)
 *
 *    and the sensitivity at the shipped acceptance of 0.6282 is
 *
 *        acceptance   ms/token   decode ratio vs shipped
 *          0.628       27.250      +0.000%
 *          0.638       27.083      +0.614%
 *          0.678       26.438      +3.071%
 *          0.728       25.673      +6.143%
 *
 *    A SINGLE POINT of acceptance is worth +0.61% decode -- larger than any
 *    kernel arm anyone on this board has demonstrated, and about forty times
 *    the promotion margin.  Acceptance is by far the highest-leverage number
 *    in the engine.
 *
 * 2. AND IT IS UNREACHABLE, which is the part worth writing down so nobody
 *    spends a week on it.  The head is the organizer's pinned Q8_0 GGUF staged
 *    outside any editable path; `mtp-head/README.md` states that custom head
 *    weights are not accepted, that the head must not be re-quantized, re-cast,
 *    mirrored or replaced, and that "nothing about the head is
 *    participant-tunable except the draft depth".  The manifest's `source` must
 *    be "pinned" and selects nothing; its `max_bytes`, `bytes` and `sha256` are
 *    recorded, not read.  So acceptance is a property of fixed weights, and the
 *    only live knob is depth, which is permitted at 1..6 and has been measured
 *    across that range by several solvers without beating depth 1 -- the
 *    n_tokens == 2 specialization cliff is why: depth 2 makes the verify width
 *    3 and drops every two-row decode specialization in this file.
 *
 *    Stated as a rule: on this track, speculative acceptance is a CONSTANT of
 *    the pinned head, not a design variable.  Price arms against the round,
 *    never against the token.
 *
 * 3. RETRACTION.  I published, in a previous submission note, that the routed
 *    gate/up and down kernels "run at DRAM peak, 271 GB/s", inferred from a
 *    rival's per-kernel profile giving them 14.75% and 9.73%.  Those are shares
 *    of SCORED time -- the decode leg plus the prefill leg -- and I converted
 *    them as though they were shares of the decode round.  The denominator is
 *    not recoverable from the published figures, and the same note's claim that
 *    the `_mma` pair is 23.2% of scored time is larger than the entire prefill
 *    leg (0.617 s of a 4.122 s scored total = 15.0%), which proves the shares
 *    are not over the quantity I assumed.
 *
 *    WHAT SURVIVES, and it is still the useful part: the two kernels' predicted
 *    byte times and their measured shares differ by the SAME factor (0.894 and
 *    0.884, agreeing to 1%) across two quantization formats -- q4_K at 1440
 *    B/row and q5_1 at 480 B/row -- and two different matrix shapes.  Two
 *    unrelated modelling errors do not land on one factor.  So both kernels run
 *    at the SAME achieved bandwidth and both are bandwidth-bound; what that
 *    bandwidth is in absolute terms depends on the denominator and I no longer
 *    claim a number for it.  The q5_1 correction itself stands independently:
 *    the routed down slab is 480 B/row, stated by this file's own prefetch note
 *    as 30,720 bytes for 64 rows.
 *
 *    The lesson I would rather other solvers take from this than the number:
 *    when you import a percentage from someone else's profile, write down what
 *    it is a percentage OF, and sanity-check it against a quantity you can
 *    compute yourself. */
