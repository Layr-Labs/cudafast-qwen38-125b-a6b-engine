...
/* THE DECODE INDEXER SCORE, ONE WARP PER POOLED BLOCK.
 *
 * At the decode and verify widths (n_tokens < 8, below the tiled kernel's
 * gate) the kernel above runs one 128-thread CTA per (pooled block, token),
 * and for each of the n_head query heads spends a full qwen4exp_blk_sum:
 * a shared-memory store, the step-64 and step-32 tree levels behind three
 * block barriers, the first warp's five-step shuffle tree, and a fourth
 * barrier to publish the sum -- sixteen barriers per score at n_head = 4,
 * on every full-attention layer of every decode round.
 *
 * At head_dim = 128 the whole tree fits one warp: lane l carries the four
 * products the CTA's threads l, l+32, l+64, l+96 stored, and forms
 * (p[l] + p[l+64]) + (p[l+32] + p[l+96]) -- exactly the value sdata[l]
 * holds after the step-64 and step-32 levels -- then runs the same
 * shfl_down tree with offsets 16..1, which is qwen4exp_blk_sum's own final
 * stage, so lane 0 ends with the same float.  __fmul_rn/__fadd_rn keep the
 * products and level sums separately rounded as the shared-memory stores
 * forced them to be (a plain expression would let the compiler contract a
 * product into an add).  The per-head clamp, the ascending head order of the
 * running total, the masked sentinel and the final division are the same
 * expressions.  Four warps per CTA serve four pooled blocks of one token; no
 * barrier remains.  DS4_QWEN4EXP_IDX_DECODE_BLOCK_SUM=1 restores the CTA
 * kernel. */
__global__ static void qwen4exp_qsa_indexer_scores_warp_kernel(
        const float *q,
        const float *pool,
        float *scores,
        uint32_t n_tokens,
        ...
        float norm_divisor) {
    const uint32_t lane = threadIdx.x & 31u;
    const uint32_t block = blockIdx.x * 4u + (threadIdx.x >> 5u);
    const uint32_t token = blockIdx.y;
    if (block >= n_blocks || token >= n_tokens) return;
    ...
    if (visible > n_blocks) visible = n_blocks;
    if (block >= visible) {
        if (lane == 0u) *dst = QWEN4EXP_QSA_MASKED_SCORE;
        return;
    }

    const float *k = pool + (uint64_t)block * 128u;
    const float k0 = k[lane], k1 = k[lane + 32u];
    const float k2 = k[lane + 64u], k3 = k[lane + 96u];
    float total = 0.0f;
    for (uint32_t h = 0; h < n_head; h++) {
        const float *qh = q + ((uint64_t)token * n_head + h) * 128u;
        const float p0 = __fmul_rn(qh[lane], k0);
        const float p1 = __fmul_rn(qh[lane + 32u], k1);
        const float p2 = __fmul_rn(qh[lane + 64u], k2);
        const float p3 = __fmul_rn(qh[lane + 96u], k3);
        float v = __fadd_rn(__fadd_rn(p0, p2), __fadd_rn(p1, p3));
#pragma unroll
        for (uint32_t step = 16u; step > 0u; step >>= 1) {
            v += __shfl_down_sync(0xffffffffu, v, step);
        }
        const float dot = __shfl_sync(0xffffffffu, v, 0);
        total += fmaxf(dot, 0.0f);
    }
    if (lane == 0u) *dst = total / norm_divisor;
}
...
extern "C" int ds4_gpu_qwen4exp_qsa_indexer_scores_tensor(
    ...
    if (head_dim == 128u &&
        getenv("DS4_QWEN4EXP_IDX_DECODE_BLOCK_SUM") == NULL) {
        qwen4exp_qsa_indexer_scores_warp_kernel<<<
            dim3((n_blocks + 3u) / 4u, n_tokens), 128u, 0,
            cuda_decode_stream()>>>(
                (const float *)q->ptr, (const float *)pool->ptr,
                (float *)scores->ptr, n_tokens, n_blocks, n_head, pos0,
                pool_size, sqrtf((float)head_dim));
        return cuda_ok(cudaGetLastError(), "Qwen4-Exp indexer scores warp launch");
    }
    const uint32_t nth = qwen4exp_cuda_threads(head_dim);
