...
/* Same request, with the L2 eviction priority the panel stream actually wants.
 * A panel asked for QW_DOWN_L2_DIST steps early has to survive that long, and
 * it is competing for the L2 against every other resident block's panels plus
 * the activation stream: at the decode width there are 20 panels of 5,440 B per
 * block, so the set this grid asks for is far larger than the L2 holds.  A line
 * marked evict_last is the last the L2 gives up, which is the right priority for
 * a line that has been requested and not yet read.  It is a hint and nothing
 * more: no value moves, no register is written, and the loads that follow read
 * the same bytes at the same addresses whether the hint was honoured, ignored,
 * or the line was evicted anyway. */
__device__ __forceinline__ static void qw_prefetch_l2_last(const char *p) {
...
#pragma unroll
     ...
     *
     * It changes no value.  `prefetch.global.L2::evict_last` moves nothing
     * into shared memory, writes no register, and the loads that follow read
     * the same bytes
