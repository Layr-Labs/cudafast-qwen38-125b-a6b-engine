...
/* How many consecutive panels to request per step.  Width W asks for panels
 * DIST .. DIST+W-1, so it raises the number of outstanding requests AND pushes
 * the leading edge out by W-1: it is not an independent axis from the distance,
 * it is the distance with overlap.  (An earlier version of this comment claimed
 * the edge stayed put.  It does not.)  What the overlap buys is redundancy -- a
 * panel is asked for W times as the window slides across it, so a request lost
 * to an eviction between the ask and the read can be re-made by a later step.
 * A repeat of a line already resident or in flight is dropped by the L2, and a
 * prefetch writes no register either way, so the emitted values are unchanged. */
#define QW_DOWN_L2_WIDTH 3u
...
#pragma unroll
     ...
     * so a panel is asked for QW_DOWN_L2_DIST steps before it is needed, and
     * QW_DOWN_L2_WIDTH times over as the window slides across it.  The distance
     * is ONE named constant used by both the prologue and the loop, so there is
     * no way to move one and forget the other.
     *
