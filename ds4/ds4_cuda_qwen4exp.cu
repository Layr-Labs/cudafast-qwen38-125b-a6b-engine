...
__global__ static void qwen4exp_moe_router_group_small_kernel(
    ...
    QWEN4EXP_PDL_SYNC();
    const bool single_pair_rank = Native && n_tokens == 1u &&
                                  n_expert_used <= 32u;
    if (single_pair_rank && threadIdx.x < n_expert)
...
#pragma unroll
    ...
    if (single_pair_rank) {
        if (threadIdx.x < 32u) {
            ...
            const int32_t expert = p < n_expert_used ? selected[p] : INT32_MAX;
            const bool valid = p < n_expert_used && expert >= 0 &&
                               (uint32_t)expert < n_expert;
            const uint32_t valid_mask = __ballot_sync(0xffffffffu, valid);
            int32_t rank = 0;
            for (uint32_t q = 0; q < n_expert_used; ++q) {
                const int32_t other = __shfl_sync(0xffffffffu, expert, q);
                rank += ((valid_mask >> q) & 1u) && other < expert;
            }
            if (valid) {
                counts[expert] = 1;
                offsets[expert] = rank;
                cursor[expert] = rank + 1;
                active[1 + rank] = expert;
                pairs[rank] = (int32_t)p;
            } else if (p < n_expert_used) {
                float *dst = mid + (uint64_t)p * mid_dim;
                for (uint32_t row = 0; row < mid_dim; ++row) dst[row] = 0.0f;
            }
            if (p == 0u) active[0] = __popc(valid_mask);
        }
        return;
    }
    /* PDL consumer of the router's top-k, which triggers at its top.  This
