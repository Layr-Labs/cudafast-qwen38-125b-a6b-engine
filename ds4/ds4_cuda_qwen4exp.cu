...
/* THE COUNTS RESET AND THE EXPERT HISTOGRAM IN ONE BLOCK.
 *
 * The grouped (n_tokens >= 8) path used to open with two stream operations:
 * a cudaMemsetAsync of n_total_expert counters, then
 * qwen4exp_moe_group_count_kernel, whose pair_blocks blocks each contend on
 * those same few hundred global counters with atomicAdd.  Within the
 * envelope below one block does both: it zeroes a shared histogram, folds
 * every pair into it with shared-memory atomics, and stores all
 * n_total_expert slots -- zeros included -- so the global counters end up
 * written exactly once each, with no reset launch ahead of them.
 *
 * Counts are integer sums of the same predicate over the same pairs (an
 * out-of-range expert id is skipped, as before), and integer addition is
 * order-free, so every count is the reference count; the scan, scatter and
 * every consumer downstream read the same values. */
enum { QWEN4EXP_MOE_COUNT_ONE_BLOCK_MAX_PAIRS = 32768 };

__global__ static void __launch_bounds__(QWEN4EXP_MOE_SCAN_THREADS)
qwen4exp_moe_group_count_one_block_kernel(
        int32_t *counts,
        const int32_t *selected,
        uint32_t n_total_expert,
        uint32_t n_pairs) {
    __shared__ int32_t hist[QWEN4EXP_MOE_SCAN_THREADS];
    const uint32_t tid = threadIdx.x;
    hist[tid] = 0;
    __syncthreads();
    for (uint32_t p = tid; p < n_pairs; p += QWEN4EXP_MOE_SCAN_THREADS) {
        const int32_t e = selected[p];
        if (e >= 0 && (uint32_t)e < n_total_expert) atomicAdd(&hist[e], 1);
    }
    __syncthreads();
    if (tid < n_total_expert) counts[tid] = hist[tid];
}
...
static int qwen4exp_routed_moe_cuda(
    ...
    if (fuse_router) {
        ...
        if (n_total_expert <= QWEN4EXP_MOE_SCAN_THREADS &&
            n_pairs <= QWEN4EXP_MOE_COUNT_ONE_BLOCK_MAX_PAIRS &&
            getenv("DS4_QWEN4EXP_SPLIT_GROUP_COUNT") == NULL) {
            qwen4exp_moe_group_count_one_block_kernel<<<
