 ...
 */
template <bool WRITE_GATES = false>
__global__ static void qwen4exp_gdn_conv_kernel(
        ...
        const float *raw_alpha = NULL, const float *raw_beta = NULL,
        const float *a_log = NULL, const float *dt_bias = NULL) {
    ...
    for (uint32_t token = 0; token < n_tokens; token++) {
        ...
        /* The recurrence uses one pair for all 128 value rows of a head.
         * Publish it once from the convolution already ordered before that
         * recurrence. This is independent of the convolution block's channel
         * identity; blocks [0,n_value_head) have unique gate ownership. */
        if (WRITE_GATES && tid == 0u && block < n_value_head) {
            const uint64_t gate = ((uint64_t)row * n_tokens + token) *
                                  n_value_head + block;
            gate_pairs[gate] = make_float2(
                expf(a_log[block] * qwen4exp_gdn_softplus(raw_alpha[gate] + dt_bias[block])),
...
__global__ static void qwen4exp_gdn_replay_kernel(
        ...
        const float2 *current_gates = NULL) {
    const uint32_t head = blockIdx.x;
    const uint32_t value = blockIdx.y * 4u + (threadIdx.x >> 5u);
    const uint32_t lane = threadIdx.x & 31u;
    ...
    for (uint32_t step = 0; step < prefix + n_tokens; step++) {
        ...
        if (replay) {
            ...
            if (current_gates) {
                const float2 pair = current_gates[gate];
                g = pair.x; beta = pair.y;
            } else {
                if (lane == 0u) {
...
static int qwen4exp_cuda_gdn_run(
    ...
    if (replay && (n_rows != 1u || n_tokens != 2u || n_snapshot_rows != 1u ||
        ...
        ds4_tensor_device_idx(replay->control) != logical_tier ||
        (replay->gates &&
         (!glm53_cuda_tensor_has(replay->gates, gate_elements, sizeof(float2)) ||
          ds4_tensor_device_idx(replay->gates) != logical_tier)))) return 0;
    ...
    } else if (replay && replay->gates) {
        qwen4exp_gdn_conv_kernel<true><<<dim3(blocks, n_rows, 1u),
                                       QWEN4EXP_GDN_DIM, 0, stream>>>(
            (float *)qkv->ptr, (float *)conv_state->ptr, conv_weight,
            conv_snapshot ? (float *)conv_snapshot->ptr : NULL,
            n_key_head, n_value_head, n_rows, n_tokens, n_snapshot_rows,
            qk_norm_eps, adopt_row, (float2 *)replay->gates->ptr,
            (const float *)raw_alpha->ptr, (const float *)raw_beta->ptr, a_log, dt_bias);
    } else {
    ...
    if (replay) {
        qwen4exp_gdn_replay_kernel<<<recurrence_grid, QWEN4EXP_GDN_DIM, 0, stream>>>(
            ...
            replay->gates ? (const float2 *)replay->gates->ptr : NULL);
    } else if (gate_pairs) {
