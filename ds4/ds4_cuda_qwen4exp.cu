...
#pragma unroll
        ...
        __syncthreads();
        /* One warp consumes both original reduction trees for its row/token.
         * It owns the SiLU epilogue too, so no cross-warp projected exchange
         * or second publication barrier is needed. Each tree's operands and
         * descending shuffle order are exactly the original ones. */
        const uint32_t row = warp >> 1u, r = warp & 1u;
        if (row < rows_here && r < take) {
            const float g = warp_sum_f32(partial[row * 2u][r][lane]);
            const float u = warp_sum_f32(partial[row * 2u + 1u][r][lane]);
            if (lane == 0u) {
                const uint32_t p = (uint32_t)pairs[base + at + r];
        ...
        /* No shared storage is reused after the last pair. Counts and loop
         * position are block-uniform; longer lists still wait for readers. */
        if (at + 2 < cnt) __syncthreads();
    }
}
