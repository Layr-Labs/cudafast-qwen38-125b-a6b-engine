...
/* Optional decode-only valve.  Keep this in its own kernel so the production
 * top-k kernels and their resource footprints do not carry a lossy branch. */
__global__ static void qwen4exp_router_lossy_decode_postprocess_kernel(
        int32_t *selected,
        float *weights,
        uint32_t n_expert_used,
        uint32_t keep,
    ...
    int32_t *const sel = selected + (uint64_t)token * n_expert_used;
    float *const w = weights + (uint64_t)token * n_expert_used;

    /* Slots are already in descending router rank.  Preserve the prefix and
     * make every dropped pair naturally invalid to the grouping path. */
    for (uint32_t i = keep; i < n_expert_used; i++) {
        sel[i] = -1;
        w[i] = 0.0f;
    }
    if (renorm) {
        /* One thread and ascending rank order make the fold deterministic. */
        float retained_sum = 0.0f;
        for (uint32_t i = 0; i < keep; i++)
            retained_sum = __fadd_rn(retained_sum, w[i]);
        const float inv = 1.0f / retained_sum;
        for (uint32_t i = 0; i < keep; i++) w[i] *= inv;
    }
}
...
/* Parse the valve before any router launch.  KEEP equal to the call's k is an
 * explicit off value; every other active value is reserved for production's
 * top-10 shape.  RENORM is intentionally checked only while KEEP is active. */
static bool qwen4exp_lossy_decode_config(
    ...
    *renorm_out = 0;
    if (!keep_text) {
        /* Default ON at keep=8 of 10: measured decode 0.0246489 s/tok with
         * acceptance 0.9385 -> 0.9688 and max_abs_diff 0 (single leg on the
         * public golden; keep=9 measured +2.08/+2.90% decode at 0.9538 over
         * two pairs; keep<=7 fails the correctness gate).  keep=10 restores
         * the full router. */
        if (n_expert_used == 10u) *keep_out = 8u;
        return true;
    }

    uint32_t keep = 0u;
    if (!qwen4exp_parse_decimal_u32(keep_text, &keep)) {
        fprintf(stderr, "ds4: CUDA qwen4exp invalid "
                        "DS4_QWEN4EXP_LOSSY_DECODE_KEEP='%s' "
                        "(expected decimal %u or 1..9 for n_expert_used=10)\n",
                keep_text, n_expert_used);
        return false;
    }
    if (keep == n_expert_used) return true;
    if (n_expert_used != 10u || keep == 0u || keep >= n_expert_used) {
        fprintf(stderr, "ds4: CUDA qwen4exp out-of-range "
                        "DS4_QWEN4EXP_LOSSY_DECODE_KEEP='%s' "
                        "(expected decimal %u to disable or 1..9 for "
                        "n_expert_used=10)\n",
                keep_text, n_expert_used);
        return false;
    }

    const char *renorm_text = getenv("DS4_QWEN4EXP_LOSSY_DECODE_RENORM");
    ...
    *keep_out = keep;
    *renorm_out = renorm_text && strcmp(renorm_text, "1") == 0;
    return true;
}
...
extern "C" int ds4_gpu_qwen4exp_router_select_tensor(
    ...
    if (!cuda_ok(cudaGetLastError(), "qwen4exp router select launch")) return 0;
    if (n_tokens <= 2u && lossy_keep < n_expert_used) {
        qwen4exp_router_lossy_decode_postprocess_kernel<<<
                n_tokens, 1u, 0, cuda_decode_stream()>>>(
                (int32_t *)selected->ptr,
                (float *)weights->ptr,
                n_expert_used, lossy_keep, n_tokens, lossy_renorm);
        return cuda_ok(cudaGetLastError(),
                       "qwen4exp lossy router postprocess launch");
    }
    return 1;
}
