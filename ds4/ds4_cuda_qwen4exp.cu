 ...
 * -11%.  The occupancy that IS worth buying is on the down kernel, which the
 * same probe found register-capped at 40 warps; see the cap there. */
...
/* Register cap, in the form that actually caps.
 *
 * A probe shipped in `ec7bb97f` published `dn[reg=48 smem=0 lmem=0 maxt=1024]`
 * for the production staged q8_0 instantiation.  (`smem=0` is not a bug: the
 * panel is *dynamic* shared memory, which cudaFuncGetAttributes does not
 * count.)  Forty-eight registers at the 256 threads this kernel is always
 * launched with is 12,288 registers per block, so a 65,536-register file holds
 * only 5 blocks: 40 of the 64 warps an SM can carry.  Its 10,880 B panel would
 * allow nine.  **Registers, not shared memory, are what bind this kernel** --
 * which was not knowable before that probe, and which means the shared-memory
 * arithmetic this kernel has been tuned by was reading the wrong constraint.
 *
 * Residency is worth something here, measured: `5b689c51` (the promoted
 * frontier) doubles the panel to 21,760 B and therefore runs at 4 blocks/SM,
 * and `3054d84b` -- same one-barrier-per-step schedule at 10,880 B, so 5
 * blocks -- decodes 0.54% faster.  A quarter more residency for half a percent.
 * Going to 32 registers takes this kernel to 8 blocks = 64 warps (8 x 10,880 =
 * 87,040 B, inside the 101,376 B opt-in), i.e. +60%.
 *
 * WHY THIS SPELLING.  `fd1cafd2` tried to cap the gate/up kernel with
 * __launch_bounds__(512, 4) -- its real block size, and the block count wanted.
 * That BACKFIRED: registers went 40 -> 60.  Lowering maxThreadsPerBlock from
 * the default 1024 to 512 doubles the per-thread register ceiling (65,536/512
 * instead of 65,536/1024), ptxas took the licence, and
 * minBlocksPerMultiprocessor turned out to be advisory, not binding.  The cap
 * is the QUOTIENT of the two arguments, so the way to ask for 32 registers
 * without relaxing anything is to leave maxThreadsPerBlock at the default 1024
 * and set minBlocksPerMultiprocessor to 2: 65,536 / (2 x 1024) = 32.  1024 is
 * >= the 256 threads actually launched, so nothing about the launch changes and
 * the declared ceiling is exactly the one that was already in force.
 *
 * Bit-exact: an allocation and scheduling constraint.  ptxas may recompute live
 * ranges, but the terms, their order, the barriers and the reduction tree are
 * fixed by the source, so the emitted tokens are identical.
 *
 * The risk is spill, and it is self-diagnosing: the probe at the end of this
 * file re-reads `dn[reg]` and `dn[lmem]` in the same run.  A non-zero `lmem`
 * means this cap should be removed regardless of the score. */
template <int R, int DownType = -1, bool Vector = false, bool Stage = false>
__global__ static void __launch_bounds__(1024, 2) qwen4exp_moe_down_q_kernel(
        float *out,
        const char *down,
        const int32_t *selected,
        const int8_t *mq,
 ...
 *
 * This run caps the DOWN kernel at 32 registers, in the spelling fd1cafd2
 * proved is the right one -- default maxThreadsPerBlock, minBlocks chosen so
 * the quotient is the cap.  Read `dn[reg]` first: 32 with `lmem=0` means the
 * cap was free and the kernel should be at 8 blocks/SM instead of 5.  A
 * non-zero `dn[lmem]` means it spilled and the cap must come out whatever the
 * score says.
 *
