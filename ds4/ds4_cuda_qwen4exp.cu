...
qwen4exp_qsa_split_probs_kernel(
    ...
    /* Programmatic dependent of the scores kernel on the decode stream (see
     * the split launcher).  The fence is the kernel's first statement, ahead
     * of every read -- d_pos, counts, the scores row, the tile maxima -- so
     * nothing this block reads can precede the producer's completion; the
     * scores kernel carries no trigger, so the edge only lets this grid be
     * launched against the tail of that one instead of after it drains.  A
     * plain launch makes the fence a no-op. */
    QWEN4EXP_PDL_SYNC();
    const uint32_t group = blockIdx.x;
    const uint32_t tile = blockIdx.y;
    const uint32_t token = blockIdx.z;
...
__global__ static void qwen4exp_qsa_split_fold_kernel(
        ...
        uint32_t tile_width,
        const uint32_t *d_pos) {
    /* Programmatic dependent of the probs kernel, fenced before its first
     * read and its first write (the probs kernel carries no trigger). */
    QWEN4EXP_PDL_SYNC();
    const uint32_t head = blockIdx.x;
    const uint32_t token = blockIdx.y;
    const uint32_t tid = threadIdx.x;
