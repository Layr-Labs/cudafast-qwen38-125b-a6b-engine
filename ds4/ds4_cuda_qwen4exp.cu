...
extern "C" int ds4_gpu_qwen4exp_routed_moe_tensor(
    ...
    const bool split_vectors = n_tokens <= 2u &&
        in_dim == 2560u && mid_dim == 640u &&
        gate_slab->type == DS4_QWEN4EXP_TY_q4_K &&
        up_slab->type == DS4_QWEN4EXP_TY_q4_K &&
        getenv("DS4_QWEN4EXP_NO_SPLIT_VECTOR") == NULL;
    /* Every activation group begins at a multiple of 32 from xq. Align its
     * base for the split kernel's two int4 reads; the metadata remains in
     * its existing layout and the down scratch still needs only word alignment. */
    const uint64_t quant_offset = split_vectors ?
        (idx_bytes + pair_bytes + 15u) & ~UINT64_C(15) : idx_bytes + pair_bytes;
    char *base = (char *)qwen4exp_group_scratch(
            logical_tier, quant_offset + xq_bytes + mq_bytes);
    if (!base) return 0;
...
#undef QWEN4EXP_GATEUP_MMA
     ...
     * keep their prior kernel. */
        ...
        if (split_vectors) {
            qwen4exp_moe_gateup_split_kernel<2, DS4_QWEN4EXP_TY_q4_K, true, 2><<<
                dim3((mid_dim + 1u) / 2u, gu_rows, 1), 128u, 0, stream>>>(
                (float *)mid->ptr, gate, up, sc.xq, sc.xs, sc.xsum,
                sc.pairs, sc.counts, sc.offsets, gu_active,
                (const float *)weights->ptr,
                gate_slab->expert_bytes, gate_slab->row_bytes,
