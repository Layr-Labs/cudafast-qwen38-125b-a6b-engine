 ...
 * Per-channel accumulation order over h is the scalar kernel's own, so each
 * output float is the scalar kernel's float, bit for bit. */
 ...
 * float4 LDG/STG vector operations; same per-channel FFMA the scalar kernel
 * issues, including the shared-expert accumulate and the PDL producer edge,
 * so the values and the stream behavior are the shipped kernel's own. */
...
extern "C" int ds4_gpu_qwen4exp_hc_mix_tensor(
    ...
    if ((n_embd % 4u) == 0 &&
        ...
        getenv("DS4_QWEN4EXP_NO_HC_VEC4") == NULL) {
...
__global__ static void qwen4exp_qsa_output_gate_quant_kernel(
     ...
     * prefill width.  The PDL=false instantiation keeps the shipped kernel,
     * byte for byte; DS4_QWEN4EXP_NO_QSA_GATE_PDL selects it. */
    if (PDL && gridDim.x <= 48u) QWEN4EXP_PDL_TRIGGER();
    const uint64_t gid = (uint64_t)blockIdx.x * blockDim.x + threadIdx.x;
    const uint32_t lane = threadIdx.x & 31u;
    const uint32_t warp = threadIdx.x >> 5u;
...
#define QWEN4EXP_QSA_GROUP_LAUNCH(G)                                          \
            ...
            if (g == 12u && head_dim == 256u && nth == 256u &&
                ...
                /* Decode-only next-V-tile L2 prefetch; the template arm keeps
                 * the shipped kernel byte-identical when it is stood down.
                 * DS4_QWEN4EXP_NO_QSA3_VPREFETCH selects the original body. */
                const bool qsa3_vpref =
                    getenv("DS4_QWEN4EXP_NO_QSA3_VPREFETCH") == NULL;
                if (qsa3_vpref) {
                    qwen4exp_qsa3_attention_group_kernel<12u, false, true><<<grid, nth, gshared3,
                        cuda_decode_stream()>>>(
                            (const float *)q->ptr, (const float *)k_cache->ptr,
                            (const float *)v_cache->ptr,
                            sparse ? (const int32_t *)selected->ptr : NULL,
...
extern "C" int ds4_gpu_qwen4exp_qsa_output_gate_q8_tensor(
    ...
    if (getenv("DS4_QWEN4EXP_NO_QSA_GATE_PDL") == NULL) {
        qwen4exp_qsa_output_gate_quant_kernel<true>
        ...
        qwen4exp_qsa_output_gate_quant_kernel<false>
            <<< (unsigned)(n_values / 256u), 256u, 0, cuda_decode_stream() >>>
            ((int8_t *)((char *)q8->ptr + q_offset),
             (float *)((char *)q8->ptr + s_offset),
             (const float *)gate->ptr, (const float *)out->ptr, n_values);
