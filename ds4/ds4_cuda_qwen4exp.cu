...
#undef QWEN4EXP_QSA_GROUP_LAUNCH
    ...
    /* Kept behind an explicit diagnostic opt-in after the combined split
     * experiment failed the official token gate.  The ranked path retains the
     * promoted single-block arithmetic while HC injection is isolated below. */
    if (n_tokens < 8u && getenv("DS4_QWEN4EXP_ENABLE_QSA_SPLIT") != NULL) {
