 ...
 * memory with coalesced sixteen-byte loads, then decoded out of shared.
 *
 * The defect is the one this tree already fixed at the shared expert's DOWN
 * projection, at the one projection that fix does not reach.  The group count
 * is small at the checkpoint's shape, so the walk is a single step: `lane <
 * groups` covers the whole row and the `lane + 32` remainder never runs.  Each
 * working lane then fetches its own group's payload out of TWO row slabs as
 * several sub-word pieces strided by the group size, so one load instruction
 * asks for a fistful of scattered four-byte pieces while the lanes past
 * `groups` contribute nothing to the request -- and it does that twice, once
 * for gate and once for up.  Every byte is consumed, but both requests are
 * strided and under-filled.
 *
 * The bytes themselves are not scattered.  The eight rows a block owns are
 * 8 * gate_row_bytes consecutive bytes of one single-expert slab and
 * 8 * up_row_bytes consecutive bytes of the other, so the same bytes can be
 * fetched as two dense bursts by the whole block, every thread participating.
 *
 ...
 * with the SAME (type, g) and a row pointer at the same offset within the row,
 * and the decoder is alignment-agnostic by construction -- it aligns the
 * payload address down, derives `shift` from the low bits and funnel-shifts
 * the logical bytes back out.  Accumulation order, the lane-to-group map, the
 ...
 * the host makes block-uniform by refusing this arm unless mid_dim % 8 == 0,
 * and the staged arm skips the sync inside the walk, so a thread performs
 * exactly one grid dependency sync either way.
 *
 ...
 * (ds4_cuda_qwen4exp.cuh).  Verify and prefill keep the plain launch. */
     ...
     * sixteen-byte multiples over sixteen-byte aligned slab bases, and the two
     * panels together must fit the block's shared-memory budget.  Anything
     * short of that takes the shipped global-decode arm unchanged, and
    ...
    const int sh_gu_stage =
        ...
        ((uintptr_t)up & 15u) == 0u &&
        sh_gu_bytes <= QW_DOWN_PANEL_MAX_BYTES &&
