...
/* Register cap, and why this number.
 *
 * A probe shipped in `ec7bb97f` published `gu[reg=40 smem=23168 lmem=0
 * maxt=1024]` for the production Coop instantiation through
 * officialMetrics.engine_backend.  Forty registers at 512 threads is 20,480
 * registers per block, so a 65,536-register file admits floor(65536/20480) = 3
 * blocks: 48 of the 64 warps an SM can hold, 75% occupancy.  Shared memory was
 * never the binding constraint -- 23,168 B against a 101,376 B opt-in would
 * allow four blocks -- and my own earlier note that this kernel ran at "64
 * warps/SM, 100% occupancy" was derived from the shared-memory number alone and
 * is wrong.
 *
 * Thirty-two registers is the exact threshold for the fourth block:
 * 4 x 16 warps x 32 lanes x 32 = 65,536, the whole file.  So the bound below
 * asks ptxas for 32, which is an 8-register cut, and lmem=0 today means there
 * is no spill to grow from.
 *
 * The bound is written in terms of the template parameters so only the Coop
 * instantiation is constrained: it is the one launched with OutputRows * 64 ==
 * 512 threads and the one whose occupancy is capped at 3.  The direct
 * instantiations launch 64 and 256 threads and ask for one block, which is a
 * no-op hint -- their generated code is unchanged.
 *
 * This is a scheduling hint, not a numerical change: ptxas may recompute the
 * live ranges but the arithmetic and its order are fixed by the source, so the
 * emitted tokens are bit-identical either way.  The risk is entirely that a
 * forced cut spills to local memory, and the probe reports `lmem` in the same
 * run, so the outcome is self-diagnosing. */
template <int R, int Type, bool Vector = false,
          unsigned OutputRows = 4, bool Coop = false>
__global__ static void __launch_bounds__(OutputRows * 64u, Coop ? 4 : 1)
 ...
 *
 * The previous run of this probe (`ec7bb97f`) returned
 *   gu[reg=40 smem=23168 lmem=0 maxt=1024] dn[reg=48 smem=0 lmem=0 maxt=1024]
 * which is what motivates the __launch_bounds__ above: 40 registers at 512
 * threads caps gate/up at 3 blocks/SM (48 warps), not the 4 (64 warps) its
 * shared-memory footprint would allow.  Re-reading it here is how the cap gets
 * checked -- if ptxas honoured it, `gu[reg]` drops to 32 and `lmem` says
 * whether it paid for that with a spill.
 *
