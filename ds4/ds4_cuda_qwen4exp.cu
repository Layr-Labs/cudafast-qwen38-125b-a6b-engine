...
__device__ __forceinline__ static void dev_qwen4exp_group_decode_w(
    ...
    case (uint32_t)DS4_QWEN4EXP_TY_q4_K: {
        ...
        const uint32_t grp = g % 8u;
        uint8_t sc = 0, m = 0;
        /* ONE HEADER LOAD INSTEAD OF FIVE.  d, dmin and all twelve scale
         * bytes are the first sixteen bytes of the block, and
         * sizeof(cuda_block_q4_K) is 144, a multiple of sixteen, so every
         * block of a sixteen-byte-aligned slab is sixteen-byte aligned and
         * the test is uniform across the warp.  The decode-side gate/up
         * staging already reads this header exactly this way and derives the
         * same three quantities from it through qw_q4k_header_scale_min; this
         * arm is the prefill staging, and it was still taking the scale pair
         * a byte at a time and d and dmin as two halfword loads.  Same bytes,
         * same values, five loads become one -- see the derivation above
         * qw_q4k_header_scale_min, and note wb keeps the shipped order:
         * negate the dmin conversion, then multiply. */
        if ((((uintptr_t)xb) & 15u) == 0u) {
            const uint4 hdr = *(const uint4 *)(const void *)xb;
            uint32_t sc32 = 0, mn32 = 0;
            qw_q4k_header_scale_min(grp, hdr.y, hdr.z, hdr.w, &sc32, &mn32);
            wa[0] = dev_f16_to_f32((uint16_t)(hdr.x & 0xffffu)) * (float)sc32;
            wb[0] = -dev_f16_to_f32((uint16_t)(hdr.x >> 16u)) * (float)mn32;
        } else {
            dev_q4_K_get_scale_min(grp, xb->scales, &sc, &m);
            wa[0] = dev_f16_to_f32(xb->d) * (float)sc;
            wb[0] = -dev_f16_to_f32(xb->dmin) * (float)m;
