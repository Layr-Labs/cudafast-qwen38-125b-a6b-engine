...
/* =1 at build time keeps the one-slot-at-a-time combine walk. */
#ifndef QWEN4EXP_COMBINE_SERIAL_SLOTS
#define QWEN4EXP_COMBINE_SERIAL_SLOTS 0
#endif
__global__ static void qwen4exp_moe_down_combine_grid_kernel(
    ...
    float acc = 0.0f;
    uint32_t slot = 0;
#if !QWEN4EXP_COMBINE_SERIAL_SLOTS
    /* Ten slots at a time -- the routed width this model runs, the "ten loads
     * and ten adds" the flat kernel's note counts.  Every slot's id and
     * partial in a chunk are ASKED FOR before the chunk's first add, so the
     * ten independent partial reads are in flight together instead of each
     * waiting behind the add of the one before it.  The adds then run in
     * ascending slot order and skip an out-of-range id exactly as the loop
     * below does, so `acc` is the same float.  A width that is not a multiple
     * of ten finishes in the serial loop, which continues at `slot`. */
    for (; slot + 10u <= n_expert_used; slot += 10u) {
        float v[10];
        bool live[10];
#pragma unroll
        for (uint32_t s = 0; s < 10u; s++) {
            const uint64_t pair = pair0 + slot + s;
            const int32_t e = selected[pair];
            live[s] = e >= 0 && (uint32_t)e < n_total_expert;
            v[s] = live[s] ? partial[pair * out_dim + row] : 0.0f;
        }
#pragma unroll
        for (uint32_t s = 0; s < 10u; s++) {
            if (live[s]) acc += v[s];
