 ...
 * that separating the load from the decode lets a caller keep several loads
 * outstanding; the arithmetic is the same arithmetic. */
...
#pragma unroll
    ...
    /* SLOT-PAIR STAGING.  Every (slot, token) item of the walk below is one
     ...
     * box's memory roofline, so the miss latency is exposed, not the bytes.
     *
     * This is the same shape the split gate/up kernel already fixed by keeping
     * two groups in flight; the down kernel never got the treatment, and it has
     * no `for g` slack to get it from.  The slack it does have is across items:
     * two slots by R tokens are four different expert rows, hence four
     * independent block loads.  Issue all four, then decode all four, and the
     * walk costs ceil(n_expert_used / 2) latencies instead of
     * n_expert_used * take.
     *
     ...
     * accumulator, not merely its multiset.  The staged decoder reads the
     ...
     *
     * The expert broadcast stays warp-collective: slot, k, n_expert_used, r and
     * take are all warp uniform, so every lane reaches every __shfl_sync. */
    constexpr bool staged = Vector && DownType == (int)DS4_QWEN4EXP_TY_q5_1;
    if (staged) {
        const uint32_t g = lane;
        for (uint32_t s0 = 0; s0 < n_expert_used; s0 += 2u) {
            const char *drow[2][R];
            uint32_t raw[2][R][6];
            bool ok[2][R];
#pragma unroll
...
#pragma unroll
                for (int r = 0; r < R; r++) {
                    const uint32_t slot = s0 + (uint32_t)k;
                    drow[k][r] = NULL;
                    ok[k][r] = false;
                    if (slot >= n_expert_used || (uint32_t)r >= take) continue;
                    ...
                    if (g < groups)
                        ok[k][r] = qw_raw_load((uint32_t)DS4_QWEN4EXP_TY_q5_1,
                                               drow[k][r], g, raw[k][r]);
                }
            }
#pragma unroll
...
#pragma unroll
                for (int r = 0; r < R; r++) {
                    const uint32_t slot = s0 + (uint32_t)k;
                    if (drow[k][r] == NULL || g >= groups) continue;
                    const uint64_t at_g =
                    ...
                    int halves = 1;
                    if (ok[k][r])
                        dev_qwen4exp_group_decode_q5_1_raw(raw[k][r], wq, wa,
                                                           wb, &halves);
                    else
                        dev_qwen4exp_group_decode(
                                (uint32_t)DS4_QWEN4EXP_TY_q5_1,
                                drow[k][r], g, wq, wa, wb, &halves);
                    if (halves == 1)
                        qwen4exp_shared_vector_accumulate(&acc[r], wq, wa[0],
                                wb[0], mq + at_g * 32u, ms[at_g], msum[at_g]);
                    else
                        qwen4exp_group_accumulate(&acc[r], wq, wa, wb, halves,
