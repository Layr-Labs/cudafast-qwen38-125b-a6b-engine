 ...
 */
/* This build's note auto09180200_130 records that a kernel launched with
 * the programmatic stream serialization attribute parks at its fence, and
 * the profiler counts that parked time as the kernel's duration, so any
