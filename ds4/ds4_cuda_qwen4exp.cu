...
__global__ static void qwen4exp_qsa_indexer_scores_kernel(
    ...
    /* Warp-per-head fast path at the model shape (head_dim 256, 256 threads).
     * Each warp owns heads warp, warp+nwarp, ... and reduces one head's dot
     * entirely in registers: lane l holds the products of elements l+32t,
     * which are exactly the eight partials qwen4exp_blk_sum's shared-memory
     * steps fold into c[l], so the register tree r[t] += r[t+s'] and the
     * shuffle tail reproduce the block reduction bit for bit with zero
     * barriers inside the head loop.  The K row is shared by every head, so
     * it is loaded once into registers instead of once per head.  Per-head
     * dots go through shared memory so thread 0 can keep the original
     * h-ascending fmaxf accumulation order for the total. */
    if (head_dim == 256u && nth == 256u && n_head <= nth) {
        const uint32_t lane = tid & 31u;
        const uint32_t warp = tid >> 5u;
        const uint32_t nwarp = nth >> 5u;
        float kr[8];
#pragma unroll
        for (uint32_t t = 0; t < 8u; t++) kr[t] = k[lane + 32u * t];
        for (uint32_t h = warp; h < n_head; h += nwarp) {
            const float *qh = q + ((uint64_t)token * n_head + h) * head_dim;
            float r[8];
#pragma unroll
            for (uint32_t t = 0; t < 8u; t++) r[t] = qh[lane + 32u * t] * kr[t];
#pragma unroll
            for (uint32_t s = 4u; s > 0u; s >>= 1) {
#pragma unroll
                for (uint32_t t = 0; t < s; t++) r[t] += r[t + s];
            }
            float v = r[0];
#pragma unroll
            for (uint32_t step = 16u; step > 0u; step >>= 1) {
