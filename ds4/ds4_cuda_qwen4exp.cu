...
__global__ static void qwen4exp_gdn_recurrence_kernel(
    ...
    for (uint32_t token = 0; token < n_tokens; token++) {
        ...
        h.z *= g;
        h.w *= g;
        /* The step used to run its two reductions SERIALLY: the second summed
         * over the state the first one's result had just updated.  Expanding
         * it removes that dependency,
         *
         *     h_new . q  =  (h_g + k * delta_v) . q
         *               =  (h_g . q) + delta_v * (k . q),
         *
         * so all three dot products are available before delta_v and their
         * butterflies are independent.  The same products are summed; only the
         * order differs. */
        const float hk = warp_sum_all_f32(dot4_f32(h, k4));
        const float hq = warp_sum_all_f32(dot4_f32(h, q4));
        const float kq = warp_sum_all_f32(dot4_f32(k4, q4));
        ...
        const float result = fmaf(delta_v, kq, hq);
        if (lane == 0u) {
