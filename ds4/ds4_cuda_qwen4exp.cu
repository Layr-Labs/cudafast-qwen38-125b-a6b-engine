...
#pragma unroll
     ...
     *
     * All expert counts are cleared before warp zero publishes the live runs;
     * offsets/cursor are consumed only for a positive count and are overwritten
     * for every such run.  The barrier prevents another warp's zero from racing
     * a live count.  Wider/non-native calls keep the shipped scan. */
    if constexpr (Native) {
        if (n_pairs <= 32u) {
