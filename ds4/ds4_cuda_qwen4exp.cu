...
#define QW_DOWN_PANEL_L2_LEAD 4u
...
#pragma unroll
     ...
     * step k+QW_DOWN_PANEL_L2_LEAD will copy while step k+1's cp.async is in flight,
     * so each panel is requested exactly once, QW_DOWN_PANEL_L2_LEAD steps before it
     ...
     * DS4_QWEN4EXP_DOWN_PANEL_L2_OFF stands it down without a rebuild.  An
...
#pragma unroll
        for (uint32_t k = 1u; k < QW_DOWN_PANEL_L2_LEAD; k++) qw_l2_ahead(k);
    }
        ...
        for (int r = 0; r < R; r++) {
            if ((uint32_t)r < take) {
                ...
                if (Stage) {
                    ...
                    qw_l2_ahead(step + QW_DOWN_PANEL_L2_LEAD);
                }
...
static int qwen4exp_down_panel_l2_lead(void) {
    static int v = -1;
    if (v < 0) v = getenv("DS4_QWEN4EXP_DOWN_PANEL_L2_OFF") == NULL ? 1 : 0;
    return v;
}
...
#define QWEN4EXP_DOWN_ARGS \
            ...
            (uint32_t)qwen4exp_down_panel_l2_lead()
