...
__global__ static void qwen4exp_hc_inject_vec4_kernel(
    ...
    /* `block`, `shexp_tot` and `shexp_gate` do not depend on the hyper-connection
     * stream, so folding h out of the grid and into a per-thread loop loads each
     * of them ONCE instead of n_hc times.  Every emitted expression below is
     * character-identical to the per-h version, so this is not a reassociation:
     * `blk` is computed from the same inputs by the same operations, and each
     * output element is still a single `res + blk * inj`.  Proven byte-identical
     * against the pre-edit kernel by a host parity rig over the scored shape
     * (n_embd4 640, n_hc 4, rows 2), both shared-expert states, and every block
     * width either form ships. */
...
extern "C" int ds4_gpu_qwen4exp_hc_inject_tensor(
    ...
    if ((n_embd % 4u) == 0 &&
        ...
        const uint32_t n_embd4 = n_embd / 4u;
        qwen4exp_hc_inject_vec4_kernel<<<dim3((n_embd4 + 63u) / 64u, rows), 64,
...
#define DS4_GU_BW_PROBE_BUILD 0
#endif
