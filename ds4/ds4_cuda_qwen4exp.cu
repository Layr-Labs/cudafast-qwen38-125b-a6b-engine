...
#pragma unroll
     ...
     * and the loop.  The base tree shipped this distance as three separate
     * literals -- two in the prologue and a `c + 3` in the loop -- and a
     * literal moved in one place and not the other leaves a silent gap in the
     * stream rather than a compile error.  Four rather than three: a chunk's
     * copies are 64-byte pieces of 128 rows 1,440 bytes apart, so its fill is
     * a scatter of partial lines, and one more chunk of lead is one more
     * barrier of cover.  The distance is the only thing this changes. */
