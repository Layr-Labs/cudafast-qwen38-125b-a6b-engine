...
/* Partials for the split-KV attention, in a FIXED device buffer rather than a
 * run-time allocation.  The scored path plans its memory once at load and the
 * split only ever runs at speculative width, so the size is bounded and known:
 * DS4_QWEN4EXP_MTP_MAX_COMMIT rows by the head count by the slice count, each a
 * head_dim accumulator plus a maximum and a sum.  A shape that would not fit is
 * refused by the caller, which keeps the single-block walk instead. */
#define QWEN4EXP_QSA_SPLIT_CAPACITY 400000u
__device__ static float g_qwen4exp_qsa_split_parts[QWEN4EXP_QSA_SPLIT_CAPACITY];
...
__global__ static void qwen4exp_qsa_attention_split_kernel(
    ...
    const uint64_t slots = (uint64_t)n_tokens * n_head * QWEN4EXP_QSA_SPLIT;
    const uint64_t pslot = ((uint64_t)token * n_head + head) * QWEN4EXP_QSA_SPLIT
                         + slice;
    float *const part_acc = g_qwen4exp_qsa_split_parts;
    float *const part_max = part_acc + slots * head_dim;
    float *const part_sum = part_max + slots;
...
__global__ static void qwen4exp_qsa_attention_combine_kernel(
    ...
    const uint64_t slots = (uint64_t)n_tokens * n_head * QWEN4EXP_QSA_SPLIT;
    const float *const part_acc = g_qwen4exp_qsa_split_parts;
    const float *const part_max = part_acc + slots * head_dim;
    const float *const part_sum = part_max + slots;
...
#undef QWEN4EXP_QSA_GROUP_LAUNCH
     ...
     * axis, so it keeps the single-block walk. */
    {
        ...
        const uint64_t need = slots * head_dim + 2ull * slots;
        /* OPT-IN. The split path is compiled in but runs only when
         * DS4_QWEN4EXP_QSA_SPLIT names it, so the default path through this
         * function is the single-block walk below, unchanged. */
        if (n_tokens < 8u && need <= (uint64_t)QWEN4EXP_QSA_SPLIT_CAPACITY &&
            getenv("DS4_QWEN4EXP_QSA_SPLIT") != NULL) {
            cudaStream_t stream = cuda_decode_stream();
            qwen4exp_qsa_attention_split_kernel<<<
                    ...
                    sparse ? (const int32_t *)selected->ptr : NULL,
                    sparse ? (const int32_t *)counts->ptr : NULL,
                    n_tokens, n_head, n_kv_head, head_dim,
                    pos0, cache_cap, max_selected, sparse ? 1u : 0u, scale);
            ...
            qwen4exp_qsa_attention_combine_kernel<<<
                    ...
                    (float *)out->ptr, n_tokens, n_head, head_dim);
            return cuda_ok(cudaGetLastError(),
