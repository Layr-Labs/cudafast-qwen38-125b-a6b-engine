...
static int qwen4exp_cuda_gdn_run(
    ...
    if (replay_gates) {
        qwen4exp_gdn_replay_gates_fold_kernel<2u><<<
                dim3(n_value_head, QWEN4EXP_GDN_DIM / (4u * 2u), n_rows),
                QWEN4EXP_GDN_DIM, 0, stream>>>(
 ...
 *              qwen4exp_gdn_replay_gates_fold_kernel<2u> at grid.y / 2u -- the
 *              same bytes from the same addresses with 2 value rows per warp, so
 *              gd[c] is still its ceiling and gd[r] is directly comparable to
 *              the base tree's reading of the same leg.  A drop in (c-r)/r
 *              against base IS this arm working; or vs oc says whether 2 rows
...
static int qw_gdn_bw_probe(int *real_gbs, int *copy_gbs,
    ...
    auto launch = [&](int leg, int s) {
        ...
        if (leg == 0) {
            qwen4exp_gdn_replay_gates_fold_kernel<2u><<<
                    dim3(grid.x, grid.y / 2u, grid.z), threads>>>(
                    out, st, ck, tape, qkv,
    ...
    *copy_gbs = time_leg(1);
    int occ = -1;
    if (cudaOccupancyMaxActiveBlocksPerMultiprocessor(
            &occ, qwen4exp_gdn_replay_gates_fold_kernel<2u>, (int)threads, 0) ==
        cudaSuccess)
        *occ_real = occ;
    else
        (void)cudaGetLastError();
