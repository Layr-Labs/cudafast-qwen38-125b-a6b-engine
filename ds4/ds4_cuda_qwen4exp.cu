...
#pragma unroll
     ...
     * QW_DOWN_L2_WIDTH times over as the window slides across it -- a repeat of
     * a line already resident or already in flight is dropped by the L2, so the
     * cost of the repeats is the issue slots and nothing else.  The distance is
     * ONE named constant used by both the prologue and the loop, so there is no
     * way to move one and forget the other.
     *
