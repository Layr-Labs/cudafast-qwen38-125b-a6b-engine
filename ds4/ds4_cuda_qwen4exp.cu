...
/* The scatter also carries the grouped path's out-of-range-pair zero fill.
 *
 * That fill used to be its own launch, qwen4exp_moe_zero_invalid_kernel,
 * with one block PER PAIR (n_pairs blocks: 4096 at a 512-token prefill
 * chunk), every one of which loads one `selected` entry and exits, because
 * the router never emits an out-of-range id.  The scatter thread that owns
 * pair p already loads that entry and already branches on the very same
 * predicate, so it now writes the pair's zero mid row itself on the branch
 * it used to return from.  Same predicate, same row address
 * (token * mid_token_stride + slot * mid_dim), same 0.0f stores; the grouped
 * gate/up kernel that writes mid never visits such a pair and runs after
 * this launch in stream order, as it ran after the separate fill.
 * DS4_QWEN4EXP_SEPARATE_INVALID_FILL restores the separate launch. */
template <bool ZeroInvalid>
__global__ static void qwen4exp_moe_group_scatter_kernel(
    ...
    if (e < 0 || (uint32_t)e >= n_total_expert) {
        if (ZeroInvalid) {
            ...
            float *dst = mid + (uint64_t)token * mid_token_stride +
                         (uint64_t)slot * mid_dim;
            for (uint32_t r = 0; r < mid_dim; ++r) dst[r] = 0.0f;
        }
        return;
    }
...
static int qwen4exp_routed_moe_cuda(
    ...
    if (fuse_router) {
        ...
        /* fuse_router requires small_group, so this branch is exactly the
         * !small_group case the separate zero fill was gated on. */
        const bool fold_invalid =
            getenv("DS4_QWEN4EXP_SEPARATE_INVALID_FILL") == NULL;
        if (fold_invalid) {
            qwen4exp_moe_group_scatter_kernel<true>
                    <<<pair_blocks, threads, 0, stream>>>(
                    sc.pairs, sc.cursor, (const int32_t *)selected->ptr,
                    n_total_expert, n_pairs, (float *)mid->ptr,
                    n_expert_used, mid_dim, mid_token_stride);
        } else {
            qwen4exp_moe_group_scatter_kernel<false>
                    <<<pair_blocks, threads, 0, stream>>>(
                    sc.pairs, sc.cursor, (const int32_t *)selected->ptr,
                    n_total_expert, n_pairs, (float *)mid->ptr,
                    n_expert_used, mid_dim, mid_token_stride);
