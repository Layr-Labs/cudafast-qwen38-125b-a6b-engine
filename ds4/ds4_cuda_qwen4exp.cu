...
#pragma unroll
            ...
            uint32_t j = 0;
            /* Keep eight dense V rows in flight.  Forming row pointers before
             * the channel loads is the GB10-measured source shape; each head's
             * FMA chain still consumes keys in ascending order. */
            if (!sparse) {
                for (; j + 8u <= n_in_tile; j += 8u) {
                    const float *v0 = v_cache +
                        (uint64_t)keys[j] * kv_stride + (uint64_t)kv_head * head_dim;
