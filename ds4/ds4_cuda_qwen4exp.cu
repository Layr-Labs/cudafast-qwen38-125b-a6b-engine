...
__global__ static void qwen4exp_qsa_indexer_select_kernel(
    ...
    /* The top-k list is a set of distinct block indices, so resolving it in
     * ascending order is a bitmap scan, not a sort: set one bit per admitted
     * candidate, prefix-sum the per-word popcounts, then emit each set bit at
     * its rank.  That replaces the bitonic network's O(log^2 width) block-wide
     * barrier stages with a handful.  The bitmap needs n_blocks/32 words of
     * shared memory past the ids[] array; past 65536 blocks the bitonic path
     * below stays as the fallback. */
    const uint32_t n_words = (n_blocks + 31u) >> 5;
    if (n_words <= 2048u) {
        uint32_t *bits = (uint32_t *)(ids + sort_width);
        ...
        /* Per-thread chunk sums, then a serial exclusive scan by thread 0.
         * ids[] doubles as the partials array: nth <= sort_width always. */
        const uint32_t chunk = (n_words + nth - 1u) / nth;
        const uint32_t w0 = tid * chunk;
        const uint32_t w1 = (w0 + chunk < n_words) ? w0 + chunk : n_words;
        uint32_t mine = 0;
        for (uint32_t w = w0; w < w1; w++) mine += (uint32_t)__popc(bits[w]);
        ids[tid] = (int32_t)mine;
        __syncthreads();
        if (tid == 0u) {
            uint32_t run = 0;
            for (uint32_t t = 0; t < nth; t++) {
                const uint32_t c = (uint32_t)ids[t];
                ids[t] = (int32_t)run;
                run += c;
            }
            n_valid = run;
        ...
        uint32_t rank = (uint32_t)ids[tid];
        for (uint32_t w = w0; w < w1; w++) {
            uint32_t word = bits[w];
            while (word != 0u) {
                const uint32_t b = (uint32_t)__ffs((int)word) - 1u;
                word &= word - 1u;
                if (rank < sort_width) ids[rank] = (int32_t)(w * 32u + b);
                rank++;
            }
        }
        __syncthreads();
...
extern "C" int ds4_gpu_qwen4exp_qsa_indexer_select_tensor(
    ...
    /* The kernel's bitmap path needs n_blocks/32 words past the ids[] array;
     * it only runs when that fits the 2048-word cap, else the bitonic path
     * uses ids[] alone. */
    const uint32_t n_words = (n_blocks + 31u) >> 5;
    const size_t smem = (size_t)sort_width * sizeof(int32_t) +
        (n_words <= 2048u ? (size_t)n_words * sizeof(uint32_t) : 0u);
    qwen4exp_qsa_indexer_select_kernel<<<n_tokens, nth,
        smem, cuda_decode_stream()>>>(
