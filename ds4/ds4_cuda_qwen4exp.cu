...
/* Elements a thread stages before using any.  Every caller norms 2560 over a
 * block of 256: ten steps for every thread, and ten stages the walk whole. */
#define QWEN4EXP_RMS_STEPS 10u

/* ONE global-latency chain, not two.  The grid is four hyper-connection
 * streams by a decode row count, so eight blocks of 256 on a 48-SM part:
 * nothing here is bandwidth-bound and the cost is round trips on the critical
 * path.  There were two, because the sum walk read x and the normalise walk
 * then read x AGAIN plus w.  Both are staged in the SAME batch before the
 * reduction, so the walk after it issues no load and that chain is gone.
 *
 * EXACTNESS.  `sum` still accumulates x[0]^2 .. x[9]^2 ascending into one
 * accumulator and the output is still `x * scale * (weight_bias + w)`, every
 * value from the address it came from before.  Callers do alias (`out == x`
 * for the in-place PLE key norm) and that is unaffected: the batch already
 * loaded all ten before storing any, and now loads them earlier still.  w is
 * read-only weight map and never aliases.  `st` is min(steps, depth), so any
 * longer walk falls to the same ascending one-at-a-time tails. */
__global__ static void qwen4exp_rms_norm_kernel(
    ...
    const uint32_t st =
        steps < QWEN4EXP_RMS_STEPS ? steps : QWEN4EXP_RMS_STEPS;
...
#pragma unroll
    for (uint32_t u = 0; u < QWEN4EXP_RMS_STEPS; u++) {
        if (u < st) {
            ...
            yg[tid + u * nth] = normed * (weight_bias + wv[u]);
        }
    }
    for (s = st; s < steps; s++) {
