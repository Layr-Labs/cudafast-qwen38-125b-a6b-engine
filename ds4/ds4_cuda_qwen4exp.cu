        ...
        for (uint32_t kc = 0; kc < groups; kc += QW_MMA_G) {
            ...
            for (uint32_t idx = tid; idx < QW_DOWN_MMA_BM * QW_MMA_G;
                ...
                if (orow < out_dim && g < groups) {
                    ...
                    if (w_dq) {
                        uint32_t raw[6];
                        /* Keep each decoder's raw pointer known at its call
                         * site. A raw-or-NULL pointer select makes nvcc place
                         * this six-word array in addressable local storage.
                         * The separate calls retain the alignment fallback
                         * while letting the aligned path keep raw in registers. */
