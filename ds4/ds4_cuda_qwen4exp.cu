...
#undef QWEN4EXP_DOWN_MMA
    ...
    /* A one-token routed call has only one live accumulator row.  Select the
     * existing R=1 down instantiation at the ranked shape; two-token verify
     * keeps R=2 even though both widths share the outer tile selector. */
    const bool single_down = n_tokens == 1u && in_dim == 2560u &&
        mid_dim == 640u && out_dim == 2560u && specialize &&
        (down_slab->type == DS4_QWEN4EXP_TY_q5_1 ||
         down_slab->type == DS4_QWEN4EXP_TY_q8_0) &&
        getenv("DS4_QWEN4EXP_NO_ROUTED_DOWN_R1") == NULL;
    if (single_down) { QWEN4EXP_DOWN(1); }
