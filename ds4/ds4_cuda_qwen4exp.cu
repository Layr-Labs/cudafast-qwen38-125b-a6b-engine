...
#pragma unroll
    ...
    const bool pair_masked = n_pairs <= 32u;
    uint32_t pair_mask = 0u;
    int32_t count = 0;
    if (e < n_expert) {
        if (pair_masked) {
            for (uint32_t p = 0; p < n_pairs; p++) {
                if (selected[p] == (int32_t)e) pair_mask |= 1u << p;
            }
            count = (int32_t)__popc(pair_mask);
        } else {
            for (uint32_t p = 0; p < n_pairs; p++) {
                count += selected[p] == (int32_t)e;
            }
...
#pragma unroll
    ...
    if (e < n_expert) {
        ...
        int32_t at = offset;
        if (pair_masked) {
            for (uint32_t mask = pair_mask; mask != 0u; mask &= mask - 1u) {
                const uint32_t p = (uint32_t)__ffs((int)mask) - 1u;
                pairs[at++] = (int32_t)p;
            }
        } else {
            for (uint32_t p = 0; p < n_pairs; p++) {
