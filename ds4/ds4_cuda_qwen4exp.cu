...
#pragma unroll
    ...
    if (Stage) {
        ...
        /* Steps 1 .. DIST-1 are deliberately left UNPREFETCHED here.  The
         * variant that covers them issues QW_DOWN_L2_DIST-1 whole-panel
         * requests back to back at kernel entry, before a single step of
         * arithmetic has run -- a burst of DIST-1 panels per block, which at
         * several blocks per SM is a large simultaneous ask and can evict the
         * very lines the first steps are about to read.  Dropping the prologue
         * trades the first DIST-1 steps (which run unprefetched, exactly as in
         * the base tree) for a steady one-request-per-step rate from the loop
         * below.  This is a scheduling change only: the prefetch writes no
         * register and moves nothing into shared memory either way, so the
         * emitted values are identical in both variants. */
    }
