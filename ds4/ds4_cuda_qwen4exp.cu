...
/* build record 20260920T062532Z-69 */
...
/* S1 step 3c.  The joint Q/KV prep grid is dim3(n_head + n_head_kv, rows) and
 * the kernel is generic in rows (grid.y is the token, every block bound-checks
 * `token >= n_tokens`), so the `rows <= 2u` below was a decode-width gate, not
 * a capability one: at three rows graph.inc entered this helper and fell
 * straight back out to the two separate prep launches.  The kernel's PDL
 * trigger states its single-wave condition on the grid itself
 * (`gridDim.x*gridDim.y*gridDim.z <= 96u`, ds4_cuda_qwen4exp.cu:13999) and a
 * three-row decode grid is 26 x 3 = 78 blocks, so the three-row launch keeps
 * the single wave the deadlock rule asks for.  Per-head, per-token arithmetic
 * is identical to the two-row launch, so row r at three rows is the value the
 * two-row launch gives it.  DS4_QWEN4EXP_NO_W3_PREP=1 restores `rows <= 2u`. */
static int qw4_w3_prep_on(void) {
    static int on = -1;
    if (on < 0) on = getenv("DS4_QWEN4EXP_NO_W3_PREP") == NULL ? 1 : 0;
    return on;
}
...
extern "C" int ds4_gpu_qwen4exp_qsa_prep_joint_dpos_tensor(
    ...
    bool joint=(rows<=2u || (rows==3u && qw4_w3_prep_on())) &&
               (dim&(dim-1u))==0u &&
