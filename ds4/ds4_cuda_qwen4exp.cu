...
__global__ static void qwen4exp_qsa_output_gate_quant_kernel(
        ...
        uint32_t     n_values) {
    /* PDL producer for the state-out projection that follows on the stream,
     * the role and the gate its doubled twin below already carries.  The
     * geometry is the twin's exactly, so the single-wave condition the
     * deadlock rule asks for holds here for the same reason it holds there;
     * gridDim.x <= 48 excludes every prefill width.  The PDL=false
     * instantiation keeps the shipped kernel; DS4_QWEN4EXP_NO_QSA_GATE_PDL
     * selects it. */
