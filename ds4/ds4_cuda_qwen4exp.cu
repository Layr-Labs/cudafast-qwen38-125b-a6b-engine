 ...
 *
 * DEFAULTED OFF, BECAUSE THE PROBE MEASURED IT AS A LOSS.  The bw[] boot leg
 * times three fills of this exact panel on this box:
 *
 *     bw[ld=223 cp=212 re=207 fo=6 s=8]     GB/s
 *
 * ld = the shipped register-staged fill (ld.global.v4 -> st.shared.v4), cp =
 * the cp.async.ca.shared.global fill the #if arm below installs, re = this
 * kernel as it actually ships, i.e. WITH the cp.async arm.  cp is 4.9% SLOWER
 * than ld on the same bytes to the same addresses, and the argument written
 * into the #if arm below -- one instruction instead of two, no destination
 * register in a register-capped kernel -- predicted the opposite.  The
 * mechanism was inherited from the routed DOWN panel in this same file on the
 * theory that it transfers; the measurement says it does not.
 *
 * Scale: the routed gate/up pair moves 1.77 GB a decode token and the round is
 * 42.15 ms, so 223/212 on that stream is 8.55 ms -> ~8.13 ms = ~0.42 ms a
 * token = ~1.0% of decode = ~75 bips on the composite, which I quote as ~61
 * bips against the pessimistic 51.37 ms round.
 *
 * WHAT IS NOT SETTLED, stated because it bounds the claim: that reading is
 * n=1, and 223 vs 212 is close to this instrument's own paired repeatability,
 * so the DIRECTION is what I am acting on, not the 61.  The ld leg is the
 * invariant panel-fill leg the whole probe normalises against, so every draw
 * of this tree republishes the triple and the series is free.
 *
 * Bit-exactness: the #else arm below writes the SAME 16-byte words from the
 * same source addresses to the same shared slots, and a copy engine cannot
 * change a value.  The two arms differ only in which instruction moves the
 * bytes; the decoder, the walk and the reduction after __syncthreads() are one
 * shared body. */
