 ...
 * environment between forwards, so each resolves once and caches the way
 * ds4_qwen4exp_pdl_enabled() and cuda_q8_mma_pipe_enabled() already do,
 * instead of walking the environ list on every MoE dispatch. */
static int qwen4exp_forced_moe_r(void) {
    static int v = -2;
    if (v == -2) {
        const char *forced = getenv("DS4_QWEN4EXP_MOE_R");
        const int r = forced ? atoi(forced) : 0;
        v = (r == 1 || r == 2 || r == 4 || r == 8) ? r : -1;
    }
    return v;
}
/* DS4_QWEN4EXP_MOE_R also gates whole dispatch paths on PRESENCE alone
 * (an invalid value still disables them), so presence gets its own cached
 * read rather than folding into the valid-value helper above. */
static int qwen4exp_moe_r_set(void) {
    static int v = -1;
    if (v < 0) v = getenv("DS4_QWEN4EXP_MOE_R") != NULL;
    return v;
}
static int qwen4exp_no_wide_verify(void) {
    static int v = -1;
...
static int qwen4exp_shared_stage_ok(const void *quant, uint32_t groups,
    ...
    if (qwen4exp_moe_r_set()) return 0;
...
static int qwen4exp_moe_tile(uint32_t n_rows) {
    const int forced = qwen4exp_forced_moe_r();
    ...
    if (n_rows == 4u && !qwen4exp_no_wide_verify()) return 2;
    ...
    if (n_rows == 3u && !qwen4exp_no_wide_verify()) return 2;
    return 8;
}
