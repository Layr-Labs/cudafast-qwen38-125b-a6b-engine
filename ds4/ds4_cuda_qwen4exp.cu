...
qwen4exp_moe_gateup_split_kernel(
        ...
        uint32_t mid_token_stride,
        uint32_t n_expert_used) {
    /* PDL consumer.  The activation quantizer is this kernel's immediate
     * stream predecessor at the decode widths the split schedule takes, and
     * it already triggers programmatic launch completion there -- until now
     * that trigger fired into a plainly-launched gate/up and was wasted.  The
     * fence is the first statement, so every global read in the body (active,
     * counts, pairs, xq/xs/xsum and the weight rows whose addresses depend on
     * `expert`) is ordered after the producer's full completion and flushed
     * writes.  No pointer in the signature is __restrict__-qualified, so no
     * load here is an ld.global.nc the fence would fail to order (the .nc
     * rule, ds4_cuda_qwen4exp.cuh).  A plainly-launched instantiation reaches
     * a fence with nothing to wait for, which is a no-op. */
    QWEN4EXP_PDL_SYNC();
    const uint32_t lane = threadIdx.x & 31u;
    const uint32_t warp = threadIdx.x >> 5u;
...
#undef QWEN4EXP_GATEUP_MMA_IMPL
     ...
     * keep their prior kernel. */
        ...
        /* PDL.  The activation quantizer is the immediate stream predecessor
         * of this launch and it ALREADY triggers programmatic launch
         * completion at these widths -- until now that trigger fired into a
         * plainly-launched gate/up, so it bought nothing.  Relaxing the edge
         * here lets this kernel's blocks come up and reach their first weight
         * loads while the quantizer's one-warp blocks are still retiring; the
         * fence at the top of the body carries the data edge.
         *
         * The arm is gated on exactly the producer's own predicate -- narrow
         * kernel, one wave -- so a width where the trigger does not fire keeps
         * the plain launch byte for byte.  The deadlock rule is about the
         * TRIGGER HOST, not the consumer: the quantizer's grid is
         * (xgroups, n_tokens) = 160 one-warp blocks at decode, single-wave, so
         * it has no later wave for this kernel's blocks to starve, and this
         * kernel's own 3200 blocks are irrelevant to it. */
        const bool gu_pdl =
            n_tokens < (uint32_t)QWEN4EXP_QUANT_WIDE_MIN_ROWS &&
            (uint64_t)xgroups * (uint64_t)n_tokens <= 768u;
#define QWEN4EXP_SPLIT_GATEUP(V, P, C) do { \
        const dim3 qw_gu_grid((mid_dim + P - 1u) / P, gu_rows, 1); \
        if (gu_pdl) { \
            QWEN4EXP_LAUNCH_PDL( \
                (qwen4exp_moe_gateup_split_kernel<2, DS4_QWEN4EXP_TY_q4_K, \
                                                  V, P, C>), \
                qw_gu_grid, P * 64u, 0, stream, \
                (float *)mid->ptr, gate, up, sc.xq, sc.xs, sc.xsum, \
                sc.pairs, sc.counts, sc.offsets, gu_active, \
                (const float *)weights->ptr, \
                gate_slab->expert_bytes, gate_slab->row_bytes, \
