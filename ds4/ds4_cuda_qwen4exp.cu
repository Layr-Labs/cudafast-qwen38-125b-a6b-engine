 ...
 * kernel-limits probe reports the consequence on this part; these readings
 * are from builds of this same tree:
 ...
 * whole binding constraint, and halving the rows again halves it again.
 *
 * At two rows the block is 128 threads, so the same ceiling admits TWELVE.
 * The other two limits stay slack and by a wider margin than at four: the
 * panel is 2 * 2 * 90 * 16 = 5,760 B, so shared memory allows
 * 101,376 / 5,760 = 17, and registers 32 * 128 = 4,096 allow
 * 65,536 / 4,096 = 16.  The thread ceiling is still the binding one.
 *
 * The grid grows to match -- (mid_dim + P - 1) / P is 320 blocks at two,
 * 160 at four, 80 at eight -- so the same warps do the same work, packed into
 * narrower blocks that the scheduler can actually co-resident.  mid_dim is
 * 640, which two divides exactly, so no block takes a partial panel.
 *
 * The non-cooperative vector arm below already sits at the end of this same
 * gradient at one row per block, where its comment records four rows as a
 * full percent slower than two.
 *
