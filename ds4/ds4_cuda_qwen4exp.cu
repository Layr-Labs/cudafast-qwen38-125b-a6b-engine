...
qwen4exp_hc_up_mix_mma_kernel(
    ...
    /* PDL consumer of qwen4exp_hc_silu_quant_kernel, which is the immediately
     * preceding launch on the decode stream and which already triggers at its
     * top under `pairs <= 20u`.  That trigger had no consumer: this kernel was
     * launched plainly, so the trigger fired into nothing and the engine paid
     * the full launch edge anyway.  The launch site's gate is written as the
     * SAME `low_pairs <= 20u` the producer's own trigger reads, so the fence is
     * live exactly on the widths where the producer is provably single-wave --
     * at `pairs <= 20u` its grid is `(pairs + 7) / 8`, at most three blocks, so
     * the deadlock rule (ds4_cuda_qwen4exp.cuh) is satisfied with the whole
     * device to spare.  At prefill widths the plain launch is kept, because
     * there the producer's grid grows with `rows` and is emphatically not
     * single-wave.
     *
     * The fence sits ABOVE the staging `__syncthreads()` in the stage loop, not
     * below it: a barrier hoisted above the fence serializes the wait against
     * work the kernel could already overlap, which measured -39 bips on this
     * engine.  Nothing else is hoisted above it either -- the accumulator zero
     * and the tile index arithmetic below touch no global memory, so there is
     * no load here to displace.
     *
     * None of this kernel's pointers carries __restrict__, so the .nc hazard
     * does not apply: the fence does not order `ld.global.nc`, and a
     * `const __restrict__` activation pointer would have compiled the xq and
     * xscale reads into exactly that. */
    QWEN4EXP_PDL_SYNC();
...
/* The up-mix projection, whose immediate predecessor on the decode stream is
 * qwen4exp_hc_silu_quant_kernel -- a kernel that has triggered programmatic
 * launch completion at its top since the routed-MoE PDL arm landed, with no
 * consumer to receive it.  The trigger fired into nothing and the engine still
 * paid the full launch edge into this kernel.
 *
 * The PSS arm below closes that edge.  Its `low_pairs <= 20u` gate is written
 * to be the SAME predicate the producer's own trigger reads in its body
 * (`pairs <= 20u`), which is what makes the deadlock rule hold rather than
 * hoping it does: at that width the producer's grid is `(pairs + 7) / 8`, at
 * most three blocks, so it is single-wave with the whole 48-SM device to
 * spare and this kernel's blocks cannot starve a later producer wave that
 * does not exist.  Above the gate -- every prefill width, where the producer's
 * grid grows with `rows` -- the plain launch is kept and the fence in the
 * kernel body degenerates to a no-op, so the pre-PDL behaviour is exact.
 *
 * Both call sites sit inside the `if (lowrank_q8)` block that declares
 * `low_pairs`; the macro reads that local deliberately, so a third call site
 * outside that scope would not compile rather than silently mis-gate. */
#define QWEN4EXP_HC_UP_MIX_LAUNCH(WM, WN, MT, G)                               \
    do {                                                                       \
        const unsigned bm = (unsigned)((WM) * (MT) * 16);                      \
        const unsigned bc = (unsigned)((WN) * 8);                              \
        const dim3 qw_um_grid((rows + bm - 1u) / bm,                           \
                              (n_embd + bc - 1u) / bc, 1u);                    \
        if (low_pairs <= 20u) {                                                \
            QWEN4EXP_LAUNCH_PDL(                                               \
                    (qwen4exp_hc_up_mix_mma_kernel<WM, WN, MT,                 \
                                                   QWEN4EXP_HC_UP_MIX_NT, G>), \
                    qw_um_grid, (WM) * (WN) * 32, 0, cuda_decode_stream(),     \
                    (float *)mixed->ptr, (const unsigned char *)upw, xq,       \
                    xscale, (const float *)hyper->ptr, nscale, normw, n_embd,  \
                    rows, n_lowrank / 32u, weight_bias, round_bf16);           \
        } else {                                                               \
            qwen4exp_hc_up_mix_mma_kernel<WM, WN, MT,                          \
                                          QWEN4EXP_HC_UP_MIX_NT, G>            \
                <<<qw_um_grid, (WM) * (WN) * 32, 0, cuda_decode_stream()>>>(   \
