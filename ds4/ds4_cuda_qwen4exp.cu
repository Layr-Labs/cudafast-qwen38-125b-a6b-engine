...
#pragma unroll
    ...
    case (uint32_t)DS4_QWEN4EXP_TY_q4_K: {
        ...
        if (qwen4exp_word_aligned(qs)) {
            /* Same eight words as the scalar word loop; uint4/uint2 cuts the
             * instruction count exactly as qw_raw_load does for the MMA path. */
            uint32_t qw[8];
            qw_load_words8((const uint32_t *)(const void *)qs, qw);
#pragma unroll
            for (int i = 0; i < 8; i++) {
...
#pragma unroll
    ...
    case (uint32_t)DS4_QWEN4EXP_TY_q5_K: {
        ...
        if (qwen4exp_word_aligned(qs) && qwen4exp_word_aligned(xb->qh)) {
            uint32_t qw[8], hw[8];
            qw_load_words8((const uint32_t *)(const void *)qs, qw);
            qw_load_words8((const uint32_t *)(const void *)xb->qh, hw);
...
#define QWEN4EXP_SH_GATEUP(R) \
    qwen4exp_shared_gateup_q_kernel<R> \
        <<<dim3((mid_dim + 7u) / 8u, tiles, 1), threads, 0, stream>>>( \
            ...
            gate_slab->row_bytes, up_slab->row_bytes, \
            gate_slab->type, up_slab->type, xgroups, mid_dim, n_tokens)
    /* Decode/verify R=2 shared gate/up: register split matching routed Q4.
     * Production shared experts are Q8_0; Q4_K covers the MoE suite. */
