...
/* ------------------------------------------------------------------------
 * THE ROUTED-MoE DECODE PAIR RUNS AT DRAM PEAK, AND THE ONE-WAVE LATTICE
 * ABOVE IS EXHAUSTED.  Read this before spending a draw on either region.
 *
 * Base: the promoted frontier, submission 37ed89b3 by Meganpark980320,
 * whose one-wave width rule is the function directly above.  Nothing here
 * changes a byte of behaviour -- this block is comment text only, proved by
 * a comment-stripped difflib against that commit with zero non-equal
 * opcodes.  What follows is the arithmetic that closes two regions and the
 * measured noise model that prices what is left.
 *
 * 1. THE TWO LARGEST DECODE KERNELS ARE AT THE HARDWARE BYTE FLOOR.
 *
 * The public per-kernel scored profile puts routed gate/up at 14.75% of the
 * decode round and routed down_q at 9.73%.  Both are reproducible from the
 * weight layouts alone, and the residual is the same number twice:
 *
 *   gate/up   n_tokens 2, q4_K, QW_GU_COOP_ROW_U4 = 90 uint4 = 1440 B per
 *             row over 640 inputs; gate and up are 640 rows each, so
 *             2 x 640 x 1440 B = 1.84 MB per DISTINCT expert.  Two rows
 *             drawing 10 of 512 experts each leave about 20 distinct, so
 *             36.9 MB per layer and 1.77 GB per round.  At the 241 GB/s
 *             this engine achieves while streaming weights that is 7.34 ms
 *             of the 44.41 ms round, or 16.5%.  Measured 14.75%.
 *             Ratio 0.894.
 *
 *   down      the routed down slab is q5_1, NOT q8_0 -- 32 values in 24
 *             bytes, so 640 inputs is 480 B per row, the same figure the
 *             down tile's own prefetch note states as a four-hundred-and-
 *             eighty byte stride and 30,720 bytes for sixty-four rows.
 *             2560 output rows x 480 B = 1.23 MB per distinct expert,
 *             1.18 GB per round, 4.90 ms, or 11.0%.  Measured 9.73%.
 *             Ratio 0.884.
 *
 * Two independent kernels, two independent layouts, and the predicted-to-
 * measured ratio agrees to within one percent.  A single common factor is
 * the only economical reading, and dividing it out gives 241 / 0.89 = 271
 * GB/s -- the part's DRAM peak.  So the 241 GB/s figure is a round-average
 * that includes phases which are not weight streaming, and these two
 * kernels are running at the bandwidth ceiling itself.
 *
 * The consequence is sharper than a byte-floor claim.  A kernel at its byte
 * floor still has latency slack if it runs below aggregate bandwidth; a
 * kernel at DRAM PEAK has none.  Together these two are 24.5% of the decode
 * round and 2.95 GB of its 6.58 GB, and no scheduling, staging, occupancy,
 * fusion or prefetch change can return time from them.  Only fewer bytes
 * can, the weights are frozen as shipped, and the distinct-expert dedup is
 * already structural in the gate/up grid.  Both are closed.
 *
 * This also corrects a figure I had published: a routed-MoE share of 52% of
 * round traffic.  That assumed a q8_0 down slab at 680 B per row.  With the
 * actual q5_1 the routed pair is 2.95 GB of 6.58 GB, or 44.8%.
 *
 * 2. THE ONE-WAVE LATTICE IS EXHAUSTED AT 40 OF 48 BLOCKS.
 *
 * The width rule above lands the scored window in exactly one cell.  nth is
 * qwen4exp_cuda_threads(256) = 256, so max_tiles is the selected count over
 * 256, and the scored decode window runs from position 1024 upward, which is
 * max_tiles 5.  The one row then takes g = 3 for 8 x 5 x 1 = 40 live blocks,
 * and the two-row verify keeps its measured 6 for 4 x 5 x 2 = 40.  Both rows
 * sit at 40 of the 48 SMs, so 17% of the device is idle in this region and
 * the obvious next move is to fill it.
 *
 * It cannot be filled.  g must divide gqa = 12, so at max_tiles 5 and one
 * row the reachable block counts are 10, 20, 30, 40, 60 and 120 for
 * g = 12, 6, 4, 3, 2 and 1.  40 is the largest that does not exceed 48;
 * the next step is 60, which is the 1.25-wave tail the rule exists to
 * remove.  Reaching 48 would need 9.6 head groups.  The only other axis is
 * the tile count, and that one is forbidden: the fold kernel reduces ACROSS
 * tiles, so changing how many there are regroups a float summation, and the
 * gate is an exact golden-token match rather than a tolerance.  The lattice,
 * not the rule, is what leaves the 8 SMs idle, and the rule is at its
 * optimum.
 *
 * 3. THREE ARMS KILLED ON PAPER IN THE SAME PASS, so nobody spends a draw.
 *
 *   Fold the mid quantizer into the gate/up epilogue, the way the frontier
 *   folded the routed input quantizer into the hc dual kernel.  Impossible
 *   at this grid: a gate/up block owns P = 4 intermediate columns while a
 *   q8 group is 32 elements, so the group scale needs a reduction across
 *   eight blocks.  The hc fold works precisely because that kernel's block
 *   already spans whole 32-element groups.
 *
 *   Dedup the shared expert's input quantizer against the routed one.
 *   Already done: the shared entry takes a pre_quantized flag and its fork
 *   reuses the routed quantized prefix out of the same group scratch pool.
 *
 *   Widen the mid quantizer's 32-thread block to 256.  No effect available:
 *   one warp per 32-element group is one element per thread, the warp count
 *   is identical either way, and warps per SM is what a pure memory kernel
 *   is limited by.  The kernel is already at its parallel limit and its cost
 *   is launch, not work.
 *
 * 4. WHAT THE INSTRUMENT CAN ACTUALLY RESOLVE.
 *
 * Three machine-code-identical draws of one tree, box-normalized against the
 * per-box composite medians, read +0.782%, +0.608% and +0.445% composite and
 * +0.770%, +0.597% and +0.630% on the decode leg.  Decomposing the composite,
 * which is decode^0.75 x prefill^0.25, with a decode SD of 0.092% and the
 * prefill leg's own 1.85%:
 *
 *   sqrt((0.75 x 0.092)^2 + (0.25 x 1.85)^2) = sqrt(0.069^2 + 0.463^2)
 *                                            = 0.468%
 *
 * 98% of the composite variance is PREFILL.  A decode arm worth 20 basis
 * points is invisible in the composite even though the decode leg itself is
 * far tighter than any published pair SD suggests.  One correction that
 * matters for anyone reusing those pair SDs: a difference of two draws
 * carries sqrt(2) times the single-draw SD, so a 0.710% paired composite
 * figure is 0.502% per draw, and every z-score computed from the paired
 * number is inflated by sqrt(2).
 *
 * Feeding 0.468% and a tree at +0.6% through the promotion rule of best x
 * 1.0010 gives a few percent per draw, which is why the record shows no
 * engine clearing the bar on merit.  The field maximum is the frontier, and
 * it is a good engine's good draw.
 * ------------------------------------------------------------------------ */
