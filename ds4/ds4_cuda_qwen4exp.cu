 ...
 *
 * MEASURED OFF, 2026-09-23.  The boot probe in this same file timed the
 * register-staged fill at 223 GB/s against 212 GB/s for the cp.async form
 * at identical shape and occupancy (bw[ld=223 cp=212]) -- 5.2% SLOWER.
 * The routed DOWN panel and the GDN projection panel, both of which run
 * the async form, issue their copies BEFORE a grid-dependency fence or
 * across a double-buffered step, so the copies overlap something real.
 * This kernel reaches QWEN4EXP_PDL_SYNC() long before it fills, because
 * the panel address depends on active[]/counts[] that the predecessor
 * produces, so the fill cannot be hoisted above the fence and there is
 * nothing left to overlap -- only added commit/wait.  1 restores it. */
#ifndef DS4_GU_COOP_CPASYNC
#define DS4_GU_COOP_CPASYNC 0
#endif
...
/* The R2 coarse-screen rate instrument.  It lives in ds4_cuda.cu because the
 * kernel it times does: ds4_cuda_mtp_native.cuh is included there, and only
 * there, so timing the REAL kernel rather than a copy of it means crossing the
 * translation unit rather than duplicating the source.  Both objects are in
 * CORE_OBJS, and ds4_cuda.cu is the unit built WITH --use_fast_math, which is
 * how the screen kernel actually ships -- a copy compiled under this file's
 * -prec-div/-ftz=false flags would not be the kernel being measured. */
#if DS4_GU_BW_PROBE_BUILD
extern "C" int ds4_gpu_mtp_hd_bw_probe(int *sc_gbs, int *lo_gbs, int *dn_gbs,
                                       int *sc2_gbs, int *occ_sc, int *occ_lo,
                                       int *rf_gbs, int *ru_gbs, int *occ_rf);
#endif
...
#endif
    int hd_sc = -1, hd_lo = -1, hd_dn = -1, hd_v = -1;
    int hd_os = -1, hd_ol = -1, hd_rows = 0;
    int hd_rf = -1, hd_ru = -1, hd_orf = -1;
#if DS4_GU_BW_PROBE_BUILD
    /* Same latch discipline and the same stand-down convention as the two legs
     * above: the probe runs unless DS4_HD_BW_PROBE is explicitly "0", and a
     * second caller gets the cached numbers rather than ~30 ms of GPU work on a
     * path that might be timed. */
    {
        static int hd_done = 0;
        static int hd_c[10] = {-1, -1, -1, -1, -1, -1, -1, -1, -1, 0};
        if (!hd_done) {
            hd_done = 1;
            const char *const hd_env = getenv("DS4_HD_BW_PROBE");
            if (hd_env == NULL || hd_env[0] != '0')
                hd_c[9] = ds4_gpu_mtp_hd_bw_probe(&hd_c[0], &hd_c[1], &hd_c[2],
                                                  &hd_c[3], &hd_c[4], &hd_c[5],
                                                  &hd_c[6], &hd_c[7], &hd_c[8]);
        }
        hd_sc = hd_c[0]; hd_lo = hd_c[1]; hd_dn = hd_c[2]; hd_v = hd_c[3];
        hd_os = hd_c[4]; hd_ol = hd_c[5];
        hd_rf = hd_c[6]; hd_ru = hd_c[7]; hd_orf = hd_c[8]; hd_rows = hd_c[9];
    }
#endif

    /* hd[] goes FIRST on purpose.  Whatever truncates this string downstream
     * truncates its tail, and the gu5[]/gu8[] tail is a pair of static
     * attribute reads already on record, while hd[] is the only number in here
     * that nobody has. */
    snprintf(buf, sizeof(buf),
             "hd[sc=%d lo=%d dn=%d v=%d os=%d ol=%d r=%d rf=%d ru=%d orf=%d] "
             ...
             hd_sc, hd_lo, hd_dn, hd_v, hd_os, hd_ol, hd_rows,
             hd_rf, hd_ru, hd_orf,
