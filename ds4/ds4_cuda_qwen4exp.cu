...
#pragma unroll
    ...
    if (!scorer) {
        /* The two channels a valuer thread owns are `2*vt` and `2*vt + 1` of
         * the same head row, so their stores are adjacent and the pair is
         * eight-byte aligned (head_dim is even and 2*vt is even).  One
         * vector store retires them together instead of two scalar stores
         * into the same sector.  Same values, same addresses, same guard --
         * each channel still divides the accumulator it always divided, by
         * the same run sum, under the same positivity test. */
#pragma unroll
        for (uint32_t h = 0; h < GROUP; h++) {
