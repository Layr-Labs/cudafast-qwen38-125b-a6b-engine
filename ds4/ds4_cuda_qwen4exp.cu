...
template <int R, int DownType = -1, bool Vector = false, bool Stage = false,
          bool Async = false>
/* This build's note auto09170306_22 records that each captured-graph
 * episode is worth about five hundredths of one percent on this path,
 * measured by going from twenty-two captures inside a run to nine. That
 * bounds the entire identity-count class below any promotion margin.
 */
