...
/* Fixed target geometry; the scalar replay arithmetic is unchanged. */
__global__ static void qwen4exp_gdn_replay_fixed_kernel(
        float *out, float *state, float *checkpoint, float *tape,
        const float *qkv, const float *raw_alpha, const float *raw_beta,
        const float *a_log, const float *dt_bias,
        const uint32_t *control) {
    constexpr uint32_t n_key_head = 16u, n_value_head = 48u, n_tokens = 2u;
    constexpr uint32_t head_layout = 1u, replay_rows = 0u;
    const uint32_t head = blockIdx.x;
    const uint32_t value = blockIdx.y * 4u + (threadIdx.x >> 5u);
    const uint32_t lane = threadIdx.x & 31u;
    if (head >= n_value_head || value >= QWEN4EXP_GDN_DIM) return;
...
static int qwen4exp_cuda_gdn_run(
    ...
    if (replay) {
        if (n_key_head == 16u && n_value_head == 48u && n_tokens == 2u &&
            head_layout == DS4_QWEN4EXP_GDN_HEADS_TILED &&
            getenv("DS4_QWEN4EXP_NO_GDN_REPLAY_FIXED") == NULL) {
            qwen4exp_gdn_replay_fixed_kernel<<<recurrence_grid, QWEN4EXP_GDN_DIM, 0, stream>>>(
                ...
                (const float *)raw_beta->ptr, a_log, dt_bias,
                (const uint32_t *)replay->control->ptr);
        } else {
