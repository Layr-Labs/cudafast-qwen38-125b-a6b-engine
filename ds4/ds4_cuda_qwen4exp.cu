        ...
        for (int r = 0; r < R; r++) {
            if ((uint32_t)r < take) {
                ...
                for (uint32_t g = lane; g < groups; g += 32u) {
                     ...
                     * VectorX follows the kernel's own activation schedule, so
                     * the aligned launch reads the group as two int4 and the
                     * scalar launch keeps qwen4exp_dp4a's word-load form; both
                     * arms of the accumulator issue the same eight dp4a on the
                     * same operand pairs in the same order. */
...
/* The down kernel's Vector schedule has been written since the tile landed and
 * has never been instantiated: every launch below passed false, so `route[]`
 * folded to the constant -1, the __shfl_sync broadcast compiled away, and each
 * group re-read `selected[]` from global and pulled its 32 activation bytes as
 * eight scalar words.  Both are load shape, not arithmetic.  Enabling it reads
 * the ten expert IDs once per lane and broadcasts them, and reads each group as
 * two int4.
 *
 * Bit-identical either way, which is why the schedule is a template parameter
 * and not a variant: the shuffle delivers the same `e` the reload did (lane
 * `slot` holds exactly `selected[t * n_expert_used + slot]`), and the int4 pair
 * feeds dp4a the same eight words in the same ascending order the scalar loads
 * did -- lo.x..lo.w then hi.x..hi.w.  Nothing about the float chain moves.
 *
 * The int4 reads need each group 16-byte aligned.  Every group sits a whole
 * multiple of 32 bytes into the scratch, so that is exactly the condition that
 * the scratch base is 16-byte aligned -- the same test the gate/up launcher
 * already applies to its own activation base.  The scalar schedule stays
 * reachable for an unaligned base and through the env pin. */
#define QWEN4EXP_DOWN(R) do { \
    const bool dn_vector = ((uintptr_t)sc.mq & 15u) == 0u && \
        getenv("DS4_QWEN4EXP_NO_DOWN_VECTOR") == NULL; \
    if (specialize && down_slab->type == DS4_QWEN4EXP_TY_q5_1) { \
        if (dn_vector) { QWEN4EXP_DOWN_IMPL(R, DS4_QWEN4EXP_TY_q5_1, true); } \
        else { QWEN4EXP_DOWN_IMPL(R, DS4_QWEN4EXP_TY_q5_1, false); } \
    } else if (specialize && down_slab->type == DS4_QWEN4EXP_TY_q8_0) { \
        if (dn_vector) { QWEN4EXP_DOWN_IMPL(R, DS4_QWEN4EXP_TY_q8_0, true); } \
        else { QWEN4EXP_DOWN_IMPL(R, DS4_QWEN4EXP_TY_q8_0, false); } \
    } else { \
        if (dn_vector) { QWEN4EXP_DOWN_IMPL(R, -1, true); } \
        else { QWEN4EXP_DOWN_IMPL(R, -1, false); } \
    } \
} while (0)
    if (down_mma) {
