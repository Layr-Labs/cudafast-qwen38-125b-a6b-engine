...
/* Retire every committed group EXCEPT the most recent one.  With two groups in
 * flight this returns once the first has landed and leaves the second running,
 * which is what a two-stage panel fill needs: the consumer can start on stage
 * A's bytes while stage A's issue slots are already spent on stage B. */
...
/* TWO-STAGE PANEL FILL, over the cp.async form.  1 enables.
 *
 * The fill is a PROLOGUE: it is the first thing the block does, so every
 * resident warp is inside it at the same instant and occupancy cannot hide it
 * -- six co-resident blocks all fill together.  The only thing that can hide
 * part of it is the block's OWN compute, which means splitting the panel and
 * consuming the first piece while the second is still in flight.
 *
 * The split point is free and bit-exact.  The group walk below already runs
 *
 *     for (g = lane; g + 32 < groups; g += 64)   -> g and g+32, groups 0..63
 *     for (;         g < groups;      g += 32)   -> groups 64..79
 *
 * at groups = 80, and q4_K packs 8 groups into a 144-byte super-block = 9 uint4,
 * so groups 0..63 are exactly row words 0..71 and groups 64..79 are words
 * 72..89.  Staging 72 words first and 18 second therefore needs NO change to
 * either loop body and NO change to per-lane accumulation order: the loop
 * boundary the code already has falls on a super-block boundary.  Contract 3.4
 * and the reassociation rule are both untouched -- this only moves WHEN bytes
 * arrive, never which bytes or in what order they are summed. */
#ifndef DS4_GU_COOP_PIPE
#define DS4_GU_COOP_PIPE 1
#endif
#define QW_GU_PIPE_GROUPS 64u      /* groups the FIRST walk loop covers at 80 */
#define QW_GU_PIPE_U4 72u                /* = QW_GU_PIPE_GROUPS / 8u * 9u */
#if DS4_GU_COOP_CPASYNC && DS4_GU_COOP_PIPE
#define QW_GU_PIPE_ON 1
#else
#define QW_GU_PIPE_ON 0
#endif
/* The split is only bit-exact because these three agree.  If in_dim, the panel
 * row stride or the walk's stride ever changes, this fails the build instead of
 * silently handing loop 1 a group whose bytes have not landed. */
static_assert(QW_GU_COOP_GROUPS == 80u,
              "the 64/16 walk split assumes groups == 80");
static_assert(QW_GU_PIPE_GROUPS == 64u,
              "loop 1 covers g and g+32 for lanes 0..31, i.e. groups 0..63");
static_assert(QW_GU_PIPE_U4 == QW_GU_PIPE_GROUPS / 8u * 9u,
              "q4_K packs 8 groups per 9-uint4 super-block");
static_assert(QW_GU_PIPE_U4 < QW_GU_COOP_ROW_U4,
              "stage B must be non-empty at q4_K");
...
qwen4exp_moe_gateup_split_kernel(
    ...
    uint32_t wrow = 0u;
    /* Set by the two-stage fill below and cleared by the one barrier that
     * publishes stage B.  Block-uniform by construction -- see the note at the
     * assignment; a per-lane value here would deadlock the barrier. */
    ...
    if (Coop) {
         ...
         * byte for byte. */
#if QW_GU_PIPE_ON
        /* TWO COMMIT GROUPS.  Stage A is the first QW_GU_PIPE_U4 words of every
         * row -- the bytes the first walk loop reads -- and stage B is the rest.
         * The split is PER ROW, not a flat prefix of `words`: the panel is laid
         * out row-major with stride row_u4, so a flat cut would hand row 0 a
         * whole-and-a-bit share and leave row 3 with nothing in stage A.
         *
         * wait1 retires stage A and leaves stage B in flight, so the block
         * starts decoding groups 0..63 with 18 words per row still travelling.
         * Stage B is published between the two walk loops below. */
        {
            (void)words;   /* the per-row split derives its own bounds */
            const uint32_t RU4 = qw_gu_panel<Type>::row_u4;
            /* Only the q4_K row has a 64-group prefix that lands on a
             * super-block boundary.  Any other instantiation stages the whole
             * row in group A and gets an empty group B, i.e. exactly the
             * single-stage fill this replaces. */
            const uint32_t A = (RU4 == QW_GU_COOP_ROW_U4) ? QW_GU_PIPE_U4 : RU4;
            const uint32_t B = RU4 - A;
            for (uint32_t i = threadIdx.x; i < rows_here * A; i += blockDim.x) {
                const uint32_t r = i / A;
                const uint32_t o = r * RU4 + (i - r * A);
                qw_cpasync16(
                    (uint32_t)__cvta_generic_to_shared(&wcoop[o]),
                    (const void *)(gb + (uint64_t)o * 16u));
                qw_cpasync16(
                    (uint32_t)__cvta_generic_to_shared(&wcoop[PanelU4 + o]),
                    (const void *)(ub + (uint64_t)o * 16u));
            }
            qw_cpasync_commit();
            for (uint32_t i = threadIdx.x; i < rows_here * B; i += blockDim.x) {
                const uint32_t r = i / B;
                const uint32_t o = r * RU4 + A + (i - r * B);
                qw_cpasync16(
                    (uint32_t)__cvta_generic_to_shared(&wcoop[o]),
                    (const void *)(gb + (uint64_t)o * 16u));
                qw_cpasync16(
            ...
            qw_cpasync_wait1();
            __syncthreads();
            /* Block-uniform: Coop, QW_GU_PIPE_ON and B are all compile-time or
             * shape-derived, never thread-derived.  Nothing below may make the
             * mid-walk barrier conditional on anything a lane decides. */
            pipe_pending = (B != 0u);
        }
#else
...
#pragma unroll
        ...
        /* HOISTED OUT OF `if (live)`.  The walk is split into its two existing
         * loops so stage B of the panel fill can be published between them, and
         * that publish carries a __syncthreads() which must be reached by every
         * thread in the block -- including the ones with `live == false`, since
         * `live = row < mid_dim` is warp-uniform but NOT block-uniform on the
         * last row-group.  `g` therefore has to outlive the first `if (live)`.
         * Value, initial value and stride are unchanged.  The enclosing block is
         * what keeps `g` from outliving the walk: the epilogue further down names
         * its gate float `g` too, and that name must go on resolving to the
         * float, exactly as it did when the walk variable lived inside
         * `if (live)`. */
        {
        uint32_t g = lane;
        if (live) {
...
#define QWEN4EXP_SPLIT_GROUP(GG, RAWP) do { \
        ...
        /* PUBLISH STAGE B OF THE PANEL FILL.
         *
         * Everything above this point read row words 0..QW_GU_PIPE_U4-1, which
         * are the only words cp.async group A carried.  Group B -- the tail of
         * every row, holding the super-blocks the second walk loop decodes --
         * has been in flight since before the first group was waited on, so the
         * whole of the loop above ran with it travelling underneath.
         *
         * The barrier sits OUTSIDE `if (live)` because `live` is not
         * block-uniform, and `pipe_pending` is block-uniform, so every thread in
         * the block executes this barrier the same number of times. It is
         * cleared on the first pass: at the decode width the enclosing loop runs
         * exactly once (`cnt <= R`), and at prefill the later passes find the
         * whole panel already resident and skip both the wait and the barrier.
         *
         * When the pipeline is off this is dead code the compiler drops --
         * `pipe_pending` is initialised false and never assigned. */
        if (pipe_pending) {
            qw_cpasync_wait0();
            __syncthreads();
            pipe_pending = false;
...
#undef QWEN4EXP_SPLIT_GROUP
        ...
        }   /* end of the walk's scope; `g` does not reach the epilogue */
#pragma unroll
        for (int r = 0; r < R; r++) {
 ...
 * (ds4_cuda_qwen4exp.cuh).  Verify and prefill keep the plain launch. */
     ...
     * checkpoint shape 8 * (2720 + 2720) = 43520 bytes.  It still fits the
     * scored kernel's 101376-byte opt-in shared-memory budget, so do not reuse
     * the routed-down 16 KiB valve here. */
 ...
 *
 *   gd[r=...]  the real qwen4exp_gdn_replay_gates_kernel, the live decode
 *              recurrence (the octet/value-reuse kernels are the wide-chunk
 *              paths; replay_gates is what a decode step takes)
...
static int qw_gdn_bw_probe(int *real_gbs, int *copy_gbs,
    ...
    auto launch = [&](int leg, int s) {
        ...
        if (leg == 0) {
            qwen4exp_gdn_replay_gates_kernel<<<grid, threads>>>(
                    out, st, ck, tape, qkv,
    ...
    *copy_gbs = time_leg(1);
    int occ = -1;
    if (cudaOccupancyMaxActiveBlocksPerMultiprocessor(
            &occ, qwen4exp_gdn_replay_gates_kernel, (int)threads, 0) ==
        cudaSuccess)
        *occ_real = occ;
    else
        (void)cudaGetLastError();
