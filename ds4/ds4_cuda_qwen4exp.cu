...
#pragma unroll
    ...
    if (Stage) {
         ...
         * arithmetic has run, and at several blocks per SM that simultaneous
         * ask can evict the very lines the first steps are about to read.
         * Dropping the prologue trades the first DIST-1 steps -- which then run
         * unprefetched, exactly as in the base tree -- for a steady
         * one-request-per-step rate from the loop below.  Scheduling only: the
         * prefetch writes no register and moves nothing into shared memory
         * either way, so the emitted values are identical. */
    }
