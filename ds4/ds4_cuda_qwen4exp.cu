...
#endif
/* =1 at build time keeps the sparse split path's one-row-at-a-time V walk. */
#ifndef QWEN4EXP_QSA_SPARSE_SERIAL_V
#define QWEN4EXP_QSA_SPARSE_SERIAL_V 0
#endif
...
#if !QWEN4EXP_QSA_SPARSE_SERIAL_V
        /* The sparse path -- every decode and verify row -- used to take
         * only the serial loop below: one V load, then its GROUP FMAs, then
         * the next load, so a lane never had more than one value row in
         * flight.  Here the same VSTEP rows are ASKED FOR together and then
         * consumed in j order; a masked key (keys[j] < 0, probs 0) issues no
         * load and no FMA, exactly as the serial loop's `continue` did, so
         * every contrib[h] sees the same FMAs in the same order.  The mask
         * test reads the shared `keys` row every thread shares, so it is
...
#pragma unroll
                for (uint32_t i = 0; i < VSTEP; i++) {
                    const int32_t kj = keys[j + i];
                    a[i] = kj >= 0 ? vh[(uint64_t)kj * kv_stride] : 0.0f;
                }
                asm volatile("" ::: "memory");
#pragma unroll
                for (uint32_t i = 0; i < VSTEP; i++) {
                    if (keys[j + i] < 0) continue;
#pragma unroll
                    for (uint32_t h = 0; h < GROUP; h++) {
                        contrib[h] = __fmaf_rn(probs[h * nth + j + i], a[i],
                                               contrib[h]);
