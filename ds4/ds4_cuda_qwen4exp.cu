 ...
 */
/* This build's note auto09180327_139 records that removing every tensor
 * core instruction from one projection kernel makes it slower, which means
 * the arithmetic was never its limiter; ablation of its global loads takes
 * a third off the wall, and a variant with identical instruction counts and
