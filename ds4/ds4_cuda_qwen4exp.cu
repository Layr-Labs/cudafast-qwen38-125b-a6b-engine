...
#undef QWEN4EXP_GATEUP_IMPL
    ...
    /* THE ROUTING BROADCAST, AND ONLY THAT.  `Vector` gates two independent
     * things in this kernel: where `e` comes from, and -- on the byte path --
     * whether the activation accumulate takes its vector arm.  The q5_1 arm
     * this checkpoint runs is compiled from the word path above that byte
     * path and passes VectorX = false explicitly, so for DS4_QWEN4EXP_TY_q5_1
     * the ONLY thing this flag moves is the source of `e`: one load of
     * `selected` per row, hoisted out of the slot loop and broadcast with
     * __shfl_sync, in place of a fresh dependent load on every (slot, row).
     * That load sits on the critical path -- `drow` cannot be formed, and so
     * the weight read cannot issue, until it lands -- and it is the same ID
     * every block on this row re-reads.  Two registers (`route[R]`), no
     * change to scratch layout, and the ID is bit-identical: slot < 32 and
     * lane `slot` is exactly the lane that loaded slot's ID.
     *
     * `route` holds one ID per lane, so the broadcast needs the slot count to
     * fit a warp.  The q8_0 and generic arms stay on the byte path and keep
     * `false`, so this stays one variable. */
    const bool dn_route = n_expert_used <= 32u &&
        getenv("DS4_QWEN4EXP_NO_DOWN_ROUTE") == NULL;
...
#define QWEN4EXP_DOWN(R) do { \
    if (specialize && down_slab->type == DS4_QWEN4EXP_TY_q5_1) { \
        if (dn_route) { QWEN4EXP_DOWN_IMPL(R, DS4_QWEN4EXP_TY_q5_1, true); } \
