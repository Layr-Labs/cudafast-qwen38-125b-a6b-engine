...
/* Widest routed-expert count any ds4 variant declares.  ds4.c's DS4_MAX_EXPERT
 * is the authority and static-asserts against this value; the number lives here
 * because ds4_cuda.cu does not include ds4_gpu.h, and its router kernels size a
 * per-token `taken[]` scratch array and refuse wider models by this bound.
 * qwen4exp routes 10 of 512, which is what raised it from 384. */
#define DS4_GPU_MAX_EXPERT 512u
...
/* Put the dense Q8_0 projections on the int8 tensor-core GEMM.
 *
 * ds4_gpu_matmul_q8_0_decode_rows_exact_tensor is reached by more than one
 * model family, and the MMA kernel's reduction order is not the eight-row
 * tile's, so switching every caller at once would move the GLM and DeepSeek
 * towers' numbers for a gain nobody has measured on them.  The qwen4exp loader
 * calls this; the other families keep the tile they were tuned and pinned
 * against.  Off until called, and once on it stays on for the process. */
