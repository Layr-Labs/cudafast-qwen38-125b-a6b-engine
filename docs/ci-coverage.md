...
| `lint` | `ubuntu-latest` | `tools/test-setup-onboarding.sh` — `setup.sh` step 2 against a tiny pinned snapshot served from a `file://` base. Default path (`MLXFAST_TARGET_SNAPSHOT_DIR` unset): a fresh machine fills the default directory and verifies it, a rerun fetches nothing and reads no shard (the marker answers), a truncated file is fetched again alone, a rewritten file and a corrupt source are refused by name, a changed pin invalidates the marker, a stale lock refuses. Staged path (set): verified in full on every run, no marker, and a missing directory, a missing file or a short file refuses with nothing fetched or created. The download skip keeps its meaning |
| `lint` | `ubuntu-latest` | `tools/test-local-baseline.sh` — the real `tools/local-baseline.sh`, root proxy and facade with a stub benchd: the defaults and every override reach benchd's argv, and a missing engine, snapshot or golden stops before benchd is spawned |
...
| `lint` | `ubuntu-latest` | `tools/test-participant-engine-edit.sh` — a participant may edit the engine: the real manifest lists `ds4/`, benchd's harness hash does not move for an engine edit but does for a harness or `tools/` edit, and an engine edit changes the build-cache key and still builds |
