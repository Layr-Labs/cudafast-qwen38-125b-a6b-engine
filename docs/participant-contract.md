...
than armed. The flag is `false` since the engine change (port notes section
10): the pinned goldens describe the previous engine. It flips to `true` when
...
pin -- byte count first, then sha256 -- and refuses on any mismatch. Each
golden carries its OWN serial baseline in
`benchmark.baseline_prefill_seconds_per_token` and
`benchmark.baseline_decode_seconds_per_token`; a scored run of one prompt scores
against that prompt's own pair.

**The launch reference candidate is stock SERIAL.** No speculative config,
`num_speculative_tokens` 0. The scored serve config FOLLOWS THE SUBMISSION's
declared spec (David MTP-0 ruling): `tools/serve-up.sh` defaults to a serial
serve (`SERVE_UP_SPECULATIVE=0`) and boots the MTP speculative config ONLY when a
submission's declared spec asks for it. The ranked workflow sets the value
explicitly and never relies on a hardcoded MTP default. The native MTP head is
the participants' improvement path: a submission that declares MTP is a separate,
later entry.

Because the launch reference is serial, scored against the serial baseline pinned
in each golden, the expected launch composite is approximately 1.000, within the
acceptance bands (prefill +/-5 %, decode +2 % up). That run is a NULL CONTROL of
the scored pipeline against the just-pinned baseline: a composite outside
approximately 1 +/- band is a finding, not a pass.

DISPLAY CONVENTION (David standing rule). The golden baseline fields and benchd's
capture are stored in seconds per token -- benchd's internal representation only.
Every human-facing number shows tokens per second (tok/s = 1 / seconds per
token). The live golden's serial decode baseline is 0.06451959972265625 s/tok, shown as
15.50 tok/s.
...
| `timed_prompt_pool[]`, 8 goldens | Checked into git under `correctness_prompts/qwen3.8-125b-a6b-cuda-v1/`, pinned by `{sha256, bytes}` in the fixture. | **Yes.** They are in your clone. Each carries its own serial baseline. |
...
`[self-hosted, Linux, ARM64, qwen3.8-125b-a6b-cuda-v1]`, and the box is not staged.
