...
slot, each carrying its OWN serial baseline in
`benchmark.baseline_prefill_seconds_per_token` and
`benchmark.baseline_decode_seconds_per_token`. `index.tsv` pins each by
...
SERIAL (`num_speculative_tokens` 0). The scored serve follows the submission's
declared spec: `tools/serve-up.sh` defaults to a serial serve
(`SERVE_UP_SPECULATIVE=0`) and boots MTP only when a submission declares it; the
ranked workflow sets it explicitly, never a hardcoded default. Scored against the
serial baseline pinned in each golden, the launch reference's expected composite
is approximately 1.000 within bands (prefill +/-5 %, decode +2 % up) -- a NULL
CONTROL of the scored pipeline; a value outside approximately 1 +/- band is a
finding, not a pass. The native MTP head is the participants' improvement path (a
later, separate submission). Human-facing numbers show tokens per second: the
baseline is stored in seconds per token (benchd's internal representation), and
0.06451959972265625 s/tok displays as 15.50 tok/s.
...
* THE RANKED PIPELINE IS NOW LINUX-AUTHORED, WITH TWO BOX STEPS LEFT.
  ...
  The fixture was ARMED at the vLLM epoch and is UNARMED again since section 10
  (`official_scoring_enabled: false`); at that epoch every pool slot
  pinned, `live_golden: botany`), so a dispatch waits on those two box steps,
  not on the fixture.
...
### 11.8 Two things this tool cannot do

These are limits of the benchmarker, not decisions of this repository.

1. **The recorder omits the baseline pair.**
   `crates/benchctl/src/bin/record-correctness-golden.rs:478-479` (at
   `44b220a`, where the crate still carried the old name)
   sets `baseline_prefill_seconds_per_token` and
   `baseline_decode_seconds_per_token` to `None`, and both fields are
   `skip_serializing_if = "Option::is_none"`. The recorder therefore writes
   neither key: the golden holds no baseline at all.

   The goldens pinned today DO hold the pair. `botany` holds
   `0.0004879835673828125` for prefill and `0.06451959972265625` for decode. A
   re-author LOSES that pair.

   An official run needs the pair from the golden or from the `--baseline-*`
   flags (`crates/benchd/src/main.rs`, `resolve_paired_baselines`). The
   re-authored set is therefore not scorable until the calibration lane measures
   the pair and stamps it into each golden.

2. **Only depth 1 is reachable.** The adapter refuses draft depths 2 and 3 by
