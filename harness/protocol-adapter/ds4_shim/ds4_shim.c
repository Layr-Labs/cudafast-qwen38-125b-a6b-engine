                ...
                for (int sweep = 0; sweep < WARM_SWEEPS; sweep++) {
                    ...
                    for (int style = 0; style < WARM_STYLES; style++) {
                        ...
                        for (int r = 0; r < WARM_MAX_ROUNDS && quiet < WARM_QUIET
                            ...
                            /* A free run spends its budget down to one, and
                             * its last round cannot draft: it verifies a
                             * single row.  Alternate the budget so that width
                             * is reached here and not inside a timed window. */
                            const int warm_budget = (r & 3) == 3 ? 1 : 2;
                            const int n = ds4s_eval_speculative(h, t, warm_budget,
                                                                out, WARM_CAP);
                            total_rounds++;
                            if (n <= 0) break;
                            t = out[n - 1];
