...
ds4s_handle *ds4s_open(const char *model_path, const char *mtp_head_path,
    ...
    if (getenv("DS4_SHIM_NO_WARMUP") == NULL) {
        ...
        /* The invariant the paragraph above argues for, enforced by the compiler
         * instead of resting on the prose.  One round cannot reveal both
         * parities -- parity flips only on a round that swaps -- so a value low
         * enough to leave one uncaptured moves a whole-stack capture into the
         * timed window.  Four is the floor at which both are even reachable;
         * twelve is what the argument above chose, and this makes lowering it a
         * build failure rather than a silent regression. */
        _Static_assert(WARM_ROUNDS >= 4,
                       "WARM_ROUNDS must be able to reach both recurrent-buffer "
                       "parities; a parity left uncaptured is captured inside "
                       "the timed window");
        if (vocab > 16) {
