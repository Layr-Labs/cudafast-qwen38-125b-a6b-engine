...
int main(void) {
     ...
     * refuses to attach. This also runs bounded device-local startup tuning
     * on private synthetic buffers, once HERE -- after the one load, before
     * the socket binds. No request exists and no model forward is moved from
     * a timed phase. On device-query failure the string is empty and the identity is exactly
